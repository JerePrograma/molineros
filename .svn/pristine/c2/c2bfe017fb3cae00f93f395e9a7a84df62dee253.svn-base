CREATE OR REPLACE FUNCTION novedades_sss.buscar_novedades_empleadores_cambio_plan(IN periodo_desde_p date, IN periodo_hasta_p date)
  RETURNS TABLE(plan_actual_desc character varying, plan_actual_id integer, cuil_titular character varying, inte integer, apellido character varying, nombre character varying, periodo timestamp without time zone, importeaportesocialuoma numeric, importearticulo46 numeric, importecuotaamtima numeric, importecuotasocialuoma numeric, importecuotausufructo numeric, importeadherenteamtima numeric, empresa_cuit character varying, ospim boolean, uoma boolean, amtimaadherente boolean, amtimacuota boolean, usufructo boolean, plan_que_corresponde_desc character varying, plan_que_corresponde_id integer, total_registros_v integer) AS
$BODY$
...
create temp table novedad_empleador_result as 
	select p.descripcion as plan_actual_desc, 
	       ap.id_plan as plan_actual_id,
	...
	from DBLINK('dbname=portalempresas port=5432 host=12.1.1.5 
		 ...
		 left join afi_plan ap on ap.cuil_titular = portal_emp.afiliado_cuil_titular 
		 and ap.inte = portal_emp.afiliado_inte,			 
		 plan p
where ap.id_plan = p.id_plan
and ap.baja_fecha is null
and ap.vigen_hasta=(select max(app.vigen_hasta) from afi_plan app where ap.cuil_titular=app.cuil_titular and app.baja_fecha is null);
...
total_registros_v = count(*) from novedad_empleador_result r where r.plan_actual_id <> r.plan_que_corresponde_id 
             and r.plan_que_corresponde_id is not null;

return query select r.* , total_registros_v
	     from novedad_empleador_result r
	     where r.plan_actual_id <> r.plan_que_corresponde_id 
             and r.plan_que_corresponde_id is not null
             limit 50;
