-- DROP FUNCTION buscar_orden_pago_ospim_reintegros(integer);

CREATE OR REPLACE FUNCTION buscar_orden_pago_ospim_reintegros(IN id_lista integer)
  ...
  r_entidad character varying, r_tipo_reintegro character varying, r_id_reintegro_user integer, 
  rp_id_prestacion integer, rp_id_prestador integer, rp_id_plan integer, rp_fecha_prestacion timestamp without time zone, 
  rp_cantidad numeric, rp_importe numeric, rp_compro_a_debitar_tipo character varying, 
  rp_compro_a_debitar_numero character varying, rp_tercerizado character varying, 
  ...
  rp_codigo character varying, n_id_especialidad integer, n_descripcion character varying, 
  ...
  n_baja_fecha timestamp without time zone, n_baja_usr character varying, n_codigo character varying, 
  ...
  a_id_amtima_baja_fecha timestamp without time zone, a_apellido character varying, 
  ...
  s_descripcion character varying, suma_por_afiliado numeric, suma_total numeric) AS
$BODY$
...
select oppr.id_lista_reintegro_pago, r.id_reintegro, cuil_titular, inte, rp.id_prestacion, sum(rp.importe) as importe, sum(rp.cantidad) as cantidad, sum(rp.importe)*sum(rp.cantidad) as total
from  lista_reintegro_pago_detalle oppr, reintegro r, reintegro_prestacion rp
where 
oppr.id_lista_reintegro_pago = id_lista
and oppr.id_reintegro = r.id_reintegro
and r.id_reintegro = rp.id_reintegro
group by oppr.id_lista_reintegro_pago, r.id_reintegro, r.cuil_titular, r.inte, rp.id_prestacion;
...
return query
	...
	from lista_reintegro_pago_detalle opor,
		reintegro r,
	...
	and r.inte = a.inte 
	and opor.id_lista_reintegro_pago = lrp.id
