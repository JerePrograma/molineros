-- DROP FUNCTION buscar_orden_pago_ospim_reintegros_odo_orto(integer);

CREATE OR REPLACE FUNCTION buscar_orden_pago_ospim_reintegros_odo_orto(IN id_lista integer)
  ...
  s_descripcion character varying, dc_id_reintegro_user integer, dc_id_reintegro integer, dc_nro_cuota smallint, 
...
select oppr.id_lista_reintegro_pago, dc.id_cuota, cuil_titular, inte, rp.id_prestacion, sum(dc.importe) as importe, sum(1) as cantidad, sum(dc.importe)*sum(1) as total
from  lista_reintegro_pago_detalle oppr, detalle_cuota dc, reintegro r, reintegro_prestacion_odo_ort rp
where 
...
group by oppr.id_lista_reintegro_pago, dc.id_cuota, r.cuil_titular, r.inte, rp.id_prestacion;
...
return query
	...
	and a.id_parenteesco_sss=p.codigo;
