-- Function: buscar_recibo_no_os_ingresos_por_fechas(date, date)

-- DROP FUNCTION buscar_recibo_no_os_ingresos_por_fechas(date, date);

CREATE OR REPLACE FUNCTION buscar_recibo_no_os_ingresos_por_fechas(IN p_fecha_ini date, IN p_fecha_fin date)
...
left outer join recibo_no_os_conceptos rc
on ri.id_anticipo_recibo_concepto = rc.id
left outer join recibo_no_os r2
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION buscar_recibo_no_os_ingresos_por_fechas(date, date)
  OWNER TO postgres;
