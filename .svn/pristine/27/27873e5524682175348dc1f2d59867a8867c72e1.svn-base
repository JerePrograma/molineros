...
public class PadronEntidadesUnificadoAction extends PortletAction {
	...
	private static Log _log = LogFactoryUtil.getLog(PadronEntidadesUnificadoAction.class);
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		if (null != soloOP && soloOP.equals("true")) {
			...
			proveedores = TraeListasServiceUtil.getEmpleadoresDeOP(cuit,
			...
			proveedores = TraeListasServiceUtil.getEmpresasIngreso(cuit,
					entidad, sucursal);
			renderRequest.setAttribute("total", proveedores.size());
		} else {
			...
			if (EmpresasIndex.isListoParaUsar() && StringUtils.checkEmpty(cuit)) {
				...
				proveedores = res.getEmpresas();
				renderRequest.setAttribute("total", res.getTotal());
			} else {
				_log.debug("consultando empresas 5");
				proveedores = TraeListasServiceUtil.getEmpleadores(cuit,
						entidad, sucursal);
				if(proveedores!= null && proveedores.size()==1&&id_seccional>0){
					proveedores.get(0).setId_seccional(id_seccional);
				}
				renderRequest.setAttribute("total", proveedores.size());
			}
		}
		...
		if(renderResponse!=null && renderResponse.getNamespace()!=null &&renderResponse.getNamespace().equals("_EST_1_")){
			renderRequest.getPortletSession().setAttribute(PADRON_ENTIDADES, entidades,PortletSession.APPLICATION_SCOPE);
		}else{		
			renderRequest.setAttribute(PADRON_ENTIDADES, entidades);
		}
		return mapping.findForward(getForward(renderRequest,
				"portlet.utils.padron.entidad.view"));
	}
}
