...
/**
 * Esta clase implementa metodos para generar y validar un número de CUIT/CUIL
 * (Codigo Unico de Identificación Tributaria/Código Unico de Identificación
 * Laboral). En la práctica no se recomienda la utilización del metodo generar()
 * ya q si bien se generará un CUIT/CUIL logicamente valido, esto no implica q
...
public final class CuilUtils {
	...
	/**
	 * @autor Sergio Valentini
	 *  
	 * Metodo estatico para validar un numero de CUIT/CUIL. 
	 * en trámite, La SSS solicita un nro de 11 numeros comenzando x 000 + nro dni
	 * 
	 * 
	 * @param cuit N° de CUIT/CUIL como String
	 * 
	 * @return int: 
	 * 				0 si es CUIT no empieza con 0, 
	 * 				1 si el cuit empieza con 000, y valida correctamente, 
	 * 				2 si empieza con 0 pero no valida o no es valido x otra razon (long <> 11 o no numerico)
	 **/
	
	public static int validarCUITEnTramite(String cuit) {
		
		int result = 0; // no corresponde validar CUIT tipo 'en trámite'
		...
		xyStr = cuit.substring(0, 3);
		dniStr = cuit.substring(3, 11);
		...
		if (xyStr.startsWith("0") && xyStr.equals("000") )
			return 1;
