...
<liferay-ui:error exception="<%= PSQLException.class %>" message="asiento-menor-fecha-contable" />
<form action="" method="post" name="<portlet:namespace />editar_asiento" enctype="multipart/form-data">

<input type="hidden" name="id" value="0"/>
<input type="hidden" value="<%=asiento.getTotalDebeAsString() %>" name="totalDebe" id="totalDebe"/>
<input type="hidden" value="<%=asiento.getTotalHaberAsString() %>" name="totalHaber" id="totalHaber"/>
	...
	<tr>
		...
		<td colspan="3">
				...
				<select name="ejercicio" id="ejercicio" onchange="cambioEjercicio()">
					...
					<option value="<%=j%>-<%=j+1%>" <%if (j == (desde==null || desde.get(Calendar.YEAR)==0?hastaAnio:desde.get(Calendar.YEAR))) { %>
						selected="selected" <%} %>>
						<% if(portlet_name.equals("farmacia")){%>
		...
		<td>
		   <input type="radio" name="<portlet:namespace />tipo"  id="<portlet:namespace />tipo"  value="C" onchange="habilitarControl()">Cierre &nbsp;
		   <input type="radio" name="<portlet:namespace />tipo"  id="<portlet:namespace />tipo"  value="A" onchange="habilitarControl()" checked="checked" >Apertura&nbsp;
		   <input type="radio" name="<portlet:namespace />tipo"  id="<portlet:namespace />tipo"  value="I" onchange="habilitarControl()">Aj.Inflación
		   <input type="radio" name="<portlet:namespace />tipo"  id="<portlet:namespace />tipo"  value="E" onchange="habilitarControl()"  >Excel
		</td>
	</tr>	
	<tr>
		<td colspan="4">&nbsp;</td>
		...
		<tr>
			...
			<td>
					<select id="<portlet:namespace />neteo" name="<portlet:namespace />neteo">
						...
						<option
							value="<%=cuenta.getId() %>"><%= cuenta.getNumero() + " - " + cuenta.getCuenta()  %>
						</option>
					    <%}%>					
				    </select>
			</td>
			...
			<td>
			  <td><input type="text" id="<portlet:namespace />importe" name="<portlet:namespace />importe" value="" size="30"  style="text-align: right;"/></td>
			</td>
			
			<td>
			  <div class="select-centro-costo_">
				<select name="<portlet:namespace />ccosto" id="<portlet:namespace />ccosto" style="width: 200px">
			...
			<td><input type="button" id="<portlet:namespace />agregar_renglones" name="<portlet:namespace />agregar_renglones" 
			    value="Agregar" onclick="javascript:<portlet:namespace />agregarRenglon();"/>
		    </td>
...
<fieldset class="block-labels">
  ...
  <table class="lfr-table">
   <tr>
     <td>
       <label style="color:blue">Seleccione Archivo Formato 4 columnas sin título: 1 Nro Cuenta - 2 Debe - 3 Haber - 4 Nro de Centro de Costo(Optativo)(xls versión 97-2003) </label>
     </td>
	 <td  align="center">
...
<tr><td colspan="3"><b>Asiento</b></td></tr>
  <tr>
    ...
    <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
    <td>
     <input type="button" id="<portlet:namespace />precargar" name="<portlet:namespace />precargar" value="Precargar Asiento" onclick="precargarAsiento()"/>&nbsp;
    </td> 
    <td colspan="16" align="right">
		<input id="<portlet:namespace />help_centros" value="Ver Centros de Costo" title="Ver Centros de Costo" type="button"
						onclick="javascript:<portlet:namespace />verCentros();"/>							
	</td>
...
<fieldset class="block-labels">
			...
			<div align="center" id="<portlet:namespace />renglonesDiv">
				<liferay-util:include page="/html/portlet/tesoreria/contabilidad/editar_asientos_apertura_cierre_search_result.jsp">
...
<script type="text/javascript">	
	...
	function guardar(){
		...
		if (jQuery.trim(jQuery("#totalDebe").val()) != jQuery.trim(jQuery("#totalHaber").val())){
		...
		var url = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>"/>&struts_action=/<%=portlet_name%>/asiento_apertura_cierre';
		 url += '&cmd=save';
...
///----- Nuevo  ---------	
	...
	function <portlet:namespace />limpiarImportacion(){
		...
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/asiento_apertura_cierre'
		    + '&cmd=clean'
		url += '&rnd=' + Math.floor(Math.random()*100);
	...
	function precargarAsiento(){
		var ejercicio=jQuery("#ejercicio").val();	
		var tipo=jQuery("input:radio[name='<portlet:namespace />tipo']:checked").val();
		jQuery("#guardando").show();
		var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/<%=portlet_name%>/asiento_apertura_cierre';
		url += '&cmd=precargar';
	...
	function <portlet:namespace />agregarRenglon(){
		jQuery("#guardando").show();
		var cuenta=jQuery('#<portlet:namespace />neteo').val();
		var debeHaber=jQuery('#<portlet:namespace />debeHaber').val();
		var importe=jQuery('#<portlet:namespace />importe').val();
		var centro=jQuery('#<portlet:namespace />ccosto').val();
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/asiento_apertura_cierre'
		    + '&cmd=agregar'
		    + '&cuenta=' + cuenta
		    + '&debeHaber=' + debeHaber
		...
		url += '&rnd=' + Math.floor(Math.random()*100);
		jQuery('#<portlet:namespace />renglonesDiv').load(url, function() {jQuery("#guardando").hide();
		jQuery('#<portlet:namespace />neteo').val("");
		jQuery('#<portlet:namespace />importe').val("");});
	}
	
	function <portlet:namespace />uploadArchivo() {	
		var url = '<portlet:actionURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"/>&struts_action=/<%=portlet_name%>/asiento_apertura_cierre';
		url+='&cmd=uploadxls';
		jQuery("#guardando").show();
		document.<portlet:namespace />editar_asiento.method = 'post';
		submitForm(document.<portlet:namespace />editar_asiento, url);
	...
	function habilitarControl(){
		...
		var tipo=jQuery("input:radio[name='<portlet:namespace />tipo']:checked").val();
		if(tipo=="E"){
		  jQuery("#<portlet:namespace />uploadDiv").show();
		  jQuery("#<portlet:namespace />precargar").hide();
		}else{
		  jQuery("#<portlet:namespace />uploadDiv").hide();		
		  jQuery("#<portlet:namespace />precargar").show();
		}  
	}
	
	var popupE;
	function <portlet:namespace />verCentros(){
	  if(popupE==null)
	    popupE = Liferay.Popup({title:"Centros de Costos Habilitados",modal:true,width:700,onClose: function() { popupE = null;}});
	  var ejercicio=jQuery("#ejercicio").val();
	  var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/<%=portlet_name%>/asiento_apertura_cierre';
	  url += '&cmd=verCentros';
	  url += '&ejercicio='+ejercicio;
	...
	function traerCentrosCostos(nro) {
	    ...
	    jQuery.ajax({   
		        ...
		        async:true,
		        success: function(data){	
		            jQuery('.select-centro-costo_' + ' select').html(data).fadeIn();
		        }
		 });
