...
public class AgregarPagoConvenioAction extends PortletAction {

	private static Log logger = LogFactoryUtil
			.getLog(AgregarPagoConvenioAction.class);

	public ActionForward render(ActionMapping mapping, ActionForm form,
			PortletConfig portletConfig, RenderRequest renderRequest,
			RenderResponse renderResponse) throws Exception {
		logger.debug("Agregando cheque a convenio");
		...
		if (tipo_pago.equals("cheque") && !existeCuota(list, convenioPago)) {
			...
			list.add(convenioPago);
		} else if (tipo_pago.equals("deposito")
		...
		logger.debug("Saliendo de agregar cheque a convenio");
	...
	private ConvenioPago getConvenioPago(RenderRequest renderRequest)
		...
		String fechaPagoDia = ParamUtil.getString(renderRequest,"fechaPagoDiaCheque");
		String fechaPagoMes = ParamUtil.getString(renderRequest,"fechaPagoMesCheque");
		fechaPagoMes = String.valueOf(Integer.valueOf(fechaPagoMes) + 1);
		String fechaPagoAnio = ParamUtil.getString(renderRequest,"fechaPagoAnioCheque");
		Date fechaPago = format.parse(fechaPagoDia + "-" + fechaPagoMes + "-" + fechaPagoAnio);
