CREATE OR REPLACE FUNCTION transf_importe_a_texto(num numeric) 
...
BEGIN
	...
	FOR i IN 1..4 LOOP
		...
		IF i = 4 THEN
			...
			-- Empieza con los decimales
				txt = txt || ' con ' || lpad(cast(n as character varying),2,'0') || '/100.';
			END IF;
		END IF;
		-- Centenas 
		IF n > 99 THEN
	...
	IF negativo = TRUE THEN
	   txt= '-' || txt;
	END IF;
    --Agrego el barra 100 para los centavos
...
ALTER FUNCTION public.transf_importe_a_texto(num numeric) OWNER TO postgres;
