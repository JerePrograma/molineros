...
<portlet:defineObjects/>
			<%
					...
					List<ComprobanteHospital> comprobantes = (ArrayList<ComprobanteHospital>)request.getSession().getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_FARMACIA);
					...
					if(null!=comprobantes){					
					 	...
					 	for (int i = 0; i < comprobantes.size(); i++) {
                            ...
                            row.addText( comp.getAcreedorEmpresa().getRazon_soc());
	 						...
	 						sbe.append(!"CERRADO".equals(comp.getReclamoEstado())?"red":"green");
	 						sbe.append( "';>");
	 						sbe.append(String.valueOf(comp.getReclamoId()!=null?comp.getReclamoId():0));
	 						sbe.append( "</p>");
...
<script type="text/javascript">
	...
	function editaComprobanteProveedor(cuit,tipo,letra,ptovta,numero){
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/comprobantes/comprobantes_portal_proveedores_farmacia" /></portlet:renderURL>';
        url += "&accion=edit";
        url += '&cuit='+cuit;
        url += "&tipo="+tipo;
