...
<portlet:defineObjects/>
			...
			PortletURL portletURLTercerizadora = renderResponse.createRenderURL();
	 		...
	 		headerNamesTercerizadora.add("Debitado");
			...
			if(null!=comprobantes){
 				List resultRowsInspector = searchContainer.getResultRows();
 			 	for (int i = 0; i < comprobantes.size(); i++) {
 			 		...
 			 		row.addText(formatter.format(comprobante.getLiquidacionPrestacion().getImporteTotal()));
 			 		row.addText(formatter.format(comprobante.getLiquidacionPrestacion().getLiquidacion().getDebitado()));
 			 		...
 			 		Boolean marcar=false;
	 					...
	 					if(seguimiento.getTratamientos()!=null &&	seguimiento.getTratamientos().size()>0){
	 						for(int xi=0;xi<seguimiento.getTratamientos().size();xi++){
	 							if(comprobante.getTratamientoId() ==seguimiento.getTratamientos().get(xi).getId_tratamiento()){
	 							  if(seguimiento.getTratamientos().get(xi).getComprobantes()!=null){	
	 								for(ComprobanteTratamientoDiscapacidad td:seguimiento.getTratamientos().get(xi).getComprobantes()){
	 								     if(comprobante.getLiquidacionPrestacion().getId_liquidacion()== td.getLiquidacionPrestacion().getId_liquidacion() &&
	 								    	comprobante.getLiquidacionPrestacion().getId_prestacion() == td.getLiquidacionPrestacion().getId_prestacion() &&
	 								    	comprobante.getLiquidacionPrestacion().getOrden()==td.getLiquidacionPrestacion().getOrden()
	 								    	){
	 								        marcar=true;
	 								        break;
	 								     }
	...
	<liferay-ui:search-iterator paginate="false"  searchContainer="<%=searchContainer%>" />
   	...
   	<table>
		<tr>
			...
			onClick="<portlet:namespace />seleccionComprobantes();" /></td>				
		</tr>
	</table>


<script type="text/javascript">
		function <portlet:namespace />seleccionComprobantes() {
			...
			<portlet:namespace />seleccionarComprobantes(tratValue);
		}
</script>			