...
<%
	...
	String rps=(String)request.getSession().getAttribute("OPS_PROCESAR_IMAGENES");
...
<form action="" method="post" name="<portlet:namespace />fmSSS" enctype="multipart/form-data"> 
	...
	<legend>
	    ...
	    <tr>
		<td>Ordenes de pago a Exportar</td>
		<td> 
	...
	<tr>
	<table><tr>
	<td><label style="color:blue; font-size: 15px" >Los números de Ordenes de pago deben estar separados por punto y coma</label></td>
	</tr></table>
	</tr>
   </table>	
...
function <portlet:namespace />uploadArchivo() {
		var url = '<portlet:actionURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"/>&struts_action=/<%=portlet_name%>/editar_orden_pago_ospim_entry';
		url = url + '&cmd=upload'; 
...
jQuery('#<portlet:namespace />exportar-imagenes').click(function exportarImagenes(){
		...
		if(trim(rps).length == 0 ){
			alert("Debe ingresar lista de Ordenes de Pago separados por punto y coma");
			jQuery('#<portlet:namespace />in').focus();
			return false;
		}
		
		window.location.href ='/txtservlet/?reporte=ORDENES_PAGO_EXPORTAR_IMAGENES'
			+'&in='+rps+'&entidad=2' ;	
});
