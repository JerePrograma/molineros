...
<%
	...
	fechaDesde.add(Calendar.DATE, -30); 
...
<fieldset class="block-labels"><legend><liferay-ui:message key="grupo-filtro-busqueda-historico-crm" /></legend>
...
<table class="lfr-table">
	<tr>
		...
		<td colspan="3"><input id="<portlet:namespace />buscar"
			...
			onClick="javascript:<portlet:namespace />buscarContactosHistoricos();" /></td>
	</tr>
</table>
</fieldset>
...
<div align="center" id="<portlet:namespace />busquedaContactoHistoricoDiv">
</div>
</fieldset>

<script type="text/javascript">
jQuery('#<portlet:namespace />buscando').hide();
function <portlet:namespace />buscarContactosHistoricos(){
	...
	var integ = <%=afiliado.getInte()%>
	
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/afiliados/historico_contactos&cuil_titular='+cuilTitular+'&inte='+integ;
	jQuery('#<portlet:namespace />buscando').show();		
	jQuery("#<portlet:namespace/>busquedaContactoHistoricoDiv").load(url,{desde_dia:desde_dia, desde_mes:desde_mes, desde_anio:desde_anio, hasta_dia:hasta_dia,
