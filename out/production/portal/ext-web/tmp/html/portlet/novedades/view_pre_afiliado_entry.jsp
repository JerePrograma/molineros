...
portletURL.setParameter("cmd", Constants.VIEW);
portletURL.setParameter("cuil_titular", afiliado.getCuil_titular());
portletURL.setParameter("inte", String.valueOf(afiliado.getInte())); 
portletURL.setParameter("id", String.valueOf(afiliado.getId()));
...
String cuil_titular = afiliado.getCuil_titular();
%> 
...
<liferay-ui-custom:tabs 
	...
	<c:choose>
		<c:when test='<%= tabsA.equals("informacion_general")%>'>
			<%-- <%if(esTitular){%> --%>
			<liferay-util:include page="/html/portlet/novedades/view_pre_afiliado.jsp" >
				<liferay-util:param name="es_titular_p" value="<%=String.valueOf(esTitular)%>"/>			
				<liferay-util:param name="<%=Constants.CMD%>" value="<%=Constants.VIEW%>"/>	
