...
%>
<form action="" method="post" name="<portlet:namespace />editar_param_cuenta" >
...
<table style="width: 50%">
	...
	<tr>
		<td>Parametro:</td>
		<td>${parametroCuenta.parametro}
		<input type="hidden" name="parametro" value="${parametroCuenta.parametro}"/>
		</td>
	</tr>
	<tr>
		<td>Detalle:</td>
		<td>${parametroCuenta.observaciones}
			<input type="hidden" name="observaciones" value="${parametroCuenta.observaciones}"/>
		</td>
	</tr>
	<tr>
		<td>Cuenta:&nbsp;</td>
...
function guargar(){
		...
		var url = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>"/>&struts_action=/<%=portlet_name%>/editar_parametro_especial_cuenta';
		document.<portlet:namespace />editar_param_cuenta.method = 'post';
		submitForm(document.<portlet:namespace />editar_param_cuenta, url);
}

	jQuery(document).ready(function() {
		jQuery("#guardando").hide();
		jQuery("#cuentaId").val("${parametroCuenta.planCuentas.id}");	
	});
