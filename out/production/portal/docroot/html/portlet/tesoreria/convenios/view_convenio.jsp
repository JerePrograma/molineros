...
String update=(String)request.getAttribute(Constants.CMD);

if (convenio !=null && convenio.getId() != 0 && null!=update && !update.equals(Constants.UPDATE)){
	esEdicion = false;
}else if(null!=update && update.equals(Constants.UPDATE)){
	esEdicion = true;
}
...
<table class="lfr-table">
		...
		<tr> 
			...
			<td><input type="text" <%if(!esEdicion){%>readonly="readonly"<%}%> name="<portlet:namespace />convenio_nro" id="<portlet:namespace />convenio_nro" 
	...
	<tr>
		...
		<fieldset class="block-labels">
			<legend><liferay-ui:message		key="actas-saldo" /></legend>
					<liferay-util:include page="/html/portlet/tesoreria/convenios/convenios_actas_asociadas_agregar.jsp">
			...
			<fieldset class="block-labels"><legend><liferay-ui:message key="detalle-pagos" /></legend>
				<table>
					<tr>
						...
						<td>
							<input type="text" name="<portlet:namespace />capital" id="<portlet:namespace />capital" <%if(!esEdicion){%>readonly="readonly"<%}%> onChange=" sumarTodo()" 
					...
					<tr>
						...
						<td>
							<input type="text"	name="<portlet:namespace />inte" id="<portlet:namespace />inte"  <%if(!esEdicion){%>readonly="readonly"<%}%> onChange=" sumarTodo()"
					...
					<tr>
						...
						<td>
							<input type="text" name="<portlet:namespace />totalPagos" id="<portlet:namespace />totalPagos"  <%if(!esEdicion){%>readonly="readonly"<%}%> />
						</td>
					</tr>
				</table>
				</fieldset>
...
<table class="lfr-table" width="100%">
		<tr>
			<td width="100%">
				<liferay-util:include page="/html/portlet/tesoreria/convenios/formas_pago_agregar.jsp">
					<liferay-util:param name="esEdicion" value="<%=String.valueOf(esEdicion) %>"/>
				</liferay-util:include>
			</td>
		</tr>
...
<input type="hidden" value="<%=null!=convenio?convenio.getId():""%>" name="<portlet:namespace />convenio_id" id="<portlet:namespace />convenio_id"/>
...
<script type="text/javascript">	 
	function <portlet:namespace />saveConvenio() {
		...
		if (<portlet:namespace />validarCampos()) {
			<%-- document.<portlet:namespace />conv.<portlet:namespace /><%= Constants.CMD %>.value = "<%if(null!=update && update.equals(Constants.UPDATE)){%><%=Constants.UPDATE%><%}else{%><%=Constants.ADD%><%}%>";	 --%>
			...
			var url = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/tesoreria/editar_convenios_entry" /></portlet:actionURL>';			
	...
	function actualizarEstadoSeguimiento(){
		...
		var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/tesoreria/convenioCambiarEstadoSeguimiento&id_estado='+id_estado_seg+'&id_convenio='+id_convenio;
