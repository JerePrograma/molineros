...
List<IntegracionCabeceraDS> archivos= IntegracionServiceUtil.lotesSSS();
...
headerNames.add("Det");
headerNames.add("Err");
headerNames.add("Ajustes");
headerNames.add("Cerrar");
headerNames.add("A Hist.");
headerNames.add("Liq");
...
if (archivos != null && !archivos.isEmpty()){
	...
	for (int i = 0; i < archivos.size(); i++) {	    
        ...
        sb10.append(" title=\"Errores\"");
 		...
 		if( /*liq.getFechaCierre()==null &&*/ (liq.getLiquidados()==null || liq.getLiquidados()==0 || liq.getLiquidados()<liq.getDetalleProcesadosOK()) && rolGeneracion ){
	 		sb13.append("&nbsp;&nbsp;<img alt=\"Ajustes Lote\" src=\"");
			sb13.append(themeDisplay.getPathThemeImages());
	 		sb13.append("/common/configuration.png\" onClick=\"javascript:ajusteLoteIntegracion('");
	 		sb13.append(liq.getLoteSSS() );
	 		sb13.append("');\"");
            sb13.append(" title=\"Ajustes\"");
 		    sb13.append("/>");
		}else{
			sb13.append("");
		}
 		...
 		if( liq.getFechaCierre()==null && (liq.getLiquidados()==null || liq.getLiquidados()==0) && rolGeneracion ){
	 		sb11.append("&nbsp;&nbsp;<img alt=\"Cerrar Lote\" src=\"");
			sb11.append(themeDisplay.getPathThemeImages());
	 		sb11.append("/common/define_permissions.png\" onClick=\"javascript:cerrarLoteIntegracion('");
	 		sb11.append(liq.getLoteSSS() );
	 		sb11.append("');\"");
 		...
 		if( liq.getFechaCierre()==null && (liq.getLiquidados()==null || liq.getLiquidados()==0) && rolGeneracion ){
	 		sb12.append("&nbsp;&nbsp;<img alt=\"Pasar a Historico\" src=\"");
			sb12.append(themeDisplay.getPathThemeImages());
	 		sb12.append("/common/time.png\" onClick=\"javascript:historicoLoteIntegracion('");
	 		sb12.append(liq.getLoteSSS() );
	 		sb12.append("');\"");
            sb12.append(" title=\"Pasar a Historico\"");
 		    sb12.append("/>");
		}else{
			sb12.append("");
		}
...
var popup;
function verDetalleLoteIntegracionSSS(id,tipo){
...
function liquidarLoteIntegracion(idLote){
	...
	var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/autorizaciones/integracion_editar';
   	url = url+'&<%= Constants.CMD %>'+'='+'liquidar_lote'+'&id_lote='+idLote;
...
function detalleLiquidacionLoteIntegracion(idLote){
	popup = Liferay.Popup({title:"Detalle Liquidación",modal:true,width:720});
	var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/autorizaciones/integracion_editar';
   	url = url+'&<%= Constants.CMD %>'+'='+'detalle_liquidacion_op_lote'+'&nrolote='+idLote;
...
function imprimirOPLoteIntegracion(idLote){
	...
	var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/autorizaciones/integracion_editar';
   	url = url+'&<%= Constants.CMD %>'+'='+'impresion_op_lote'+'&nrolote='+idLote;
   	jQuery(popup).load(url);
   	  	
}

function cerrarLoteIntegracion(idLote){
	var confirmar = false;
	confirmar=confirm ('Esta seguro de cerrar el Período?');
	if(confirmar){
	  jQuery('#<portlet:namespace />buscando').show();
	  var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/autorizaciones/integracion_editar';
   	  url = url+'&<%= Constants.CMD %>'+'='+'cerrar_lote'+'&id_lote='+idLote;
...
function ajusteLoteIntegracion(idLote){
	popup = Liferay.Popup({title:"Ajuste Lote",modal:true,width:1040, position:['center',30]});
	var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/autorizaciones/integracion_editar';
   	url = url+'&<%= Constants.CMD %>'+'='+'ajustes_lote'+'&nrolote='+idLote;
   	jQuery(popup).load(url);
