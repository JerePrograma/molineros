CREATE OR REPLACE FUNCTION reporte_sss_331_recetas_anexo(fecha_ini_p date, fecha_fin_p date)
  RETURNS SETOF reporte_sss_331_recetas_detalle AS
$BODY$
BEGIN
...
return query
select  per_codigo, nro_prestador, prestador, cast(count(*) as int), cast(sum(monto_ospim) as double precision)
from recetas r
...
group by per_codigo, nro_prestador, prestador;
