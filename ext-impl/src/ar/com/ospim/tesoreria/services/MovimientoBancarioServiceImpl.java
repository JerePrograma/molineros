...
public class MovimientoBancarioServiceImpl {
	...
	public int grabaMovimientoBancario(MovimientoBancario mov, String usr,
		...
		try {
			...
			stmt.setInt(cont++, mov.getCentroCosto().getId());
	...
	public int editaMovimientoBancario(MovimientoBancario mov, String usr,
		...
		try {
			...
			if(mov.getCentroCosto()!=null && mov.getCentroCosto().getId()!=null) {
				  stmt.setInt(15,mov.getCentroCosto().getId());
			}else {
		...
		try {
			...
			while (rs.next()) {
				MovimientoBancario mov = new MovimientoBancario(
						...
						rs.getString("desc_centro_costo"));
				result.add(mov);
			}
		} catch (Exception e) {
	...
	public List<? extends ItemSubdiarioEgreso> reporteParaSubdiario(
		...
		try {
			...
			while (rs.next()) {
				...
				centro.setId(rs.getInt("id_centro_costo"));
				centro.setDescripcion(rs.getString("descripcion_cc"));
				mov.setCentroCosto(centro);
				...
				if (mov.getBaja_fecha() != null) {
					...
					movBaja.setCentroCosto(mov.getCentroCosto());
