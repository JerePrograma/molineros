...
public class PrecioPlanSuperadorAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		PrecioPlanSuperador precio = null;
		...
		if (!StringUtils.checkEmpty(cmd)) {
			...
			if("NEW".equals(cmd) ){ 
				...
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION , precio);
				
				List<Plan> planes=TraeListasServiceUtil.getPlanesFacturables();
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PLANES,planes);
				
				List<Parentesco> parentescos = TraeListasServiceUtil.getParentescosFacturables();
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PARENTESCOS,parentescos);
				
				List<Provincia> provinciasPrecio = TraeListasServiceUtil.getProvinciasFacturables();
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PROVINCIAS,provinciasPrecio);
				...
				return mapping.findForward(getForward(renderRequest,
						"portlet.tesoreria.facturacion.listaprecios.edit"));
			}
			
			if(cmd.equals(Constants.EDIT) ){
				precio= LiquidacionPlanesSuperadoresServiceUtil.getPlanSuperador(idPrecio.intValue());
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION, precio);
			...
			if(cmd.equals("agregarParentesco") ){
				...
				List<Parentesco>listaParentescos = (List<Parentesco>) session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PARENTESCOS);
				precio =(PrecioPlanSuperador)  session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION);
				...
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PARENTESCOS , listaParentescos);
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_EDICION , precio);
				return mapping
        				.findForward("portlet.tesoreria.facturacion.listaprecios.parentescos");
			}
			...
			if(cmd.equals("agregarPlan") ){
				...
				List<Plan>listaPlanes = (List<Plan>) session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PLANES);
				precio =(PrecioPlanSuperador)  session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION);
				...
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PLANES , listaPlanes);
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_EDICION , precio);
				return mapping
        				.findForward("portlet.tesoreria.facturacion.listaprecios.planes");
			}
			...
			if(cmd.equals("agregarLocalidad") ){
				...
				List<Provincia>listaLocalidades = (List<Provincia>) session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PROVINCIAS);
				precio =(PrecioPlanSuperador)  session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION);
				...
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_SESSION_PROVINCIAS , listaLocalidades);
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_EDICION , precio);
				return mapping
        				.findForward("portlet.tesoreria.facturacion.listaprecios.localidades");
			}
			...
			if(cmd.equals("agregarValor") ){
				...
				p.setPrecioUnitario(new BigDecimal(importe));
				p.setId(precio.getValores().size()+1);
				precio.getValores().add(p);
				
				session.setAttribute(WebKeysTesoreria.PRECIO_EN_EDICION , precio);
				return mapping
        				.findForward("portlet.tesoreria.facturacion.listaprecios.valores");
			}
			
			
			if(cmd.equals("sacarValor") ){
				Integer idValor = ParamUtil.getInteger(renderRequest,"idValor");
				precio =(PrecioPlanSuperador)  session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION);
				...
				List<Producto>newValores =new ArrayList<Producto>();
				for(Producto p:precio.getValores()) {
					if(p.getId()!=idValor) {
						newValores.add(p);
					}
				}
				Integer pos =1;
				for(Producto p:newValores) {
					p.setId(pos++);
				}
				
				precio.setValores(newValores);
			...
			if(cmd.equals(Constants.UPDATE) ){
				precio = (PrecioPlanSuperador) session.getAttribute(WebKeysTesoreria.PRECIO_EN_SESSION);
				actualizaPrecio(precio,renderRequest);
				if(idPrecio == 0){
					idPrecio=LiquidacionPlanesSuperadoresServiceUtil.addPrecioPlanSuperador(precio, user.getScreenName());
					 ...
					 precio.setValores(new ArrayList<Producto>()); 
				}else if(idPrecio!=0){
					precio.setId(idPrecio.intValue());
					idPrecio=LiquidacionPlanesSuperadoresServiceUtil.updatePrecioPlanSuperador(precio, user.getScreenName());
				...
				return mapping
		 				.findForward("portlet.tesoreria.facturacion.listaprecios.edit");
		   }
			
           if(cmd.equals(Constants.DELETE) ){ 
            	
            	try {           		
                   LiquidacionPlanesSuperadoresServiceUtil.deletePlanSuperadorPrecio(idPrecio.intValue());
	            } catch (Exception e) {
	                _log.error("Error eliminando precio");
	            }
            	
            	List<PrecioPlanSuperador>ln=   (List<PrecioPlanSuperador>) session.getAttribute(WebKeysTesoreria.PRECIOS_RESULT);
            	List<PrecioPlanSuperador>lista=new ArrayList<PrecioPlanSuperador>();
	           	for(PrecioPlanSuperador n:ln){
	            	if(!n.getId().equals(idPrecio.intValue() )){
	            	   lista.add(n);
	            	}
	            }
	           	
	            session.setAttribute(WebKeysTesoreria.PRECIOS_RESULT,lista);
	            return mapping
	        				.findForward("portlet.tesoreria.facturacion.listaprecios.result");
			}
            ...
            if(cmd.equals("atras") ){ 
            	return mapping
        				.findForward("portlet.tesoreria.facturacion.listaprecios");
            }
	...
	private void actualizaPrecio(PrecioPlanSuperador precio,RenderRequest renderRequest){
