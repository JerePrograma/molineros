...
public class ComprobantesProveedoresHospitalesAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		if (!StringUtils.checkEmpty(cmd)) {
			...
			if(cmd.equals("buscar")){
		           ...
		           return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobantes_search_result_hospitales"));		
			...
			if(cmd.equals(Constants.EDIT) ){
				 ...
				 session.setAttribute(WebKeysComprobantes.COMPROBANTE_HOSPITALES_EN_EDICION, lista.get(0));
				 return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobante_hospitales_editar"));
			}
           
			if(cmd.equals(Constants.UPDATE) ){
				
				comprobante=(ComprobanteHospital)  session.getAttribute(WebKeysComprobantes.COMPROBANTE_HOSPITALES_EN_EDICION);
		...
		return mapping.findForward("portlet.comprobantes.comprobantes_proveedores_hospitales");
...
private void filterComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
	...
	List<ComprobanteHospital> lista = ComprobanteServiceUtil.getListaHospitales(filtro,0);
	
	if("Prestador".equals(sector)) {
		List<ComprobanteHospital> listaRet =new ArrayList<ComprobanteHospital>();
		for(ComprobanteHospital c: lista) {
		   ...
		   if(lp!=null && lp.size()>0){
			   if(c.getIdPrestador()!=null && c.getIdPrestador()!=0) {
				   for(Prestador pr:lp) {
			           if(pr.getId_prestador()==c.getIdPrestador()) {
			        	   if(pr.getConvenioDirecto()) {
			        		   listaRet.add(c);
			        	   }
			        	   break;
			           }
				  ...
				  if(lp.get(0).getConvenioDirecto() ) {
					  listaRet.add(c);
				  }
			   }
		   }	   
	...
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_FILTRO_HOSPITALES ,filtro);
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_HOSPITALES,lista);
...
private void verificarComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
    ...
    List<ComprobanteHospital> lista = (List<ComprobanteHospital>) session.getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_HOSPITALES);
    ...
    if(lista!=null && !lista.isEmpty()) {
      for(Integer i=0;i<lista.size();i++) {
    	if (ids.contains(lista.get(i).getId().toString().trim())) {
    	    ...
    	    listaAux=ComprobanteServiceUtil.getListaHospitales(filtro,0);
	...
	*/
	if(c.getIdPrestador()==null || c.getIdPrestador()==0){
		...
		if("Prestador".equals(c.getSectorDestino())   && !"NCR".equals(c.getTipoComprobante()) &&  !"NDB".equals(c.getTipoComprobante())) {
			   ...
			   if(p.getConvenioDirecto()==null || !p.getConvenioDirecto()) {
			    if(cError.length()>0) cError+=";";
			    cError+="SCD"; 
			   }
		}
...
//generar Liquidacion    
    			if("OK".equalsIgnoreCase(cbte.getError()) &&
    				...
    				/*
    			    Integer idLiquidacion =EditarLiquidacionServiceUtil.getInstance().cargaLiquidacionEntry(new Date(), cbte.getFechaEmision(),
   					cbte.getFechaRecepcion(), cbte.getFechaVencimiento(), cbte.getPeriodoPrestacion(),"O.S.P.I.M.", cbte.getIdPrestador(),
   					0, cbte.getTipoComprobante(), cbte.getLetraComprobante(),
   					cbte.getPtoVenta(), cbte.getNroComprobante(), WebKeysLiquidaciones.LIQUIDACION_PRE, WebKeysLiquidaciones.LIQUIDACION_ESTADO_CARGADO, user.getScreenName(), 
   					null, null, cbte.getImporte(), BigDecimal.ZERO, null, 
   					null, "1", null , BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO);
    			    */
    			
    				Integer idLiquidacion =EditarLiquidacionServiceUtil.cargaLiquidacionEntry(new Date(), cbte.getFechaEmision(),
