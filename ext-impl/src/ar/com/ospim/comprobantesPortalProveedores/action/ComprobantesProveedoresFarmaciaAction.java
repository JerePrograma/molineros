...
public class ComprobantesProveedoresFarmaciaAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		if (!StringUtils.checkEmpty(cmd)) {
			...
			if(cmd.equals("buscar")){
		           ...
		           return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobantes_search_result_farmacia"));		
			...
			if(cmd.equals(Constants.EDIT) ){
				 ...
				 c.setPtoVenta(Integer.parseInt(ptovta));
				 c.setNroComprobante(nro);
				 List<ComprobanteHospital> lista = ComprobanteServiceUtil.getListaFarmacia(c,0);
				 
				 session.setAttribute(WebKeysComprobantes.COMPROBANTE_FARMACIA_EN_EDICION, lista.get(0));
				 return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobante_farmacia_editar"));
			}
           
			if(cmd.equals(Constants.UPDATE) ){
				
				comprobante=(ComprobanteHospital)  session.getAttribute(WebKeysComprobantes.COMPROBANTE_FARMACIA_EN_EDICION);
		...
		return mapping.findForward("portlet.comprobantes.comprobantes_proveedores_farmacia");
...
private void filterComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
	...
	List<ComprobanteHospital> lista = ComprobanteServiceUtil.getListaFarmacia(filtro,0);
	
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_FILTRO_FARMACIA ,filtro);
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_FARMACIA,lista);
}
...
private void verificarComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
    ...
    List<ComprobanteHospital> lista = (List<ComprobanteHospital>) session.getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_FARMACIA);
    ...
    if(lista!=null && !lista.isEmpty()) {
      for(Integer i=0;i<lista.size();i++) {
    	if (ids.contains(lista.get(i).getId().toString().trim())) {
    	    ...
    	    listaAux=ComprobanteServiceUtil.getListaFarmacia(filtro,0);
...
private String verificaComprobante(ComprobanteHospital c) throws Exception {
	...
	*/
	...
	if(c.getReclamoId()==null || c.getReclamoId().doubleValue()==0D) {
		if(cError.length()>0) cError+=";";
		cError+="SRP";
	}else {
...
//generar Liquidacion    
    			if("OK".equalsIgnoreCase(cbte.getError()) &&
                    ...
                    ReclamoPrestacional reclamo = ReclamosPrestacionesServiceUtil.getReclamoPrestacional(cbte.getReclamoId().intValue());     				
/*    				
    				//9 - Tipo nomenclador farmacia
    			    List<Nomenclador> nomencladores = NomencladorServiceUtil.getListaNomenclador(9,"",0, cbte.getCodigoPrestacion() ,false,"");
    			    Integer idPrestacion=0;
 				    for(Nomenclador nom:nomencladores){			   				   
 					   idPrestacion= nom.getId_prestacion();   		// prestacion clinica 		   				    
 				    }	
    			
 				    List<Afiliado> afiliados = EditarAfiliadoServiceUtil.getAfiliadosPorDocumentoInclusoDadoDeBaja(cbte.getAfiliado().getDocu_numero(), "DU");
 				    Afiliado af = new Afiliado();
				    if( afiliados .size()>0){
				    	af =  (Afiliado) afiliados.get(0);
				    }
 */				  
