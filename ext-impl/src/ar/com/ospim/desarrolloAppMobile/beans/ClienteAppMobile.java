		...
		try {
			...
			if (status == 200) {
			    ...
			    try {
			        ...
			        logger.warn("JSON recibido del reintegro: " + items.toString());
	    ...
	    try {
	        try {	        	
	        	if (hasUrl(urlComprobante)) {
	                ...
	                if (extComp == null) {
	                    logger.error("REINTEGRO " + r.getId_reclamo() + " URL sin extensión: " + urlComprobante);
	                } else {
	                    ...
	                    logger.info("REINTEGRO " + r.getId_reclamo() + " Comprobante subido - titulo:" + title + " nombre:" + entry.getName());
                        ...
                        logger.warn("REINTEGRO " + r.getId_reclamo() + " No se pudo descargar comprobante desde: " + urlComprobante);
                    }
	            }
	            } else {
	                logger.info("REINTEGRO " + r.getId_reclamo() + " sin URL de comprobante");
	            }
	           
	        } catch (Exception e) {
	            logger.warn("REINTEGRO " + r.getId_reclamo() + " error al subir comprobante", e);
	        }
	        ...
	        try {
	            if (hasUrl(cbuAutorizante)) {
	                ...
	                if (msgErrorNota == null) {
	                    ...
	                    if (extNota == null) {
	                        logger.error("REINTEGRO " + r.getId_reclamo() + " nota autorizada URL sin extensión): " + cbuAutorizante);
	                    } else {
	                    ...
	                    if (archivoNota != null && archivoNota.exists()) {
	                        ...
	                        logger.info("REINTEGRO " + r.getId_reclamo() + " nota autorizada subida -> " + imagenNotaAutorizada);
	                    } else {
	                        logger.warn("REINTEGRO " + r.getId_reclamo() + " no se pudo descargar nota autorizada");
	                    }
	                }
	                } else {
	                    logger.info("REINTEGRO " + r.getId_reclamo() + " nota autorizada ya existente");
	                }
	            } else {
	                logger.info("REINTEGRO " + r.getId_reclamo() + " sin URL de nota autorizada.");
	            }
	        } catch (Exception e) {
	            logger.warn("REINTEGRO " + r.getId_reclamo() + " error al procesar nota autorizada", e);
	        }
	        ...
	        try {
	            if (hasUrl(urlDocExtra)) {
	                ...
	                if (msgErrorDocExtra == null) {
	                    ...
	                    if (extDocExtra == null) {
	                        logger.error("REINTEGRO " + r.getId_reclamo() + " Doc extra URL sin extensión: " + urlDocExtra);
	                    } else {
	                        ...
	                        if (archivoDocExtra != null && archivoDocExtra.exists()) {
	                            ...
	                            logger.info("REINTEGRO " + r.getId_reclamo() + " Doc extra subido -> " + imagenDocExtra);
	                        } else {
	                            logger.warn("REINTEGRO " + r.getId_reclamo() + " no se pudo descargar doc extra desde: " + urlDocExtra);
	                        }
	                    }
	                } else {
	                    logger.info("[REI " + r.getId_reclamo() + "] Doc extra ya existente: " + msgErrorDocExtra);
	                }
	            } else {
	                logger.info("[REI " + r.getId_reclamo() + "] Sin URL de doc extra.");
	            }
	        } catch (Exception e) {
	            logger.warn("[REI " + r.getId_reclamo() + "] Error al procesar doc extra", e);
	        }	        
	        
	        boolean tengoCBU = (cbuNuevo != null && !cbuNuevo.trim().isEmpty());
	        DLFileEntry entryCbuSubida = null;
	        String imagenCBUName = null;
	        ...
	        try {
	            if (cuilLocal != null && !cuilLocal.trim().isEmpty()) {
	                ...
	                logger.info("[REI " + r.getId_reclamo() + "] Cuentas para CUIL " + cuilLocal + ": " + (cuentas != null ? cuentas.size() : 0));
	            } else {
	                logger.warn("[REI " + r.getId_reclamo() + "] CUIL vacío o nulo");
	            }
	        } catch (Exception e) {
	            logger.warn("[REI " + r.getId_reclamo() + "] No se pudo consultar cuentas", e);
	        }
	            ...
	            try {
	                ...
	                if (msgError == null) {
	                    ...
	                    if (archivoCbu != null && archivoCbu.exists()) {
	                        ...
	                        imagenCBUName = entryCbuSubida.getName();
	                        logger.info("[REI " + r.getId_reclamo() + "] Constancia CBU subida -> " + imagenCBUName);
	                    }
		            }
		          
	                } else {
	                    logger.info("[REI " + r.getId_reclamo() + "] Constancia CBU ya existente para el reclamo: " + msgError);           
	                    ...
	                    try {
	                        ...
	                        if (tmp.getCuenta() != null && tmp.getCuenta().getImagenCBU() != null) {
	                            imagenCBUName = tmp.getCuenta().getImagenCBU();
	                            logger.info("[REI " + r.getId_reclamo() + "] Reutilizo file CBU existente -> " + imagenCBUName);
	                        }
	                    } catch (Exception ex) {
	                        logger.warn("[REI " + r.getId_reclamo() + "] No pude recuperar file CBU existente", ex);
	                    }	                    
	                }
	            } catch (Exception e) {
	                logger.warn("[REI " + r.getId_reclamo() + "] Error al subir constancia CBU", e);
	            }
	        }

	        // si no se subio constancia, intent usar la que ya existe
	        if (tengoCBU && imagenCBUName == null && cuentas != null) {
	        ...
	        if (tengoCBU) {
	            try {
	                ...
	                cuenta.setImagenCBU(imagenCBUName);
	                ...
	                //Datos del titular (inte = 0) o apoderado
	                if (!esApoderado) {
	                    try {
	                        Afiliado titular = EditarAfiliadoServiceUtil.getAfiliadoDadoBaja(
	                            r.getAfiliado() != null ? r.getAfiliado().getCuil_titular() : r.getCuilTitularCuenta(), 0);// siempre el titular del grupo familiar

	                        if (titular != null) {
	                            cuenta.setApellido(titular.getApellido());
	                            cuenta.setNombre(titular.getNombre());
	                            cuenta.setEmail(titular.getEmail() != null ? titular.getEmail() : "");
	                        } else {
	                            logger.warn("[REI " + r.getId_reclamo() + "] No se encontró titular inte=0 para cargar datos de cuenta");
	                            cuenta.setApellido(""); cuenta.setNombre(""); cuenta.setEmail("");
	                        }

	                    } catch (Exception ex) {
	                        logger.error("[REI " + r.getId_reclamo() + "] Error al obtener titular para cuenta bancaria", ex);
	                        cuenta.setApellido(""); cuenta.setNombre(""); cuenta.setEmail("");
	                    }

	                } else {
	                    // si es apoderado, se limpia el titular y se usa la nota autorizada
	                    cuenta.setApellido("");
	                    cuenta.setNombre("");
	                    cuenta.setEmail("");
	                }
	                ...
	                if (cuenta.getCuilGrupoFamiliar() == null || cuenta.getCuilGrupoFamiliar().trim().isEmpty()) {
	                    logger.warn("[REI " + r.getId_reclamo() + "] altaModiCuenta cuil_titular vacío");
	                } else if (cuenta.getCuil() == null || cuenta.getCuil().trim().isEmpty()) {
	                    logger.warn("[REI " + r.getId_reclamo() + "] altaModiCuenta cuil_cbu vacío");
	                } else {
	                    r.setCuenta(cuenta);
