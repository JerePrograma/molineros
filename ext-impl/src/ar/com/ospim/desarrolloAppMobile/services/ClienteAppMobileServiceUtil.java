...
public class ClienteAppMobileServiceUtil {
    ...
    public static Integer getIdReintegroAppPorCuilInte(String cuilTitular, int inte) {
        Connection conn = null;
        CallableStatement cs = null;

        try {
            conn = ConnectionHelper.getConnection();
            String sql = "{? = call autorizaciones.obtener_id_reintegro_app_por_cuil_inte(?, ?)}";
            cs = conn.prepareCall(sql);
            cs.registerOutParameter(1, Types.INTEGER);
            cs.setString(2, cuilTitular);
            cs.setInt(3, inte);
            cs.execute();
            ...
            _log.error("Error obteniendo id_reintegro_app para CUIL=" + cuilTitular + " inte=" + inte, e);
            return null;
