...
public class ImportarVademecumAdmifarmAction extends PortletAction {
    ...
    private static final Log log =
            LogFactoryUtil.getLog(ImportarVademecumAdmifarmAction.class);

    public ActionForward render(ActionMapping mapping, ActionForm form,
            PortletConfig config, RenderRequest request, RenderResponse response)
            throws Exception {
        VademecumAdmifarmHelper.validarPermiso(PortalUtil.getUser(request));
    ...
    public void processAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            String operacion = ParamUtil.getString(request, "operacion");

            if ("descargar".equals(operacion)) {
                descargar(request, response);
                return;
            }
            if (operacion.length() > 0 && !"importar".equals(operacion)) {
                throw new IllegalArgumentException("La operacion de Vademecum no es valida.");
            }
            importar(request, usuario);
            ...
            SessionErrors.add(request, "vademecum-error",
                    "No se pudo completar la operacion de Vademecum. "
                            + "Verifique el estado antes de reintentar.");
        }
    ...
    private void importar(ActionRequest request, User usuario) throws Exception {
        if (!"POST".equalsIgnoreCase(
                PortalUtil.getHttpServletRequest(request).getMethod())) {
            throw new IllegalArgumentException(
                    "La importacion debe enviarse desde el formulario.");
        }

        UploadPortletRequest upload = PortalUtil.getUploadPortletRequest(request);
        String tipo = ParamUtil.getString(upload, "tipo");
        VademecumAdmifarmHelper.consumirToken(request.getPortletSession(),
                usuario.getUserId(), tipo, ParamUtil.getString(upload, "tokenVademecum"));

        List<Registro> registros = VademecumAdmifarmHelper.leerArchivo(
                upload.getFile("archivo"), upload.getFileName("archivo"), tipo);
        ImportacionVademecumAdmifarm importacion =
                VademecumAdmifarmServiceUtil.importar(tipo, registros);
    ...
    private void descargar(ActionRequest request, ActionResponse response)
        ...
        Timestamp fecha;
        try {
            fecha = Timestamp.valueOf(ParamUtil.getString(request, "fecha"));
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("La fecha de importacion no es valida.");
        }

        ImportacionVademecumAdmifarm importacion =
                VademecumAdmifarmServiceUtil.getImportacion(tipo, fecha);
        ...
        String nombre = "vademecum_" + tipo + "_"
                + new SimpleDateFormat("yyyyMMdd_HHmmss").format(fecha) + ".xls";
        ServletResponseUtil.sendFile(httpResponse, nombre,
                new ByteArrayInputStream(contenido), contenido.length,
                "application/vnd.ms-excel");
        setForward(request, ActionConstants.COMMON_NULL);
    }

    protected boolean isCheckMethodOnProcessAction() {
        // El enlace de descarga se conserva. La importacion exige POST en importar().
        return false;
    }
}
