...
public class ReclamosPrestacionesServiceUtil {
    ...
    public static List<ItemReclamoPrestacionalesTotal>
            buscarReclamosPrestacionalTotales(
                    BusquedaReclamosPrestacionalesFiltro filtro)
                    throws Exception {
        return getInstance().buscarReclamosPrestacionalesTotales(filtro);
    }

    public static int insertar(ReclamoPrestacional reclamo, User user)
            throws Exception {
        int idReclamo = getInstance().insertar(reclamo, user);
    ...
    public static void update(ReclamoPrestacional reclamo, User user)
            throws Exception {
        if (getInstance().actualizar(reclamo, user)
                && reclamo.getEstado() == 3) {
            grabarContactoCRM(reclamo, user);
            Afiliado afil = reclamo.getAfiliado();
    ...
    public static void setDatosOpReclamoPrestacional(
            ReclamoPrestacional reclamo) throws Exception {
        getInstance().setDatosOpReclamoPrestacional(reclamo);
    }

    public static void altaModiCuenta(
            ReclamoPrestacional reclamo,
            User user) throws Exception {
        getInstance().altaModiCuenta(reclamo, user);
    }

    public static void altaModiCuenta2(
            ReclamoPrestacional reclamo,
            User user,
            Integer idCuenta) throws Exception {
        getInstance().altaModiCuenta2(reclamo, user, idCuenta);
    }

    public static ReclamoPrestacionalCuenta getReclamoPrestacionalCuenta(
            int idReclamo) throws Exception {
        return getInstance().getReclamoPrestacionalCuenta(idReclamo);
    }

    public static ReclamoPrestacional getReclamoPrestacional(int id)
            throws Exception {
        return getInstance().getReclamoPrestacional(id);
    }

    public static List<PrestacionesReclamo> getPrestacionesAsociadas(
            int casoasociado) throws Exception {
        return getInstance().retornaPrestacionesAsociadasxCasoVinculado(
                casoasociado
        );
    }

    /**
     * Conserva el identificador de la aplicación móvil antes de ejecutar la
     * baja local. La versión anterior intentaba recuperar el reclamo después
     * de borrarlo, cuando el identificador externo ya podía no existir.
     *
     * La actualización externa continúa siendo best-effort porque la API
     * móvil no participa de la transacción PostgreSQL. Los errores quedan
     * identificados con RECLAMO_APP_SYNC_PENDING para reconciliación.
     */
    public static void borrar(int id, User user)
            throws ImposibleBorrarReclamoPrestacionalException,
            SystemException {
        ...
        try {
            ...
            if (snapshot != null
                    && snapshot.getIdReintegroApp() != null
                    && snapshot.getIdReintegroApp().intValue() > 0) {
                idReintegroApp = snapshot.getIdReintegroApp();
            }
        } catch (Exception e) {
            _log.error(
                    "No se pudo obtener el snapshot previo al borrado del "
                            + "Reclamo Prestacional " + id
                            + ". Se cancela la baja para no perder el "
        ...
        try {
            ...
            if (token == null) {
                _log.error(
                        "RECLAMO_APP_SYNC_PENDING reclamo=" + id
            ...
            _log.info(
                    "Solicitud de anulación enviada a AppMobile. reclamo="
                            + id + " reintegroApp=" + idReintegroApp
            );
        } catch (Exception e) {
            _log.error(
                    "RECLAMO_APP_SYNC_PENDING reclamo=" + id
                            + " reintegroApp=" + idReintegroApp
                            + " estado=AN motivo=EXCEPCION",
                    e
            );
        }
    }
     ...
     */
    public static void grabarContactoCRM(
            ReclamoPrestacional reclamo,
        ...
        try {
            List<UserGroup> userGroups = UserUtil.getUserGroups(
                    user.getUserId()
            );
            ...
            if (userGroups != null && !userGroups.isEmpty()) {
                ...
                userGroupId = String.valueOf(
                        userGroups.get(0).getUserGroupId()
                );
            } else {
                _log.warn(
                        "El usuario no tiene UserGroups asociados. Usuario: "
                                + user.getScreenName()
                );
            }

            contactoCrm.setAfiliado(
                    new Afiliado(
                            reclamo.getCuit_titular(),
                            reclamo.getInte()
                    )
            );
            contactoCrm.setAltaSector(altaSector);
            contactoCrm.setTipo(
                    new TipoContacto(6, "CORREO SALIENTE")
            );
            contactoCrm.setCategoria(
                    new CategoriaContacto(2, "RECLAMO")
            );
            contactoCrm.setMotivo(
                    new MotivoContacto(5, "PRESTACIONES MÉDICAS")
            );
            contactoCrm.setDescripcion(
                    "RECLAMO PRESTACIONAL NRO::"
                            + String.valueOf(reclamo.getId_reclamo())
            );
            contactoCrm.setIdCrmRelacionado(0);
            contactoCrm.setComentarioCierre(
                    "Cierre automático por carga de reclamo prestacional"
            );
    ...
    private static void enviarMailsCierreReclamoPrestacional(
            ReclamoPrestacional reclamo,
            User user) {

        String subjectautorizados = " Reclamo Prestacional Nro:"
                + String.valueOf(reclamo.getId_reclamo())
                + ", Afiliado:" + reclamo.getAfiliado().getApeNombre()
                + "(" + reclamo.getEstadoResolucionAutorizadaString()
                + ").";
        String subjectSeccionalCAB = " Reclamo Prestacional Nro:"
                + String.valueOf(reclamo.getId_reclamo())
                + ", Afiliado:" + reclamo.getAfiliado().getApeNombre();
        String bodyAutorizados = "Se ha cerrado el reclamo Nro "
                + String.valueOf(reclamo.getId_reclamo())
                + " con resolucion "
                + reclamo.getEstadoResolucionAutorizadaString()
                + ".\n\n\n";
        String bodySeccionalCAB = "Prestación evaluada por Auditoría "
                + "Médica. Contáctese al 0810-345-0208.";
        ...
        try {
            ...
            contactose = SeccionalServiceUtil
                    .buscarContactosSeccionalEmail(id);
            for (ContactoElectronico contac : contactose) {
                if (contac.getContacto().indexOf("uoma") > 1) {
                    emails.add(contac.getContacto());
                    emailsSeccionalCAB.add(contac.getContacto());
                }
            }

            if (reclamo.getEstadoResolucionAutorizada().equals(
                    ESTADOSEVALUACIONRECLAMO.RECHAZADA
            )) {
                List<String> destinatarios = ProcesosCorreoServiceUtil
                        .getListaCorreoDestinatariosInformadosPorProceso(
                                ProcesosCorreoServiceUtil
                                        .CIERRE_RECLAMO_PRESTACIONAL_RECHAZO
                        );
            ...
            List<String> destinatariosExtra = ProcesosCorreoServiceUtil
                    .getListaCorreoDestinatariosInformadosPorProceso(
                            ProcesosCorreoServiceUtil
                                    .CIERRE_RECLAMO_PRESTACIONAL
                    );
        ...
        if (reclamo.getEstado() == 3
                && reclamo.getEstadoResolucionAutorizada().equals(
                        ESTADOSEVALUACIONRECLAMO.AUTORIZADA
                )) {
            bodyAutorizados = bodyAutorizados + "Afiliado  "
                    + reclamo.getAfiliado().getApellidoNombre() + "\n"
                    + "DNI:  " + reclamo.getAfiliado().getDocu_numero()
                    + "\n"
                    + "CUIL TITULAR: "
                    + reclamo.getAfiliado().getCuil_titular() + "\n"
                    + "CUIL: " + reclamo.getAfiliado().getCuil() + "\n"
                    + "ID Reclamo: " + reclamo.getId_reclamo() + "\n"
                    + "\n\n Prestación/es\n";
            for (PrestacionesReclamo pres : reclamo.getPrestaciones()) {
                if (pres.getEstadoRechazoAprobado() == 2) {
                    bodyAutorizados = bodyAutorizados
                            + pres.getDescripcion() + "\n";
                }
            }
            bodyAutorizados = bodyAutorizados + "\n\n";
            bodyAutorizados = bodyAutorizados + "Resolución:"
                    + reclamo.getEstadoResolucionAutorizadaString();
        }
    ...
    public static int insertarDesdePreautorizacion(
            ReclamoPrestacional reclamo,
    ...
    public static void updateDesdePreautorizacion(
            ReclamoPrestacional reclamo,
            String screenName) throws Exception {
    ...
    public static Integer getLoteVigenteReclamoPrestacional()
            throws Exception {
        return getInstance().getLoteVigenteReclamoPrestacional();
    }
    ...
    public static void reabrirReclamo(
            int nroReclamo,
            String screenName) throws Exception {
        getInstance().reabrirReclamo(nroReclamo, screenName);
    }

    public static String validarExisteComprobante(
            PrestacionesReclamo reclamo) {
        return getInstance().validarExisteComprobante(reclamo);
    }

    public static void grabarFechaEnvioSeccional(
            ReclamoPrestacional reclamoPrestacional) {
        getInstance().grabarFechaEnvioSeccional(reclamoPrestacional);
    }

    public static List<ReportePreCargaReclamo>
            reclamosPrestacionalPreCarga() throws Exception {
        return getInstance().reclamosPrestacionalPreCarga();
    }

    public static List<ReporteIntegracionReclamo>
            reclamosPrestaEstadisticaIntegracion() throws Exception {
        return getInstance().reclamosPrestaEstadisticaIntegracion();
    }

    public static List<EstadisticaGastoMedico> getEstadisticaGastoMedico(
            Date fini,
            Date ffin) throws Exception {
        return getInstance().getEstadisticaGastoMedico(fini, ffin);
    }

    public static List<EstadisticaGastoMedicoDetalle>
            getEstadisticaGastoMedicoDetalle(
                    Date fini,
                    Date ffin) throws Exception {
        return getInstance().getEstadisticaGastoMedicoDetalle(fini, ffin);
    }

    public static List<AfiCuentaBancaria> traerCuentaBancariaAsociadas(
            String cuilTitular)
            throws SystemException, NumberFormatException, ParseException {
        return getInstance().traerCuentaBancariaAsociadas(cuilTitular);
    }

    public static AfiCuentaBancaria traerCuentaBancariaAsociadasPorId(
            int idCuenta)
            throws SystemException, NumberFormatException, ParseException {
        return getInstance().traerCuentaBancariaAsociadasPorId(idCuenta);
    }

    public static void deleteImagenCuenta(
            String name,
            String screenName) throws SystemException {
        getInstance().deleteImagenCuenta(name, screenName);
    }

    public static void updateNombreImagen(
            ReclamoPrestacional reclamoPrestacional,
            String name,
            String description,
            String screenName) throws SystemException {
        getInstance().updateNombreImagen(
                reclamoPrestacional,
                name,
                description,
                screenName
    ...
    public static String traerNumerosReclamos(
            int op,
            int idFormaPago) throws SystemException {
        return getInstance().traerNumerosReclamos(op, idFormaPago);
    }

    public static List<ReclamoPrestacionalCuenta> getCuentasPorCuil(
            String cuil) throws Exception {
