		...
		if (cmd.equals("avisovencimientocud")) {
			...
			if(ret>0) {
				SessionMessages.add(renderRequest, "request_processed",
						"Se encontraron "+ ret +" afiliados a los cuales se les envi� el aviso");
			}else {
...
*/
	...
	private StringBuilder getPrestacionHechaError() {
		StringBuilder error = new StringBuilder();
		error
				.append("La prestaci�n ya fue realizada al afiliado y no puede hacerse dos veces");
		return error;
	}
	...
	private StringBuilder getFechasError() {
		StringBuilder error = new StringBuilder();
		error
				.append("La fecha de la prestaci�n no puede ser posterior a la fecha de baja del afiliado");
		return error;
	}
