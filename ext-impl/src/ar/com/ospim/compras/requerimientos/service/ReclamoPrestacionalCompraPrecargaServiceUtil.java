...
/**
 * Precarga un Reclamo Prestacional desde un requerimiento de compra COTIZADO.
 *
 * Los identificadores de artículos de Compras no se reutilizan como IDs del
 * nomenclador médico. Se transportan como código visible ART-{id} y se deja el
 * ID de prestación en cero para que el usuario confirme el nomenclador.
 */
public final class ReclamoPrestacionalCompraPrecargaServiceUtil {
    ...
    public static void precargar(
        ...
        /*
         * No pisa una edición ya iniciada ni los datos que vuelven de una
         * validación fallida del propio RP.
         */
        ...
        if (prestaciones.isEmpty()) {
            throw new Exception(
                    "El requerimiento COTIZADO no contiene ítems para precargar."
            );
        }
    ...
    public static ReclamoPrestacional crearReclamo(
        ...
        int integrante = requerimiento.getAfiliadoInt() != null
        ...
        ReclamoPrestacional reclamo = new ReclamoPrestacional(
                ...
                integrante,
                fechaAlta,
                mapearSector(requerimiento.getSectorDescripcion()),
                null
        );
        ...
        reclamo.setRecuperable(
                requerimiento.isRecupero() || requerimiento.isSurge()
        );
    ...
    private static PrestacionesReclamo crearPrestacion(
        ...
        BigDecimal importeUnitario = normalizarImporte(
                detalle.getPrecioUnitarioEstimado()
        );
        BigDecimal total = normalizarImporte(
        ...
        BigDecimal cargoOspim = total
                .multiply(
                        BigDecimal.valueOf(
                                porcentaje(requerimiento.getCargoOspim())
                        )
                )
                .divide(
        ...
        /*
         * El remanente va a la tercerizadora. Así se evita que dos redondeos
         * independientes hagan diferir la suma del total cotizado.
         */
        ...
        prestacion.setRecuperable(
                Integer.valueOf(resolverRecuperable(requerimiento))
        );
        prestacion.setRecuperableSur(
                Boolean.valueOf(requerimiento.isSurge())
        );
        ...
        /*
         * Una cotización no es una factura. OTR evita nulls en el JSP legacy,
         * pero número, fecha y sucursales quedan vacíos hasta ser confirmados.
         */
        ...
        prestacion.setObservaciones(
                construirObservacion(requerimiento, detalle)
        );
    ...
    private static void validarRequerimiento(
        ...
        if (!requerimiento.tieneAfiliadoInformado()) {
            throw new Exception(
                    "El requerimiento COTIZADO no tiene un afiliado válido."
            );
        }

        String cuilRequerimiento = normalizarCuit(
                requerimiento.getAfiliadoCuilTitular()
        );
        String cuilContexto = normalizarCuit(
                contexto.getAfiliadoCuilTitular()
        );

        int integranteContexto = contexto.getAfiliadoInt() != null
                ? contexto.getAfiliadoInt().intValue()
                : -1;

        if (!cuilRequerimiento.equals(cuilContexto)
                || requerimiento.getAfiliadoInt().intValue()
                        != integranteContexto) {
        ...
        if (!detalle.estaCompletoParaCotizacion()
            ...
            throw new Exception(
                    "El requerimiento figura COTIZADO, pero contiene un ítem "
                            + "sin cantidad, precio o prestador adjudicado."
            );
        }
    }
    ...
    private static void validarPorcentajes(
        ...
        int suma = porcentaje(requerimiento.getCargoOspim())
                + porcentaje(requerimiento.getCargoTercerizadora());

        if (suma != 100) {
            throw new Exception(
                    "Los porcentajes de cargo del requerimiento no suman 100."
            );
        }
    }
    ...
    private static String construirObservacion(
        ...
        observacion.append(
                ". Confirmar nomenclador, fecha, comprobante y reconocido SSS."
        );
        ...
        return value.length() <= MAX_OBSERVACION
                ? value
                : value.substring(0, MAX_OBSERVACION);
    }
    ...
    private static BigDecimal normalizarImporte(BigDecimal value)
            throws Exception {
