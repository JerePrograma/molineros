...
public class ProcesaArchivosAdmifarmOspimGeneralServiceImpl {

    private static Log logger = LogFactoryUtil.getLog(ProcesaArchivosAdmifarmOspimGeneralServiceImpl.class);

    //inserta cabecera
    public int insertarCabeceraAdmifarmOspimGeneral(Connection con, String usuario, Date fechaArchivo) throws SQLException {
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            String sql = "INSERT INTO farmacia.farmacia_ospim_general_cabecera (" +
    ...
    //insertar detalle
    public void insertarDetalleAdmifarmOspimGeneral(Connection con, String nombreTabla, String[] cols, Date fechaArchivo, int idCabecera) throws SQLException {
        PreparedStatement stmt = null;
        try {
        	String sql =
    ...
    //actualizar cabecera
    public void actualizarCabeceraAdmifarmOspimGeneral(Connection con, int idCabecera, int totalRecords, double totalPvp, double totalEntidad, double totalOspim, double totalUoma, double totalAmtima) throws SQLException {
        PreparedStatement stmt = null;
        try {
            String sql = "UPDATE farmacia.farmacia_ospim_general_cabecera SET " +
    ...
    public void correProcesosAdmifarmOspimGeneralArchivos(Connection con, String nombreTabla, String fechaPeriodoArchivo) 
    		throws SQLException { 
    	CallableStatement stmt = null; 
    	try { 
    		String sql = "{call proceso_archivo_admifarm_ospim_general_updates(?,?)}"; 
    		stmt = con.prepareCall(sql); 
    		stmt.setString(1, nombreTabla); 
    		stmt.setString(2, fechaPeriodoArchivo); 
    		stmt.executeUpdate(); 
    	} catch (SQLException e) { 
    		logger.error("ERROR EN EL PROCESO DE LA TABLA DE ADMIFARM OSPIM GENERAL", e); 
    		throw e; 
    	} finally { 
    		ConnectionHelper.cerrar(stmt); 
    	} 
