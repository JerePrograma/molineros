alter table comprobante_amtima add anulado_fecha timestamp without time zone;
alter table comprobante_amtima add anulado_usr character varying(15);
CREATE TABLE comprobante_amtima
(
  ...
  CONSTRAINT pk_comprobante_amtima PRIMARY KEY (id_punto_venta, compro_tipo, compro_letra, compro_sucu, compro_nro, cuit),
  CONSTRAINT fk_compro_amtima_afi FOREIGN KEY (cuil_titular, inte)
      REFERENCES afiliado (cuil_titular, inte) MATCH FULL
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT fk_amtima_empre_acreedor FOREIGN KEY (cuit_acreedor, sucu_acreedor)
