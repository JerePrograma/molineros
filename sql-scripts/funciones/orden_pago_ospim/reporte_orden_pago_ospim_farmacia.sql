CREATE OR REPLACE FUNCTION reporte_orden_pago_ospim_farmacia(p_id integer, p_id_fin integer)
  RETURNS SETOF return_reporte_orden_pago_amtima_farmacia AS
$BODY$
DECLARE _op  RECORD;
...
for _op in select id_orden_pago from orden_pago_ospim where id_orden_pago >= $1 and id_orden_pago <= $2
loop
	_comps = '';
	_comps_importe = 0;
	FOR _record IN
	select (c.compro_tipo  || ' ' || c.id_punto_venta ||'-'||  c.compro_nro  || '  Importe: $' || (case when c.debito_para_egreso then -1*c.total else c.total end) )
	 as inf, (case when c.compro_tipo<>'NDF' and c.debito_para_egreso then -1*c.total else case when c.compro_tipo<>'NDF' then c.total else 0 end end) as comps_importe from comprobante c
	inner join comprobante_orden_pago_ospim copo
	on copo.id_orden_pago_ospim = _op.id_orden_pago
	...
	--and c.compro_tipo<>'NDF'
	union all
	select ('ANT' || ' ' || opop.compro_nro_antic || ' ' ||opop.compro_letra_antic )   ||   '  Importe: $-' || cast(c.total as character varying)  as inf,
	(case when c.compro_tipo<>'NDF' and c.debito_para_egreso then -1*c.total else case when c.compro_tipo<>'NDF' then c.total else 0 end end) as comps_importe 
...
loop
	...
	FOR _record2 IN
	select (c.compro_tipo  || ' ' || c.id_punto_venta ||'-'||  c.compro_nro  || '  Importe: $' || (case when c.debito_para_egreso then -1*c.total else c.total end) )
	 as inf, (case when c.debito_para_egreso then -1*c.total else c.total end) as comps2_importe from comprobante c
	inner join comprobante_orden_pago_ospim copo
	on copo.id_orden_pago_ospim = _op.id_orden_pago
	and copo.id_punto_venta = c.id_punto_venta
	and copo.compro_tipo = c.compro_tipo
...
loop
	_conceptos = '';
	for r_conceptos in 
	select (conc.descripcion_original || '  $' ||  sum(case when c.debito_para_egreso then -1*cc.importe else cc.importe end) ) as inf  from comprobante_orden_pago_ospim copo
	inner join comprobante c
	on copo.id_orden_pago_ospim = _op.id_orden_pago
	and copo.id_punto_venta = c.id_punto_venta
	and copo.compro_tipo = c.compro_tipo
	...
	and c.cuit = cc.cuit
	inner join concepto_maestro conc
	on cc.concepto_id = conc.id
	group by conc.descripcion_original
...
loop
	_conceptos = '';
	for r_conceptos in 
	select (conc.descripcion_original || '  $' || sum(-1*cc.importe)) as inf  
