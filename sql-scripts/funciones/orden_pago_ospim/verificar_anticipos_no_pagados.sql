CREATE OR REPLACE FUNCTION verificar_anticipos_no_pagados(p_id_op_ospim integer) 
RETURNS integer
    LANGUAGE plpgsql
    AS $BODY$
  ...
  begin
	  ...
	  resultDom = 1 from orden_pago_ospim_pagos opop 
			where opop.id_orden_pago = p_id_op_ospim
			and compro_tipo_antic = 'ANT'
			and not exists (select 1 from orden_pago_ospim opo, comprobante_orden_pago_ospim copo
