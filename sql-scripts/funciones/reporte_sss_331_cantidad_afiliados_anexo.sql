CREATE OR REPLACE FUNCTION reporte_sss_331_cantidad_afiliados_anexo(fecha_ini_p date, fecha_fin_p date)
...
select per_codigo as periodo,--periodo
       ...
       pr.detalle as provincia, --PROVINCIA,
...
--AL 40%
insert into cant_afi(periodo,id_ospim,inte,porc, provincia, cantidad)
select per_codigo as periodo,--periodo
       lfa.id_ospim,
       lfa.inte,
       lpad(cast(cast('40' as double precision) as varchar),3,'0') as porc, --cobertura
       pr.detalle as provincia, --PROVINCIA,
       lpad(cast(count(*) as varchar),11,'0') as cantidad
from amtima_federico lfa--liquidacion_farmacia_amtima lfa--
left outer join afiliado a
...
select  periodo,--periodo	
	cast('' as text),
        cast(provincia as text), --PROVINCIA,        
        cast(count(*) as text)
from cant_afi        
group by periodo, provincia
order by provincia;
