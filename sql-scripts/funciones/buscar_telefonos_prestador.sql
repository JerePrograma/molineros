CREATE OR REPLACE FUNCTION buscar_telefonos_prestador(id_prestador integer) 
RETURNS TABLE(id_telefono integer,
 ...
 baja_usr character varying)
	...
	tel.baja_fecha,
	tel.baja_usr
	from  prestad_telefono etel
	inner join telefono tel
	on etel.id_telefono = tel.id_telefono
	where etel.vigen_desde <= LOCALTIMESTAMP 
	and tel.baja_fecha is  null
	and etel.id_prestador = $1	
	
	order by tel.id_telefono asc
	$BODY$;


ALTER FUNCTION public.buscar_telefonos_prestador(id_prestador integer) OWNER TO postgres;
