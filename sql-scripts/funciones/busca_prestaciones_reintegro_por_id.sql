...
rp.cuit_entidad,
rp.sucursal_entidad,
rp.fecha_comprobante,
rp.importe_comprobante,
...
left outer join empresa e on
rp.cuit_entidad = e.cuit and
rp.sucursal_entidad = e.sucursal

inner join
plan_prestacion pp
on rp.id_plan = pp.id_plan and
rp.id_prestacion = pp.id_prestacion
...
inner join
     afiliado a
on r.cuil_titular = a.cuil_titular and
r.inte = a.inte
...
af.inte = 0
and af.modi_fecha = (select max(a1.modi_fecha) from afi_domicilio a1 where a1.cuil_titular = af.cuil_titular and a1.inte = af.inte)     

inner join     
     seccional s         
on a.id_seccional = s.id_seccional
     
  where  r.id_reintegro = $1 
