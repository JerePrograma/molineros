CREATE OR REPLACE FUNCTION buscar_penultimo_plan_aportes(cuil_titular_p character varying)
  RETURNS SETOF afi_plan_aporte_omint AS
$BODY$
declare id_plan_serial_aux bigint;
BEGIN

id_plan_serial_aux = max(id) from afi_plan ap
			     where ap.cuil_titular = cuil_titular_p
			     and ap.baja_fecha is null
			     and ap.id < (select max(ap_.id) 
					  from afi_plan ap_ 
					  where ap_.cuil_titular = ap.cuil_titular
					  and ap_.baja_fecha is null);
