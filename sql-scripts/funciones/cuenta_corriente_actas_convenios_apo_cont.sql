...
create type  return_cuenta_corriente_actas_convenios as (
...
DROP FUNCTION cuenta_corriente_actas_convenios_apo_cont(IN p_cuit character varying, IN p_sucu character varying, IN p_fecha_ini date, IN p_fecha_fin date,  IN p_fecha_ini_pan date);

CREATE OR REPLACE FUNCTION cuenta_corriente_actas_convenios_apo_cont(IN p_cuit character varying, IN p_sucu character varying, IN p_fecha_ini date, IN p_fecha_fin date)
RETURNS SETOF return_cuenta_corriente_actas_convenios
LANGUAGE plpgsql
AS $BODY$
begin
	...
	a.cuit, a.sucursal,  ca.saldo, 'D'
	 ...
	 and cast(a.baja_fecha as date) > cast('18000101' as date) and cast(a.baja_fecha as date) < ($4 + interval '1 day')	 
	 union all 
	 (select null,null, cuit, sucu, null,null from actas_convenios_saldo_inicial group by cuit, sucu having ($1 is null or ($1 is not null and cuit = $1)))
	 union all  
	 select $3,'AFIP - SALDO INICIAL DESDE EL 01/2008', cuit_contribuyente, '000', -sum(calculado)+sum(pagado), 
	        case when sum(calculado)-sum(pagado)>0 then 'C' else 'D' end
	 from reporte_aportes_contrib_empresa_periodo_s($1, '20080101', $3, null) r
	 where not exists (select 1
			  from acta a , acta_periodos ap
			  where a.cuit=$1
			  and ap.periodo=r.periodo
	 ...
	 and not exists (select 1
			 ...
			 and a.cierre_fecha is not null
			 and a.acta_cerrada=true)		  		  			  
	 group by cuit_contribuyente, razon 
	 union all
	 select periodo,'AFIP - APORTES Y CONTRIBUCIONES', cuit_contribuyente, '000', sum(calculado), 'C'
	 from reporte_aportes_contrib_empresa_periodo_s($1, $3, $4, null) r
	 where calculado>0
	 and not exists (select 1
			  from acta a , acta_periodos ap
			  where a.cuit=$1
	 ...
	 and not exists (select 1
			 ...
			 and a.cierre_fecha is not null
			 and a.acta_cerrada=true)				  
	 group by cuit_contribuyente, razon, periodo
	 union all 
	 select periodo,'AFIP - APORTES Y CONTRIBUCIONES', cuit_contribuyente, '000', sum(pagado), 'D'
	 from reporte_aportes_contrib_empresa_periodo_s($1, $3, $4, null) r	 
	 where pagado>0
	 and not exists (select 1
			  from acta a , acta_periodos ap
