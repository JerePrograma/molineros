CREATE OR REPLACE FUNCTION verificar_op_cheques_canjeados(nro_op_ospim integer) 
RETURNS TABLE(ch__nro_cheque numeric,
 ...
 ba__descripcion character varying)
	...
	select 	
	 ...
	 inner join canje_cheques_propios_viejos cccpv
	 on ccp.id = cccpv.canje_id
	 inner join cheque c 
	 on cccpv.nro_cheque = c.nro_cheque
	 and cccpv.id_banco = c.id_banco
	 inner join cheque_estado ce
	 on c.id_estado = ce.id
	 inner join banco b
	 on c.id_banco = b.id_banco 
	where ccp.id_orden_pago_ospim = $1
	and ccp.baja_fecha is null;
$BODY$;