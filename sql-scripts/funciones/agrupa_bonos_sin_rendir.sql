CREATE OR REPLACE FUNCTION agrupa_bonos_sin_rendir(tipo_bono_v integer, seccional_v integer, fecha_desde_v date, fecha_hasta_v date, nro_bono_desde integer, nro_bono_hasta integer)
  RETURNS integer AS
$BODY$
...
FOR _record_bonos IN select tipo_bono, nro_bono, id_seccional, id_envio, fecha_envio
		     from bonos_seccional b
	             where fecha_rendido is null 
	...
	if min is null or primero=false or (tipo_bono_ant=0 or tipo_bono_ant<>_record_bonos.tipo_bono) or (seccional_ant=0 or seccional_ant<>_record_bonos.id_seccional) 
		...
		select cast(tipo_bono_ant as varchar)||'-'||t.descripcion,s.id_seccional,s.descripcion,fecha_envio_ant, min, max,null,id_envio_ant,max+1-min 
...
--cuando llega al ultimo registro se olvidaba de actualizar maximo, y ahi salia la cuenta negativa.
if (max=0 or max is null or max<min) then max=min; end if;
insert into aux_bonos (tipo_bono, id_seccional, descripcion, fecha_envio , min_bono, max_bono, fecha_rendido, id_envio, total)  
select cast(tipo_bono_ant as varchar)||'-'||t.descripcion as aaa,s.id_seccional,s.descripcion,fecha_envio_ant,min,max,null,id_envio_ant,max+1-min	        
	        from tipos_bono t, seccional s
	        where t.tipo_bono=tipo_bono_ant
	        and s.id_seccional=seccional_ant;
	        nro_bono_ant=_record_bonos.nro_bono;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION agrupa_bonos_sin_rendir() OWNER TO postgres;
