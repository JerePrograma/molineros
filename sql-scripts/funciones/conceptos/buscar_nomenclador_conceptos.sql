drop FUNCTION buscar_nomenclador_conceptos(desde date, hasta date) ;

--TRAE TODOS LOS NOMENCLADOR_CONCEPTO QUE SON VALIDOS EN ALGUN MOMENTO DE DESDE-HASTA
CREATE OR REPLACE FUNCTION buscar_nomenclador_conceptos(desde date, hasta date) 
RETURNS TABLE(
...
--SOLAPAMIENTO CON EL FINAL DEL PERIODO ($2)
select n.codigo,  
	n.id_prestacion,
	n.descripcion,
...
left outer join
	...
	and tipo_id = 1
	and  ((cast(nc.valido_desde as date) <= cast($2 as date)  and cast(nc.valido_hasta as date)>=cast($2 as date)) 
    or (cast(nc.valido_desde as date) <= cast($1 as date)  and cast(nc.valido_hasta as date)>=cast($1 as date))
    or (cast(nc.valido_desde as date) >= cast($1 as date)  and cast(nc.valido_hasta as date)<=cast($2 as date)))
...
and ha.valido_Desde = hi.valido_desde
and ha.valido_hasta = hi.valido_hasta
left outer join 
	  (select nc.id, nc.id_prestacion,c.descripcion, c.id_concepto_maestro as concepto_id, nc.valido_desde, nc.valido_hasta from nomenclador_conceptos  nc,
	 conceptos c
	 where  nc.concepto_id = c.id_concepto_maestro
...
and ha.valido_Desde = ga.valido_desde
and ha.valido_hasta = ga.valido_hasta
left outer join
	(select nc.id, nc.id_prestacion,c.descripcion, c.id_concepto_maestro as concepto_id, nc.valido_desde, nc.valido_hasta from nomenclador_conceptos  nc,
	conceptos c
	where  nc.concepto_id = c.id_concepto_maestro
...
and ha.valido_Desde = gi.valido_desde
and ha.valido_hasta = gi.valido_hasta
where n.baja_fecha is null
order by codigo;
