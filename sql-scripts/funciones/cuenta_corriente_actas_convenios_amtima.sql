CREATE OR REPLACE FUNCTION cuenta_corriente_actas_convenios_amtima(p_cuit character varying, p_sucu character varying, p_fecha_ini date, p_fecha_fin date)
...
begin
	...
	from acta_no_os c left outer join (
		select * from actas_convenios_saldo_inicial_amtima asi
	...
	where entidad='A.M.T.I.M.A.'
	and ($1 is null or c.cuit = $1)
	...
	from actas_convenios_saldo_inicial_amtima  acsi
	where cast(fecha_inicio_ejercicio as date) <= cast($3 as date)
	...
	and fecha_inicio_ejercicio = (select max(fecha_inicio_ejercicio) 
							from actas_convenios_saldo_inicial_amtima  
							where cast(fecha_inicio_ejercicio as date) <= cast($3 as date)
							and cuit = acsi.cuit
							and sucu = acsi.sucu)
	...
	from recibo_conceptos_amtima rc, recibo_amtima r
	where rc.recibo_id = r.id
	and caja_concepto_id  in (select id from concepto_maestro_amtima  where descripcion_original = 'ANTICIPO ACTAS/CONVENIOS')
	...
	'D' as debito_credito
	 ...
	 inner join recibo_conceptos_amtima rc
	 on a.id = rc.acta_id
	  inner  join recibo_amtima r
	 on rc.recibo_id = r.id
	 left outer join recibo_conceptos_pagos_amtima   rcp
	 on rcp.recibo_concepto_id = rc.id
	 left outer join recibo_ingresos_amtima ri
	 on rcp.recibo_ingreso_id = ri.id
	 where a.entidad='A.M.T.I.M.A.'
	 and a.acta_cerrada = true
	 and ($1 is null or a.cuit = $1)
	 and ($2 is null or a.sucursal = $2)
	 and cast(r.fecha  as date) > cast('18000101' as date) and cast(r.fecha  as date) < ($4 + interval '1 day')
	 and ri.id_anticipo_recibo_concepto is null
	and r.id not in (select id from recibo_amtima r
		where baja_fecha is not null
		and exists (Select 1 from recibo_amtima where numero = r.numero and id <> r.id and alta_fecha > r.alta_fecha))
	...
	'C' as debito_credito
	 ...
	 from acta_no_os a
	 inner join acta_no_os_relacion ar
	...
	a.cuit, a.sucursal,  ca.importe, 'D'
	 from acta_no_os a
	 inner join convenio_actas_no_os ca
	 on a.id = ca.acta_id
	 and ca.baja_fecha is null
	 inner join convenio_no_os c2
	...
	a.cuit, a.sucursal,  ca.saldo, 'D'
	 from convenio_no_os a
	 inner join convenio_no_os_relacion ca
	 on a.id = ca.convenio_relacionado_id
	 and ca.baja_fecha is null
	 inner join convenio_no_os c2
	 on ca.convenio_id = c2.id
	 and c2.baja_fecha is null
	 inner join convenio_no_os_pagos ap
	 on ca.id = ap.convenio_relacion_id
	 and ap.baja_fecha is null
	 where c2.entidad='A.M.T.I.M.A.'
	 and ($1 is null or a.cuit = $1)
	 and ($2 is null or a.sucursal = $2)
	 and cast(a.fecha_inicio as date) > cast('18000101' as date) and cast(a.fecha_inicio as date) < ($4 + interval '1 day')
	 union all
	...
	a.cuit, a.sucursal,  ca.saldo, 'D'
	 ...
	 on ca.convenio_id = c2.id
	 and c2.baja_fecha is null
	 inner join convenio_pagos_no_os ap
	 on ca.id = ap.convenio_relacion_id
	 and ap.baja_fecha is null
	 where  a.entidad='A.M.T.I.M.A.'
	 and ($1 is null or a.cuit = $1)
