	CREATE OR REPLACE FUNCTION buscar_reintegros(IN reintegro integer, IN fecha_desde timestamp without time zone, IN fecha_hasta timestamp without time zone, IN pediodo_desde timestamp without time zone, IN periodo_hasta timestamp without time zone, IN seccional integer, IN prestacion character varying, IN entidad character varying, IN nro_afi integer, IN inte integer, IN cuil character varying, IN alta_usr character varying)
	  ...
	  rp__fecha_prestacion timestamp without time zone, rp__comprobante character varying
	  ) AS
	$BODY$
	select
	r.fecha,
	...
	$BODY$
	  ...
	  COST 100
	  ROWS 1000;
	ALTER FUNCTION buscar_reintegros(integer, timestamp without time zone, timestamp without time zone, timestamp without time zone, timestamp without time zone, integer, character varying, character varying, integer, integer, character varying, character varying) OWNER TO postgres;