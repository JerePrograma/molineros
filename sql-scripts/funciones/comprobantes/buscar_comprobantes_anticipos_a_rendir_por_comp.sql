DROP FUNCTION buscar_comprobantes_anticipos_a_rendir_por_comp
(IN p_nro character varying);
CREATE OR REPLACE FUNCTION buscar_comprobantes_anticipos_a_rendir_por_comp
(IN p_nro character varying)
  RETURNS TABLE(
...
where c.compro_tipo = 'ANT'
   ...
   and not exists (select 1 from orden_pago_ospim_pagos opop 
			inner join orden_pago_ospim opo_pago
