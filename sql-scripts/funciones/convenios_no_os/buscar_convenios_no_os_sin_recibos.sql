-- Function: buscar_convenios_no_os_sin_recibos(character varying, character varying)

-- DROP FUNCTION buscar_convenios_no_os_sin_recibos(character varying, character varying);

CREATE OR REPLACE FUNCTION buscar_convenios_no_os_sin_recibos(IN p_cuit character varying, IN p_entidad character varying)
...
left outer join (
select convenio_id, null as fecha_pago, sum(importe) as importe , sum(interes) as interes,  tipo , null as conv_que_paga 
from convenio_no_os_pagos
where tipo = 'CUO'
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION buscar_convenios_no_os_sin_recibos(character varying, character varying)
  OWNER TO postgres;
