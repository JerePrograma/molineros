-- Function: buscar_convenio_no_os_pagos(integer)

-- DROP FUNCTION buscar_convenio_no_os_pagos(integer);

CREATE OR REPLACE FUNCTION buscar_convenio_no_os_pagos(IN p_convid integer)
...
select   cp.convenio_id ,
  ...
  c.nro_cheque,
	 ...
	 c.id_banco,
	 co.id
  from convenio_no_os_pagos cp
  left outer join cheque c
  on cp.nro_cheque = c.nro_cheque
  left outer join convenio_no_os_relacion  cr
  on convenio_relacion_id = cr.id
  left outer join convenio_no_os co
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION buscar_convenio_no_os_pagos(integer)
  OWNER TO postgres;
