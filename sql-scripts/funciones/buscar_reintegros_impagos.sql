...
$BODY$
	...
	left outer join reintegro_prestacion rp
	on r.id_reintegro = rp.id_reintegro
	and ($7 = '' or ($7 != '' and rp.codigo = $7))
	left outer join 		
	lista_reintegro_pago_detalle opor
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION buscar_reintegros_impagos(integer, timestamp without time zone, timestamp without time zone, timestamp without time zone, timestamp without time zone, integer, character varying, character varying, integer, integer, character varying, character varying) OWNER TO postgres;
