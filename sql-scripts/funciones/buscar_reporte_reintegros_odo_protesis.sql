CREATE OR REPLACE FUNCTION buscar_reporte_reintegros_odo_protesis(IN id_reporte integer)
  ...
  rp_modi_fecha timestamp without time zone, rp_modi_usr character varying, rp_pieza character varying, 
  rp_cara character varying, rp_honorarios numeric, rp_gastos numeric, rp_id_prestador_externo integer, 
  rp_codigo character varying, n_id_especialidad integer, n_descripcion character varying, n_marca_rein_liq smallint, 
  ...
  n_baja_usr character varying, n_codigo character varying, a_cuil_titular character varying, 
  ...
  a_baja_usr character varying, a_discapacitado character varying, a_docu_numero character varying, a_nacionalidad integer, 
  a_aportante_titular integer, a_nro_afiliado integer, s_id_seccional integer, s_descripcion character varying, 
...
select oppr.id_lista_reintegro_reporte, r.id_reintegro, cuil_titular, inte, rp.id_prestacion, sum(rp.importe) as importe, sum(rp.cantidad) as cantidad, sum(rp.importe)*sum(rp.cantidad) as total
from  lista_reintegro_reporte_detalle oppr, reintegro r, reintegro_prestacion_odo_protesis rp
where 
oppr.id_lista_reintegro_reporte = id_reporte
and oppr.id_reintegro = r.id_reintegro
and r.id_reintegro = rp.id_reintegro
group by oppr.id_lista_reintegro_reporte, r.id_reintegro, r.cuil_titular, r.inte, rp.id_prestacion;
...
return query
	...
	where opor.id_lista_reintegro_reporte = id_reporte
	and opor.id_reintegro = r.id_reintegro
	and r.id_reintegro = rp.id_reintegro 
