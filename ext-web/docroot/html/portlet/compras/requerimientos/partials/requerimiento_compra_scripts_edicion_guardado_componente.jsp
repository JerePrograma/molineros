...
<script type="text/javascript">
    ...
    function <portlet:namespace />setGuardandoCompraActivo(activo) {
        ...
        var incierto = form && form.comprasGuardadoIncierto === true;
        var boton = document.getElementById('<portlet:namespace />btnGuardarCompras');
        if (!boton) {
            return;
        }
        boton.disabled = activo || incierto;
        if (boton.disabled) {
            boton.setAttribute('disabled', 'disabled');
        } else {
            boton.removeAttribute('disabled');
        }
        boton.value = activo ? 'Guardando...' : 'Guardar';
        if (activo) {
            jQuery(boton).addClass('compras-btn-guardando');
        } else {
            jQuery(boton).removeClass('compras-btn-guardando');
        }
    }
