...
<portlet:defineObjects/>
			<%
				...
				//Si debe mostrarse el btn de agregar afiliado								
					boolean showABMButtons = true;// PermissionUtil.userContainsRole(user,WebKeysTesoreria.ROL_ABM_CONVENIOS);		
					...
					if(null!=convenios){
					 	...
					 	for (int i = 0; i < convenios.size(); i++) {
					 		...
					 		rowURL.setWindowState(WindowState.MAXIMIZED);		 				
					 			...
					 			sb01.append("<a href='javascript:popupConvenio(\"");
					 			sb01.append(String.valueOf(String.valueOf(convenio.getId())));										
								sb01.append("\")'>");					 			
					 			sb01.append(convenio.getEntidad());
					 			sb01.append("</a>");
								...
								if(showABMButtons && !soloVer ) {
									row.addJSP( "left", SearchEntry.DEFAULT_VALIGN, "/html/portlet/uoma/conveniosNoOS/editar_borrar_convenio_no_os.jsp");
								}else{
									row.addText("");
								}
...
<script type="text/javascript">
	...
	function popupConvenio(convenio_id){
		...
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/view_convenios_no_os_entry&convenio_id='+convenio_id;
		jQuery(popup).load(url); 
	}
	function anularConvenio(id_con) {	
	    popup = Liferay.Popup({title:"<liferay-ui:message key="convenio" />",modal:true,position:[150,50],xy: ['center', 100],width:1000,
			 onClose: function() {
				 jQuery('#<portlet:namespace />buscar').click();
			 	}});        
	    var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/borrar_convenios_fecha';
	    url += '&id=' + id_con;
