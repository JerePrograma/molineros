...
<portlet:defineObjects/>
			...
			PortletURL portletURLPreAutMed = renderResponse.createRenderURL();
	 		...
	 		headerNamesPreAutMed.add("CUIL");
	 		headerNamesPreAutMed.add("Apellido y Nombre");
			...
			if(null!=precio.getAfiliados()){
 				List resultRowsInspector = searchContainer.getResultRows();
 			 	for (int i = 0; i < precio.getAfiliados().size(); i++) {
 			 		Afiliado detalle = precio.getAfiliados().get(i);
	 					ResultRow row = new ResultRow(detalle, detalle.getCuil_titular() , i);
//	 					row.addText(detalle.getNomenclador().getId_prestacion_string() );
	 					row.addText(String.valueOf(detalle.getCuil_titular()) );
	 					row.addText(detalle.getApellidoNombre());
	 					
	 						StringBuilder sb= new StringBuilder();
		 					sb.append("<img alt=\"<liferay-ui:message key='eliminar'/>\" src=\"");
		 					sb.append(themeDisplay.getPathThemeImages());
		 					sb.append("/common/delete.png\" onClick=\"javascript:sacarValor('");
		 					sb.append(detalle.getCuil_titular());
		 					sb.append("');\" />");
    ...
    <input type="hidden" name="<portlet:namespace />q_precio"
		id="<portlet:namespace />q_precio" value="<%=precio.getAfiliados().size()%>" />
