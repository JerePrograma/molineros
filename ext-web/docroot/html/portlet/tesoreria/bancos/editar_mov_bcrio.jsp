...
<%
	...
	try{
	   centros=CentroCostoServiceUtil.getContables(fecha_mov!=null?fecha_mov.getTime(): fechaDesde.getTime(), entidad);
	}catch(Exception e){}		
		...
		<table class="lfr-table">
			...
			<tr>
			   <td><label><liferay-ui:message key="fecha-mov" />:</label></td>
				<td colspan="2">
				    <div id="<portlet:namespace />divMovimiento">
			...
			<tr>
				...
				<td colspan="2">
					<select name="<portlet:namespace/>debcred" id="<portlet:namespace/>debcred"  onchange="cambiarDebCred();" <% if (!esEdicion) {%> disabled="disabled" <%} %>>									
				...
				<td>
				    <div id="<portlet:namespace />divCCosto">
				      <table class="lfr-table">
					  <tr>
				         <td><label>Centro de costo:</label></td>
				         <td>
					      <select id="<portlet:namespace />id_centroCosto" name="<portlet:namespace />id_centroCosto">
					           <option value="0">Seleccion un centro de costo</option>
						        <% 	for (CentroCosto centro : centros) {  %>
									<option <%= movSess != null && movSess.getCentroCosto().getId()!=null && movSess.getCentroCosto().getId() == centro.getId() ? "selected" : "" %> value="<%= centro.getId() %>"><%= centro.getDescripcion() %></option>
						        <%	} %>
					      </select>
				         </td>
			         </tr>
...
<script type="text/javascript">	
	...
	function cambiarDebCred(){
		var doc=jQuery("#<portlet:namespace/>debcred").val();

		if(doc=="DEBITO"){
		   jQuery('#<portlet:namespace />divCCosto').show();
		}else{
			jQuery('#<portlet:namespace />divCCosto').hide();
			jQuery('#<portlet:namespace />id_centroCosto').val(0);
		}
	
		
	}
	
	//Agregado para que replique la fecha del Movimiento en la del comprobante para que no la tengan que tipear

	jQuery(document).ready(function() {
	    // Definir los nombres base de los componentes (sin sufijos)
	    var sourceBase = '<portlet:namespace />fechaMov';
	    var targetBase = '<portlet:namespace />fechaComp';

	    // Función para replicar los valores de los selectores (Día, Mes, Año)
	    function syncDates() {
	        var d = jQuery('#' + sourceBase + 'Dia').val();
	        var m = jQuery('#' + sourceBase + 'Mes').val();
	        var y = jQuery('#' + sourceBase + 'Anio').val();

	        jQuery('#' + targetBase + 'Dia').val(d);
	        jQuery('#' + targetBase + 'Mes').val(m);
	        jQuery('#' + targetBase + 'Anio').val(y);
	    }
	    
	    var lastOpenedCalendar = "";
	    
	    // 1. Detectar cuándo se abre el calendario de ORIGEN
	    // Buscamos el icono (img) que Liferay pone al lado del select de Año
	    
	    jQuery('#' + sourceBase + 'Anio').siblings('.ui-datepicker-trigger').click(function() {
            lastOpenedCalendar = "source";
        });

        jQuery('#' + targetBase + 'Anio').siblings('.ui-datepicker-trigger').click(function() {
            lastOpenedCalendar = "target";
        });
	    
	    
	    // 1. Sincronizar si el usuario cambia manualmente los selectores desplegables
	    jQuery('#' + sourceBase + 'Dia, #' + sourceBase + 'Mes, #' + sourceBase + 'Anio').change(syncDates);
	    
	    
	    jQuery('input[name="' + sourceBase + '"]').change(function() {
	    	 setTimeout(syncDates(),100);
	    });
	    

	    // 2. Interceptar la selección desde el calendario desplegable
	    // En Liferay 5.2, el componente crea un objeto: [namespace][nombre]DatePicker
	    jQuery(document).mousedown(function(e) {
	        var isCalendarClick = jQuery(e.target).parents('.ui-datepicker').length > 0 || 
	                              jQuery(e.target).hasClass('ui-datepicker');
	        
	        if (isCalendarClick) {
	            // Esperamos a que el widget termine de procesar la selección
	            if (lastOpenedCalendar === "source") {
	                 setTimeout(function() {syncDates();}, 350);
	            }     
	        }
	    });
	});
