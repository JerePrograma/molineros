...
%>
		<fieldset class="block-labels">
				<legend>Balance Sumas Y Saldos</legend>
				...
				<select name="periodo_hasta" id="periodo_hasta">
					...
					<tr>
						<td >
							<select size="20" id="cuentas" name="cuentas" multiple="multiple">
								
							</select><a href="javascript:void(0)" onclick="help(event, 'helpCuentas')"><img style="height: 16px; width: 16px" src="/html/images/help.png" title="Ayuda" alt="Ayuda"/></a>
						</td>
					</tr>
					<tr>
						<td >
							...
							Incluir Ajuste por Inflación&nbsp;<input type="checkbox" id="incluir_ajuste_inflacion" name="incluir_ajuste_inflacion" />&nbsp;
							<input type="button" id="generar" onclick="generarDiario()" value="Generar Balance Sumas y Saldos" /><a href="javascript:void(0)" onclick="help(event, 'helpGeneracion')"><img style="height: 16px; width: 16px" src="/html/images/help.png" title="Ayuda" alt="Ayuda"/></a>
						</td>
					</tr>
				</table>	      	  
		</fieldset>	
...
function generarDiario(){
	...
	var incluir_ajuste_inflacion=document.getElementById('incluir_ajuste_inflacion');
	...
	jQuery.ajax({   
		...
		success: function(data){	
			...
			if (obj.status == "ok"){
				window.location.href ='/xlsservlet/?reporte=REPORTE_CONTABILIDAD_BALANCE_SUMAS_SALDOS'
				 ...
				 + '&incluir_ajuste_inflacion=' + (incluir_ajuste_inflacion.checked ? incluir_ajuste_inflacion.value : 'false')
...
function actualizarCuentas(){
	...
	jQuery.ajax({   
		...
		success: function(data){
			var obj = jQuery.parseJSON(data);
			jQuery('#cuentas').find('option').remove();
			for(var i =0;i< obj.cuentas.length; i++){
				jQuery('#cuentas').append('<option value="'+obj.cuentas[i].numero+'">'+obj.cuentas[i].numero + ' ' + obj.cuentas[i].cuenta +'</option>');
			}                                                                                                                                                                                                                                                            
		}
	});		
}
