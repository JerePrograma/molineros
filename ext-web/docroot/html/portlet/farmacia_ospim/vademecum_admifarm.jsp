...
%>
<liferay-ui:error key="vademecum-error" message='<%= HtmlUtil.escape(errorVademecum == null ? "No se pudo completar la operacion." : errorVademecum) %>' />
<liferay-ui:success key="vademecum-importado" message='<%= HtmlUtil.escape(exitoVademecum == null ? "Vademecum importado." : exitoVademecum) %>' />
...
for (String tipoVademecum : tiposVademecum) {
    ...
    try {
        ...
        errorConsulta = true;
        LogFactoryUtil.getLog("farmacia.vademecum_admifarm").error("Error consultando historico " + tipoVademecum, e);
    }
    String tokenVademecum = VademecumAdmifarmHelper.obtenerToken(renderRequest.getPortletSession(), user.getUserId(), tipoVademecum);
%>
<portlet:actionURL var="importarVademecumURL" windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>">
            ...
            <tr>
                <td style="vertical-align: top; width: 45%;">
                    <table class="lfr-table">
                        <tr>
                            <td><label for="<portlet:namespace />archivo_<%= tipoVademecum %>">Seleccionar archivo:</label></td>
                            <td><input type="file" id="<portlet:namespace />archivo_<%= tipoVademecum %>" name="archivo" accept=".xls,.xlsx"/></td>
                            <td>
                                <a href="javascript:void(0)" onclick="help(event, '<portlet:namespace />helpVademecum_<%= tipoVademecum %>')">
                                    <img style="height: 16px; width: 16px" src="/html/images/help.png" title="Ayuda" alt="Ayuda"/>
                                </a>
                            </td>
                            <td><input id="<portlet:namespace />botonImportar_<%= tipoVademecum %>" type="submit" value="Subir archivo"/></td>
                        </tr>
                    </table>
                    <div id="<portlet:namespace />procesandoVademecum_<%= tipoVademecum %>" style="display: none; margin-top: 15px;">
                        <liferay-ui:message key="procesando"/>
                        <img alt="Procesando" src="<%= themeDisplay.getPathThemeImages() %>/progress_bar/loading_animation.gif"/>
                    </div>
                </td>
                <td style="vertical-align: top; width: 55%;">
                    <table style="width: 100%;">
                        <tr><td><label>Listado de Vademecum <%= tituloVademecum %> importados:</label></td></tr>
                        <tr><td>&nbsp;</td></tr>
                        <% if (errorConsulta) { %>
                        ...
                        <% } else { %>
                            ...
                            <tr><td>
                                <table class="vademecum-tabla">
                                    <thead><tr><th>Fecha Importaci&oacute;n</th><th>Cantidad de registros</th><th>Descargar</th></tr></thead>
                                    <tbody>
                                    <%
                                    int numeroFila = 0;
                                    for (Object[] importacion : importaciones) {
                                        Timestamp fechaImportacion = (Timestamp) importacion[0];
                                        String claseFila = numeroFila++ % 2 == 0 ? "vademecum-fila-par" : "vademecum-fila-impar";
                                    %>
                                        <portlet:actionURL var="descargarVademecumURL" windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>">
                                            <portlet:param name="struts_action" value="/farmaciaospim/vademecum_admifarm"/>
                                            <portlet:param name="operacion" value="descargar"/>
                                            <portlet:param name="tipo" value="<%= tipoVademecum %>"/>
                                            <portlet:param name="fecha" value="<%= fechaImportacion.toString() %>"/>
                                        </portlet:actionURL>
                                        <tr class="<%= claseFila %>">
                                            <td title="<%= fechaImportacion.toString() %>"><%= formatoFecha.format(fechaImportacion) %></td>
                                            <td><%= importacion[1] %></td>
                                            <td><a href="<%= descargarVademecumURL %>" title="Descargar contenido, altas, bajas y modificaciones">Descargar</a></td>
                                        </tr>
                                    <% } %>
                                    </tbody>
                                </table>
...
<style type="text/css">
    #<portlet:namespace />vademecumAdmifarm .vademecum-tabla { width: 100%; border-collapse: collapse; }
    #<portlet:namespace />vademecumAdmifarm .vademecum-tabla th { background-color: #918580; color: #FFFFFF; font-weight: bold; padding: 8px 6px; text-align: center; }
    #<portlet:namespace />vademecumAdmifarm .vademecum-tabla td { padding: 7px 6px; text-align: center; }
    #<portlet:namespace />vademecumAdmifarm .vademecum-fila-par { background-color: #F3EEEB; }
    #<portlet:namespace />vademecumAdmifarm .vademecum-fila-impar { background-color: #DED9D6; }
</style>
