...
List<ArchivoAdmifarm> archivos ;

archivos=FarmaciaServiceUtil.getArchivosSubidosAdmifarmOspimGeneral(); 
...
if (archivos != null && !archivos.isEmpty()){
	...
	List resultRows = searchContainer.getResultRows();
 	...
 	for (int i = 0; i < archivos.size(); i++) {	    
 		ArchivoAdmifarm liq = (ArchivoAdmifarm) archivos.get(i);
		...
		row.addText(format2D.format(liq.getTotalospim())); //mostrar ImporteNeto
		row.addText(format2D.format(liq.getTotaluoma()));
	 	...
	 	StringBuilder sbo=new StringBuilder();
			...
			sbo.append(" onClick=\"javascript:exportacionAdmifarmOspimGeneral('");
...
function exportacionAdmifarmOspimGeneral(periodo){
    window.location.href ='/xlsservlet/?reporte=REPORTE_ARCHIVO_ADMIFARM_OSPIM_GENERAL_PERIODO'
        +'&periodo='+periodo;
}
</script>
