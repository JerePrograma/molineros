...
<portlet:defineObjects/>
			<%
				//Si debe mostrarse el btn de agregar afiliado								
					...
					if (user != null) {
						try {
							puedeModificarSolicitarCotizacion = RoleLocalServiceUtil.hasUserRole(
									user.getUserId(), user.getCompanyId(), rolSolicitarCotizacion, true);
						} catch (Exception e) {
					...
					if(null!=prestadores){				 								 
					 	...
					 	for (int i = 0; i < prestadores.size(); i++) {
							...
							String titleSolicitarCotizacion = puedeModificarSolicitarCotizacion
									? " title=\"Solicitar Cotización\" "
									: " title=\"No posee permisos para modificar Solicitar Cotización\" ";
							String solicitarCotizacionHtml = "<input type=\"checkbox\" "
									...
									+ checkedSolicitarCotizacion + disabledSolicitarCotizacion
