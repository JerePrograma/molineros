	...
	<tr>
		<td colspan="1">
			...
			<%-- <select id="<portlet:namespace/>vistaDetalle" name="<portlet:namespace/>vistaDetalle">
				<optgroup label="Visualización del Detalle">
...
function <portlet:namespace />validarCamposDetalle(){
	...
	if(jQuery("#<portlet:namespace />codigo_desde").val() == ""){
		alert("Código Desde obligatorio");
	...
	if ( (codigo_d.localeCompare(codigo_h) == 0) && (parseInt(codigo_d, 10) > parseInt(codigo_h, 10)) ){
		alert('El Código Hasta debe ser mayor al Código Desde');
		return false;
	}
...
function <portlet:namespace />agregarDetalle() {
	if (<portlet:namespace />validarCamposDetalle()) {
		...
		var tipoNome = jQuery("#<portlet:namespace />tipoNomencladorfiltro").val();
		var tipoNome_text = jQuery("#<portlet:namespace />tipoNomencladorfiltro :selected").text();
		var codigoDsd = jQuery("#<portlet:namespace />codigo_desde").val();
		var codigoHta = jQuery("#<portlet:namespace />codigo_hasta").val();
		var idPrestDsd = jQuery("#<portlet:namespace />id_prestacion_desde").val();
		var idPrestHta = jQuery("#<portlet:namespace />id_prestacion_hasta").val();
		
		var diaD = document.getElementById("<portlet:namespace />prestacionFechaDesdeDia").value; 
		/* var mesD = parseInt(document.getElementById("<portlet:namespace />prestacionFechaDesdeMes").value, 10); */
		var mesD = parseInt(document.getElementById("<portlet:namespace />prestacionFechaDesdeMes").value)+1;
		var anioD = document.getElementById("<portlet:namespace />prestacionFechaDesdeAnio").value;		
		var fechaDesdeFinal = diaD+'/'+mesD+'/'+anioD;

		var diaH = document.getElementById("<portlet:namespace />prestacionFechaHastaDia").value; 
		/* var mesH = parseInt(document.getElementById("<portlet:namespace />prestacionFechaHastaMes").value, 10); */
		var mesH = parseInt(document.getElementById("<portlet:namespace />prestacionFechaHastaMes").value)+1;
		var anioH = document.getElementById("<portlet:namespace />prestacionFechaHastaAnio").value;
		var fechaHastaFinal = diaH+'/'+mesH+'/'+anioH;

		var servicio = jQuery("#<portlet:namespace />servicio").val();
		...
		var cosegu = jQuery("#<portlet:namespace />coseguro").val();
		...
		var params = {"tipoNomenclador" : tipoNome, "tipoNomencladorDesc" : tipoNome_text,
				"idPrestDesde" : idPrestDsd, "idPrestHasta" : idPrestHta, 
				"codigoDesde" : codigoDsd, "codigoHasta" : codigoHta, 
				"fechaDesde" : fechaDesdeFinal, "fechaHasta" : fechaHastaFinal, 
				"servicio" : servicio, "planId" : planId, "planDesc" : plan_text, 
				"tipoValorizacion" : tipoValoriz, "coseguro" : cosegu, 
				"importe" : importe, "porcentaje" : porcentaje, "<%=Constants.CMD%>":"<%=Constants.ADD%>"};
		
		jQuery('#<portlet:namespace />convenio_prest_detalle').load(url,params, function() {
			jQuery('#<portlet:namespace />buscandoDetalles').hide();            															
		});
		
		<portlet:namespace />limpiarDetalle();
	}
	return false;
}
