...
<%
	...
	String rps=(String)request.getSession().getAttribute("RECLAMOS_PROCESAR_IMAGENES");
...
<form action="" method="post" name="<portlet:namespace />fmSSS" enctype="multipart/form-data"> 
	...
	<tr>
	<table><tr>
	<td><label style="color:blue; font-size: 15px" >Los números de Reclamos deben estar separados por punto y coma</label></td>
	</tr></table>
	</tr>
   </table>	
...
function <portlet:namespace />uploadArchivo() {
		...
		var url = '<portlet:actionURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"/>&struts_action=/<%=portlet_name%>/editar_reclamosprestaciones_entry';
		url = url + '&cmd=upload'; 
