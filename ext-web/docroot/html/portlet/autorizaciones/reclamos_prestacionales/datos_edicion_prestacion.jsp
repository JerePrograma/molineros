...
(ReclamoPrestacional) request.getSession().getAttribute(
    WebKeysAutorizaciones.RECLAMO_PRESTACION_EN_EDICION
);

boolean esFarmacia =
reclamoEnEdicion != null
&& "FARMACIA".equalsIgnoreCase(reclamoEnEdicion.getSector())
&& "REINTEGRO".equalsIgnoreCase(reclamoEnEdicion.getTipoPedido());
...
<%if (!esFarmacia){ %>
    ...
    jQuery("#<portlet:namespace />codigoSeguimiento_filtro_edit")
        .val('<%=Validator.isNotNull(prestacionEnEdicion.getCodigoPrestacion())
            ? prestacionEnEdicion.getCodigoPrestacion()
            : ""%>');

    jQuery("#<portlet:namespace />descripcionSeguimiento_filtro_edit")
        .val('<%=Validator.isNotNull(prestacionEnEdicion.getDescripcion())
            ? prestacionEnEdicion.getDescripcion()
            : ""%>');

    <% if (Validator.isNotNull(prestacionEnEdicion.getCodigoPrestacion())) { %>
        <portlet:namespace />buscarNomencladorAutocompletar_edit();
    <% } %>

<%}else{%>

    jQuery("#<portlet:namespace />nom_seleccionado").val("2");

    jQuery("#<portlet:namespace />troquel_edit")
        .val('<%=prestacionEnEdicion.getId_medicamento()%>');
			...
			<table>					
	         <tr>		
			    ...
			    <td><input id="<portlet:namespace />reconocidoSSSEdicion"
				    ...
				    value ='<%=Validator.isNotNull(prestacionEnEdicion)  ? new BigDecimal(prestacionEnEdicion.getReconocidoSSS()).setScale(2, RoundingMode.HALF_UP).toPlainString(): ""  %>'     
				    type="text" value=""  onkeypress="return validaMonto(event,this)" /></td>    
				
			    <td><label><liferay-ui:message key="Recuperable SSS" />:</label></td>
				<td>
