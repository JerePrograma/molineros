...
function manejarTipoSector(){
	...
	try {
		...
		jQuery("#" + reclamoPrestacionalNamespace + "busqueda_farmacia").hide();
		jQuery("#" + reclamoPrestacionalNamespace + "nom_seleccionado").val("1"); // se selecciono maestra de prestaciones medicas 
		...
		if (tipoSector.selectedIndex==3) {
   		...
   		if (tipoSector.selectedIndex==1){     	       
   			...
   			jQuery("#" + reclamoPrestacionalNamespace + "tipoNomencladorSeguimiento_filtro").val(1); // discapacidad	
   		}
	}
	catch (err) {
		alert('error manejarTipoSector() ');
