...
<table  align="center" class="lfr-table" style="border-collapse: separate; border-spacing: 3px;">
		 <tr>															
			<td colspan="3">				
				<fieldset class="block-labels">
				<legend>
				<liferay-ui:message key="Psic�loga" />
				</legend>
		...
		<tr>
			<td colspan="3">
				<% if (BtnFirmar == 0 && esFirmaPsicologo == false){  %>
					<div   id="<portlet:namespace />id_firma_psicologo"  align="center"> <input type="button"  value="Firma Psic�logo"
						      onClick="<portlet:namespace />agregarFirmaPrestacion(0);"  id="<portlet:namespace />buttonaddfirma" name="<portlet:namespace />buttonaddfirma"  title="Firma psic�logo"/>
					</div>
				<%}%>	
			</td>
		...
		<tr>
			<td colspan="3">
				<% if (BtnFirmar == 3 && esFirmaTerapiaFisica == false){  %>
					<div   id="<portlet:namespace />id_firma_terapia_fisica" align="center"> <input type="button"  value="<liferay-ui:message key="firma-dictamen-licenciado-kinesiolog�a-terapia-fisica" />"
...
function <portlet:namespace />editaEquipoInterdisciplinario() {
	...
	if (validaDatos()) 
		{	
			...
			<%if(esFirmaAsistenteSocial && esFirmaAuditor && esFirmaTerapiaFisica){%>
				alert('Se encuentran las firmas de los dict�menes, puede cerrar el caso.');
			<%}%>
