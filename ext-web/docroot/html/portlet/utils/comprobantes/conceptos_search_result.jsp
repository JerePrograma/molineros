...
<%
			...
			if (comp.getConceptos() != null){
				for (ComprobanteConcepto cc : comp.getConceptos()){
					if (conceptos.contains(cc)){
						...
						if (comp.isDebitoParaEgreso()) {
							conceptos.add(new ComprobanteConcepto(cc.getConceptoComprobante(),cc.getCentroCosto(),0D,cc.getImporte().negate()));
						} else {
							conceptos.add(new ComprobanteConcepto(cc.getConceptoComprobante(),cc.getCentroCosto(),0D,cc.getImporte()));
						}
					}
				}
			}
	...
	if (null != conceptos && conceptos.size() > 0) {
		for (int i = 0; i < conceptos.size(); i++) {
			...
			if(comp.getCentroCosto()!=null && comp.getCentroCosto().getDescripcion()!=null){
				   row.addText(comp.getCentroCosto().getDescripcion());
			}else{
				   row.addText("");
			}
