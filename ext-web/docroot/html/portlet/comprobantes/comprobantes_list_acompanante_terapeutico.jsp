...
%>
	...
	<table class="lfr-table">			
				...
				<tr>
					...
					<td><label>Reclamo Prestacional:</label></td>
					 <td colspan="2"><input type="text" id="<portlet:namespace />nro_reclamo" 
					     name="<portlet:namespace />nro_reclamo" value="" maxlength="25"/></td>
					<td colspan="4">&nbsp;</td>
					<td style="background-color:#AEB6BF">
						<label>Pendientes de Asignar a Reclamo Prestacional:</label>
			            <input type="checkbox"
    ...
    <table>	
		...
		<table>		
				<tr>
					...
					<td>
						<input id="<portlet:namespace />incluir" value="Generar Reclamos" title="Generar Reclamos Prestacionales" type="button"
						onclick="javascript:<portlet:namespace />generarReclamos();"/>							
					</td>
		...
		<fieldset class="block-labels">
			...
			<div align="center" id="<portlet:namespace />busquedaComprobDiv">
				<liferay-util:include page="/html/portlet/comprobantes/comprobantes_search_result_acompanante_terapeutico.jsp">
...
function <portlet:namespace />buscarCptes(){		
				...
				var reclamo = jQuery("#<portlet:namespace />nro_reclamo").val();
				var usr ="<%=usuario%>";
				...
				var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/comprobantes_portal_proveedores_acompanantes';
...
function <portlet:namespace />limpiarFiltro(){		
	...
	jQuery("#<portlet:namespace />nro_reclamo").val('');
...
function <portlet:namespace />generarReclamos(){
	var trat = document.getElementsByName('comprob');
	var tratValue = "";
	var i = 0;
	for (i = 0; i<trat.length; i++){
	...
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/comprobantes_portal_proveedores_acompanantes';
	url += '&cmd=generarReclamos&ids='+tratValue;
	url += '&rnd=' + Math.floor(Math.random()*100);
	jQuery('#<portlet:namespace />buscando').show();
	jQuery('#<portlet:namespace />busquedaComprobDiv').load(url, function() {jQuery('#<portlet:namespace />buscando').hide();});
