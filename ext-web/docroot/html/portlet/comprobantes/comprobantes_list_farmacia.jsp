		   ...
		   <tr>			
			...
			<td>
					<select id="<portlet:namespace />sector" name="<portlet:namespace />sector">
					    <option value="Acompañamiento Terapéutico y Cuidadores">Acompañamiento Terapéutico y Cuidadores</option>
						<option value="Farmacia (Droguería)">Farmacia (Droguería)</option>	
						<option value="Internación Domiciliaria">Internación Domiciliaria</option>
	...
	<table class="lfr-table">			
				...
				<tr>
					...
					<td><label>Liquidación:</label></td>
					 <td colspan="2"><input type="text" id="<portlet:namespace />nro_liquidacion" 
					     name="<portlet:namespace />nro_liquidacion" value="" maxlength="25"/></td>
					<td colspan="4">&nbsp;</td>
					<td style="background-color:#AEB6BF">
						<label>Pendientes de Asignar a Liquidación:</label>
			            <input type="checkbox"
    ...
    <table>	
		...
		<table>		
				<tr>
					...
					<td>
						<input id="<portlet:namespace />incluir" value="Generar Liquidaciones" title="Generar Liquidaciones" type="button"
						onclick="javascript:<portlet:namespace />generarLiquidaciones();"/>							
					</td>
		...
		<fieldset class="block-labels">
			...
			<div align="center" id="<portlet:namespace />busquedaComprobDiv">
				<liferay-util:include page="/html/portlet/comprobantes/comprobantes_search_result_farmacia.jsp">
...
function <portlet:namespace />buscarCptes(){		
				...
				var liquidacion = jQuery("#<portlet:namespace />nro_liquidacion").val();
				var usr ="<%=usuario%>";
				...
				var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/comprobantes_portal_proveedores_farmacia';
...
function <portlet:namespace />limpiarFiltro(){		
	...
	jQuery("#<portlet:namespace />nro_liquidacion").val('');
...
function <portlet:namespace />generarLiquidaciones(){
	var trat = document.getElementsByName('comprob');
	var tratValue = "";
	var i = 0;
	for (i = 0; i<trat.length; i++){
	...
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/comprobantes_portal_proveedores_farmacia';
	url += '&cmd=generarLiquidaciones&ids='+tratValue;
	url += '&rnd=' + Math.floor(Math.random()*100);
	jQuery('#<portlet:namespace />buscando').show();
	jQuery('#<portlet:namespace />busquedaComprobDiv').load(url, function() {jQuery('#<portlet:namespace />buscando').hide();});
