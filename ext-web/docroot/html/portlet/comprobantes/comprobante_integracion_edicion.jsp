...
<%
   NumberFormat format2D = new DecimalFormat("#0.00");
	...
	ComprobanteIntegracion comprobante=(ComprobanteIntegracion)request.getSession().getAttribute(WebKeysComprobantes.COMPROBANTE_INTEGRACION_EN_EDICION);
	...
	Calendar fechaVencimientoCUD = CalendarFactoryUtil.getCalendar();
	fechaVencimientoCUD.setTime(new Date());
	if(comprobante.getCudVto()==null){
		fechaVencimientoCUD.setTime(new Date());
	}else{
	  fechaVencimiento.setTime(comprobante.getCudVto());
	}
...
<form action="" method="post" name="<portlet:namespace />fmS">
	...
	<td><label>CUD: </label></td>
	<td>
	     <input type="text" id="<portlet:namespace />cud" name="<portlet:namespace />cud" 
						     size="50" value="<%=comprobante.getCud() !=null?comprobante.getCud():""%>" maxlength="60"/>
	</td>
	<td><label>Vto CUD: </label></td>
    <td colspan="3">
	  <liferay-ui:input-date
						    monthNullable="true" 
						    dayNullable="true"
						    yearNullable="true"
						    dayParam="fechaCudDia"
						    dayValue="<%=comprobante.getCudVto()!=null?fechaVencimiento.get(Calendar.DAY_OF_MONTH ):fechaHasta.get(Calendar.DAY_OF_MONTH )%>"
						    monthParam="fechaCudMes"
						    monthValue="<%=comprobante.getCudVto()!=null?fechaVencimiento.get(Calendar.MONTH ):fechaHasta.get(Calendar.MONTH )%>"
						    yearParam="fechaCudAnio" 
						    yearRangeStart="<%= fecha.get(Calendar.YEAR) - 10 %>"
						    yearValue="<%=comprobante.getCudVto()!=null?fechaVencimiento.get(Calendar.YEAR ):fechaHasta.get(Calendar.YEAR ) %>"
						    yearRangeEnd="<%= fecha.get(Calendar.YEAR) + 10 %>"
						    firstDayOfWeek="<%= fecha.getFirstDayOfWeek() - 1 %>"
						    disabled="<%= false %>" />
    </td>
	...
	<tr><td colspan="8">&nbsp;</td></tr>
	<tr>
	 <td><label>Importe Solicitado:</label> </td>
	 <td>
		<input type="text" id="<portlet:namespace />importe_solicitado" name="<portlet:namespace />importe_solicitado" 
		     size="20" value="<%=comprobante.getImporteSolicitado()!=null?format2D.format(comprobante.getImporteSolicitado()):""%>" maxlength="60"/>
	 </td>
	 
	 <td><label>Provincia:</label> </td>
	 <td>
		<input type="text" id="<portlet:namespace />provincia" name="<portlet:namespace />provincia" 
		     size="10" value="<%=comprobante.getProvincia()!=null?comprobante.getProvincia():"0"%>" maxlength="5"/>
	 </td>
	 <td><label>Dependencia:</label> </td>
	 
	 <td>
	 
	    <select name="<portlet:namespace />dependencia"  id="<portlet:namespace />dependencia" >
						<option
							value="N"
							<%if (comprobante != null && comprobante.getDependencia()!=null  && 
					              (comprobante.getDependencia().equalsIgnoreCase("N"))) { %>
							selected="selected" <%}%>>
							N
						</option>
						...
						<option
							value="S"
							<%if (comprobante != null && comprobante.getDependencia()!=null  && 
					              (comprobante.getDependencia().equalsIgnoreCase("S"))) { %>
							selected="selected" <%} %>>
							S
						</option>
...
function <portlet:namespace />salvarEdicion(){
	...
	if (<portlet:namespace />validarCampos()) {
		...
		'<liferay-portlet:param name="struts_action" value="/comprobantes/comprobantes_portal_proveedores_integracion" />'+
...
function <portlet:namespace />validarCampos(){
	var result = true;
	var dependencia =jQuery('#<portlet:namespace />dependencia').val();
	var provincia =jQuery('#<portlet:namespace />provincia').val();
	var importe =jQuery('#<portlet:namespace />importe').val();
	var importeSolicitado =jQuery('#<portlet:namespace />importe_solicitado').val();
	var cud =jQuery('#<portlet:namespace />cud').val();
	
	var diaE = document.getElementById("<portlet:namespace />fechaCudDia").value; 
	var mesE = document.getElementById("<portlet:namespace />fechaCudMes").value; 
	var anioE = document.getElementById("<portlet:namespace />fechaCudAnio").value;
	...
	if(importeSolicitado.includes(",")){
		importeSolicitado=importeSolicitado.replace(".","").replace(",",".");
	}
	...
	if(parseFloat(importeSolicitado) == "0"){
		alert("El Importe solicitado no puede ser cero")
		return false;
	}
	
	
	
	if(parseFloat(importeSolicitado)> parseFloat(importe)){
		alert("El Importe solicitado no puede ser mayor al importe del comprobante")
		return false;
	}
	
	if(dependencia==null || dependencia==""){
		alert("La dependencia no puede estar vacía");
		return false;
	}
	...
	if(cud==null || cud==""){
		alert("El código CUD no puede estar vacío");
		return false;
	}
	
	if((diaE==null || diaE=="") || (mesE==null || mesE=="") ||
			(anioE==null || anioE=="")	){
		alert("La fecha de Vto no puede estar vacía");
		return false;
	}
