...
<portlet:defineObjects/>
			<%
					...
					List<ComprobanteIntegracion> comprobantes = (ArrayList<ComprobanteIntegracion>)request.getSession().getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_INTEGRACION);
					...
					if(null!=comprobantes){					
					 	...
					 	for (int i = 0; i < comprobantes.size(); i++) {
					 		ComprobanteIntegracion comp = comprobantes.get(i);
                            ...
                            */
                            row.addText(sb000);
	 						...
	 						row.addText(comp.getCarpetaStr());
	 						
	 						row.addText(comp.getCodigoPrestacion()+"  "+ comp.getDescripcionPrestacion());
...
<script type="text/javascript">
	...
	function editaComprobanteProveedor(cuit,tipo,letra,ptovta,numero){
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/comprobantes/comprobantes_portal_proveedores_integracion" /></portlet:renderURL>';
        url += "&accion=edit";
        url += '&cuit='+cuit;
        url += "&tipo="+tipo;
