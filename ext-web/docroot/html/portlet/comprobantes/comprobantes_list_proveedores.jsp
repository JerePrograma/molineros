	...
	<table class="lfr-table">			
				...
				<tr>
					...
					<td style="background-color:#AEB6BF">
						<label>Pendientes de Generar Precarga:</label>
			            <input type="checkbox"
    ...
    <table>	
		...
		<table>		
				<tr>
					...
					<td>
						<input id="<portlet:namespace />incluir" value="Generar Precarga" title="Generar Precarga" type="button"
						onclick="javascript:<portlet:namespace />generarPrecarga();"/>							
					</td>
		...
		<fieldset class="block-labels">
			...
			<div align="center" id="<portlet:namespace />busquedaComprobDiv">
				<liferay-util:include page="/html/portlet/comprobantes/comprobantes_search_result_proveedores.jsp">
...
function <portlet:namespace />buscarCptes(){		
				...
				var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/comprobantes_portal_proveedores_comunes';
...
function <portlet:namespace />generarPrecarga(){
	var trat = document.getElementsByName('comprob');
	var tratValue = "";
	var i = 0;
	for (i = 0; i<trat.length; i++){
	...
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/comprobantes_portal_proveedores_comunes';
	url += '&cmd=generarPrecarga&ids='+tratValue;
	url += '&rnd=' + Math.floor(Math.random()*100);
	jQuery('#<portlet:namespace />buscando').show();
	jQuery('#<portlet:namespace />busquedaComprobDiv').load(url, function() {jQuery('#<portlet:namespace />buscando').hide();});
