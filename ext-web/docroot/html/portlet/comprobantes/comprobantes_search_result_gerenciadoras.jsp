...
<portlet:defineObjects/>
			<%
					...
					List<ComprobanteHospital> comprobantes = (ArrayList<ComprobanteHospital>)request.getSession().getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_GERENCIADORAS);
					...
					if(null!=comprobantes){					
					 	...
					 	for (int i = 0; i < comprobantes.size(); i++) {
                            ...
                            row.addText( comp.getAcreedorEmpresa().getRazon_soc());
	 						...
	 						for(ReclamoPrestacional r: comp.getReclamos()){
	 						   sbe.append("<p style='color:");
	 						   sbe.append(r.getEstado()!=3?"red":"green");
	 						   sbe.append( "';>");
	 						   sbe.append(String.valueOf(r.getId_reclamo()));
	 						   sbe.append( "</p>");
	 						}
...
<script type="text/javascript">
	...
	function editaComprobanteProveedor(cuit,tipo,letra,ptovta,numero){
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/comprobantes/comprobantes_portal_proveedores_gerenciadoras" /></portlet:renderURL>';
        url += "&accion=edit";
        url += '&cuit='+cuit;
        url += "&tipo="+tipo;
	...
	function verImagenComprobante(folderId,fileName){
		   ...
		   //window.open(url,'mywindow','width=800,height=800,toolbar=no,resizable=yes') 
		    ...
		    window.open(url,fileName,'width=800,height=800,toolbar=no,resizable=yes') 
	}
