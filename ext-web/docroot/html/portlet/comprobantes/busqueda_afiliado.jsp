				...
				<table class="lfr-table" style="border-collapse: separate; border-spacing: 5px;">
					...
					<tr>
						...
						<td colspan="4" style="vertical-align:top" >
						       <liferay-util:include page='/html/portlet/comprobantes/busqueda_seccional.jsp'>
...
<script type="text/javascript">
	...
	function <portlet:namespace />buscarAfiliados<%=prefijo%>(){
		...
		<c:if test="<%= !Boolean.parseBoolean(pag_reintegro) %>">
			...
			var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/comprobantes/buscar_afiliados&cuil='+cuil+
	...
	function seleccionaAfiliado<%=prefijo%>(cuil,inte,docu_tipo,docu_nro,nombre,apellido,id_secc,desc_secc,ospim,uoma,amtima,bajaFecha,nombre_plan,id_plan,fecha_alta_af,incapacidad_af,id_tercerizadora ,afi_tercerizadora){
...
//		     return false;
		...
		seleccionaCamposAfiliado<%=prefijo%>(cuil,inte,docu_tipo,docu_nro,nombre,apellido,id_secc,desc_secc,ospim,uoma,amtima,bajaFecha,nombre_plan,id_plan,fecha_alta_af,incapacidad_af,id_tercerizadora,afi_tercerizadora);
		Liferay.Popup.close(popupAfill);
		
//      Aviso Situacion Laboral
        if("true"=="<%=fromReclamo%>"){
    	    ...
    	    jQuery.ajax({   
    		   ...
    		   success: function(data){
    	        ...
    	        if("true"== obj.mostrarAviso){
    	            popSituLab = Liferay.Popup({title:"Alerta de Permanencia",modal:true,width:530});        	
...
//
	...
	function seleccionaCamposAfiliado<%=prefijo%>(cuil,inte,docu_tipo,docu_nro,nombre,apellido,id_secc,desc_secc,ospim,uoma,amtima,bajaFecha,nombre_plan,id_plan,fecha_alta_af,incapacidad_af,id_tercerizadora,afi_tercerizadora){			
				...
				jQuery.ajax({   
					...
					success: function(data){
						...
						if(fechaVto !=null){	
							...
							jQuery('#<portlet:namespace/>discapacidad_vto').html("Vto.Documentaci�n "+fechaVto);
							if(vto<hoy ){
