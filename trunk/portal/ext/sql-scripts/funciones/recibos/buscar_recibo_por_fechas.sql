drop FUNCTION buscar_recibo_por_fechas(IN p_fecha_ini date, IN p_fecha_fin date, IN p_cuit character varying, IN p_sucu character varying);

CREATE OR REPLACE FUNCTION buscar_recibo_por_fechas(IN p_fecha_ini date, IN p_fecha_fin date, IN p_cuit character varying, IN p_sucu character varying)
  ...
  emp__baja_ip character varying, emp__id_posicion_iva smallint, emp__domiafip character varying, emp__domiremo varchar, emp__domiestudio varchar, rec__id_seccional integer) AS
$BODY$
...
and id not in (select id from recibo r
