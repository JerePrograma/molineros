...
<%if(esEdicion){%>	
	...
	<tr>
		...
		<td>
			<input type="text" id="<portlet:namespace />cargo" name="<portlet:namespace />cargo" value=""/>							
		</td>
		<td>			
			<liferay-ui:message key="telefono"/>:			
		</td>
		<td>			
			<input type="text" value="" name="<portlet:namespace />telefono_contacto" id="<portlet:namespace />telefono_contacto" />
		</td>
		<td>			
	...
	<tr>
		<td colspan="11" align="center">
			<div align="center" id="<portlet:namespace />contactos">
				<liferay-util:include page="/html/portlet/cgt_ddhh/organismos/contactos_search_result.jsp">						
...
<script type="text/javascript">	
	...
	function <portlet:namespace />agregarContacto(){
			...
			var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/cgt_ddhh/agregar_contacto';			
			url=url+'&nombre=' +encodeURI(nombre) 
						+'&apellido=' + encodeURI(apellido)
						+'&id_cargo=' + encodeURI(idCargo) 
						+'&email=' + encodeURI(email)
						+'&telefono_contacto='+encodeURI(telefono)
			...
			jQuery('#<portlet:namespace />contactos').load(url, function() {
														jQuery('#<portlet:namespace />agregandoContacto').hide();	
														 ...
														 jQuery('#<portlet:namespace />cargo').val("");
														 jQuery('#<portlet:namespace />telefono_contacto').val("");
	...
	function borraContacto(id_cargo, nombre, apellido){		
		if(!confirm("<liferay-ui:message key='are-you-sure-you-want-to-delete-this-entry'/>")){
			return false;
		}else{		
			var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/cgt_ddhh/sacar_contacto_organismo';		
			url=url+'&id_cargo=' +encodeURI(id_cargo) 
