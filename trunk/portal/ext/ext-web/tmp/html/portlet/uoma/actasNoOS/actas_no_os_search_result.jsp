...
<%
		...
		if (portlet_name == null || portlet_name.trim().equals("")){
			portlet_name = "estudio_isidro";
	 		...
	 		boolean showABMButtons = PermissionUtil.userContainsRole(user,"ABM_CalculoDeuda")|| portlet_name.equals("farmacia")|| portlet_name.equals("uoma");
		    ...
		    if (isActa) {
				...
				//Si debe mostrarse el btn de agregar afiliado								
					...
					SearchContainer.DEFAULT_CUR_PARAM,1600, portletURL, headerNames,
					 	...
					 	for (int i = 0; i < actas.size(); i++) {
					 		Acta acta = (Acta) actas.get(i);
					 				...
					 				rowURL.setWindowState(WindowState.MAXIMIZED);		 
										...
										sb11.append("<a href='javascript:popupActa(\"");
										sb11.append(String.valueOf(acta.getId()));										
										sb11.append("\")'>");
										sb11.append(acta.getEntidad());
										sb11.append("</a>");
										...
										sb3.append("\")'>");			
										sb3.append(acta.getTotalCapital() != null ? acta.getTotalCapital().toString():"");
										sb3.append("</a>");				
										...
										sb4.append("\")'>");			
										sb4.append(acta.getTotalInteres() != null ? acta.getTotalInteres().toString() : "");
										sb4.append("</a>");
									...
									if(showABMButtons && !soloVer) {
										row.addJSP( "left", SearchEntry.DEFAULT_VALIGN, "/html/portlet/uoma/actasNoOS/editar_borrar_acta_no_os.jsp");										
									}else{
										row.addText("");
									}
...
<script type="text/javascript">	
	...
	function popupActa(acta_id){
		...
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/view_actas_no_os_entry&acta_id='+acta_id;
		jQuery(popup).load(url); 
	}
	
	function anularActaNoOS(id_op, is_acta_cerrada,entidad) {		
		 var url = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>"/>&struts_action=/<%=portlet_name%>/borrar_actas_no_os_entry';
		 jQuery('#id').val(id_op);		 
