...
public class FacturaIngreso {
	...
	public FacturaIngreso(Ingreso ingreso, int id) {
		this.ingreso = ingreso;
		this.id = id;
	}

	public FacturaIngreso(Ingreso ingreso) {
		this.ingreso = ingreso;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		FacturaIngreso other = (FacturaIngreso) obj;
		if (ingreso == null) {
	...
	public static FacturaIngreso getMapping(ResultSet rs, int entidad)
			throws SQLException {
		return getMapping(rs, "", entidad);
	}

	public static FacturaIngreso getMapping(ResultSet rs, String prefix,
		...
		int tipoDeposito = rs.getInt(prefix + "id_factura_ingreso_tipo_deposito");
		String nroCheque = rs.getString(prefix + "nro_cheque");
		String ctaBcriaDesc=rs.getString("ch__cta_bcria")!=null?rs.getString("ch__cta_bcria"):"";
		...
		int idAntic = 0;
		try {
			idAntic = rs.getInt(prefix + "id_anticipo_factura_concepto");
		} catch (Exception e) {	}
		...
		if (nroCheque != null) {
			...
			if(tipoDeposito==3 || tipoDeposito==4) {
			   TarjetaDebitoCredito tarjeta= new TarjetaDebitoCredito();
			   tarjeta.setBanco(new Banco(rs.getInt("fi__id_banco"), ""));
			   tarjeta.setTipo(tipoDeposito);
			   tarjeta.setNumero(nroDepositoBancario);
			   tarjeta.setImporte(importe);
			   tarjeta.setFecha(fecha);
			   tarjeta.setEmisorDescripcion(rs.getString("fi__tarjeta_emisor_descripcion"));
			   try {
			      tarjeta.setEmisor(rs.getInt("fi__tarjeta_emisor_id"));
			      tarjeta.setCuotas(rs.getInt("fi__tarjeta_cuotas"));
			   } catch(Exception e) {}   
			   ingreso=tarjeta;
			}else if(tipoDeposito==FinanciacionTurismo.TURISMO) {
				FinanciacionTurismo ft = new FinanciacionTurismo();
				ft.setImporte(importe);
				ft.setFecha(fecha);
				ingreso=ft;
			}else if(tipoDeposito==CuentaCorriente.CTACTE) {
				CuentaCorriente ft = new CuentaCorriente();
				ft.setImporte(importe);
				ft.setFecha(fecha);
				ingreso=ft;
			}else if(tipoDeposito==Retencion.GRAL || tipoDeposito==Retencion.IIBB ||
					tipoDeposito==Retencion.IVA || tipoDeposito==Retencion.SUSS  ) {
				Retencion ft = new Retencion();
				ft.setTipo(tipoDeposito);
				ft.setImporte(importe);
				ft.setFecha(fecha);
				ingreso=ft;
			}else {	
			   ...
			   CuentaBancaria ctaBcria = new CuentaBancaria(
					id_cuenta_bcria_destino_deposito,ctaBcriaDesc);
			   ctaBcria.setBanco(new Banco(rs.getInt("fi__id_banco_destino_deposito"), ""));
			   ...
			   if (entidad == WebKeysGlobal.UOMA) {
				try {
					depo.setSucuNacion(rs.getInt("fi__sucu_bco"));
				} catch (Exception e) { }
			   }
			   ingreso = depo;
			}   
...
//			ingreso = ra;
		...
		ingreso.setNroRecibo(rs.getString("nro_factura"));
		//ingreso.setNroRecibo(rs.getString("nro_recibo"));
		ingreso.setFechaRecibo(rs.getDate("fecha_factura"));
//ingreso.setFechaRecibo(rs.getDate("fecha_recibo"));
		FacturaIngreso reciboIngreso = new FacturaIngreso(ingreso, id);
