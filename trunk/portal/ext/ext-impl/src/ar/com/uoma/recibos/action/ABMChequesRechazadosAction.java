...
public class ABMChequesRechazadosAction extends PortletAction {
	private static Log _log = LogFactoryUtil
			.getLog(ABMChequesRechazadosAction.class);
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		if (recibo.getChequesRechazados() == null) {
			recibo.setChequesRechazados(new ArrayList<ReciboCheque>());
		}
		...
		return mapping
				.findForward("portlet.tesoreria.recibos.cheques_rechazados.result.search");
	}

	private void borrarCheque(RenderRequest renderRequest, Recibo recibo) {
		String nro = renderRequest.getParameter("cheque_nro");
		String banco = renderRequest.getParameter("id_banco");
		Cheque ch = new Cheque(new BigDecimal(nro), Integer.parseInt(banco));
		recibo.getChequesRechazados().remove(new ReciboCheque(ch));
	...
	private void agregarCheque(RenderRequest renderRequest, Recibo recibo, int entidad)
		...
		List<Cheque> cheques = ChequeServiceUtil.getChequesRechazados(cuit, entidad);
		if (cheques != null) {
			for (Cheque ch : cheques) {
				...
				if (!recibo.getChequesRechazados().contains(reciboCheque)) {
					recibo.getChequesRechazados().add(reciboCheque);
				}
			}
		}
	}
