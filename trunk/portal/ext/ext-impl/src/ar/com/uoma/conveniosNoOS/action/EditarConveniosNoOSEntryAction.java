...
public class EditarConveniosNoOSEntryAction extends PortletAction {
	...
	public void processAction(ActionMapping mapping, ActionForm form,
		...
		try {
			if (cmd.equals(Constants.ADD) || cmd.equals(Constants.UPDATE)) {
				Convenio convenio = updateConvenioEntry(actionRequest, cmd, entidad);
	...
	private Convenio updateConvenioEntry(ActionRequest actionRequest, String cmd, int entidad)
		...
		if (cmd.equals(Constants.ADD)) {
			ConvenioNoOSServiceUtil.save(convenio, user, entidad);
		} else {
			ConvenioNoOSServiceUtil.update(convenio, user, entidad);
		}
		return convenio;
	}
	...
	private Convenio getConvenioFromRequest(HttpServletRequest req,
		...
		String entidad = ParamUtil.getString(req, "entidad_con");
		...
		int idconvenio = ParamUtil.getInteger(req, "convenio_id");
	...
	private void completarDatosActasRelacionadas(HttpServletRequest req,
		...
		if (convenio.getActasRelacionadas() != null) {
			for (ActaRelacionada ar : convenio.getActasRelacionadas()) {
				...
				ar.setSaldo((!ajuste.equals("")?new BigDecimal(ajuste):BigDecimal.ZERO).add(ar.getImporte()));
			}
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		HttpSession session = PortalUtil.getHttpServletRequest(renderRequest)
				.getSession();
		Convenio convenio = getConvenioEntry(renderRequest, entidad);
		...
		return mapping.findForward(getForward(renderRequest,
				"portlet.estudio_isidro.convenios_no_os.editar_convenios_entry"));
	}

	private Convenio getConvenioEntry(RenderRequest renderRequest, int entidad) {
		Convenio conv = null;
		...
		if (idString != null && !idString.trim().equals("")) {
			...
			if (id > 0) {
				conv = ConvenioNoOSServiceUtil.getConvenio(id,0,entidad);
			}
		}
		return conv;
	}
