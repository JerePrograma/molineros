...
public class AvisoResumenSinCerrar extends AgendadoJava implements Serializable {
	...
	private static Log logger = LogFactoryUtil.getLog(AvisoResumenSinCerrar.class);
	...
	public void correrAgendado(ReporteAutomatico ra) {
		...
		try {
			alertasCRM = ReportesServiceUtil.getResumenContactosCRMsinCerrar(ra);
			int i = 0;
			String mensajeHTML = "";
			...
			while (i < alertasCRM.size()) {
				...
				destinatario = contacto.getDerivacion().getUsuario();
				...
				mensajeHTML += "<th style='border: 1px solid black;'>De</th>";
				mensajeHTML += "<th style='border: 1px solid black;'>Importancia</th>";
				mensajeHTML += "<th style='border: 1px solid black;'>Contenido</th>";
				mensajeHTML += "</tr>";
				
				while(i < alertasCRM.size() && destinatario.equalsIgnoreCase(alertasCRM.get(i).getDerivacion().getUsuario())) {
					contacto = alertasCRM.get(i);
					destinatario = contacto.getDerivacion().getUsuario();
					...
					mensajeHTML += "<td style='border: 1px solid black;'>" + contacto.getAltaUsr() + "</td>";
					mensajeHTML += "<td style='border: 1px solid black;'>" + (contacto.getImportancia()==1?"Importante":"Normal") + "</td>";
					mensajeHTML += "<td style='border: 1px solid black;'  width: 60%;>" + contacto.getDescripcion() + "</td>";
					mensajeHTML += "</tr>";
...
//				
				...
				for(String s:emailsNotificaAlerta) {
				   ...
				   MailUtils.enviarMailGmailSinAdjHTML(rac.getMailFrom(), 
						rac.getPass(), 
						lm, 
						"Resumen de Contactos CRM pendientes",
						bodyDerivado, 
						1);
				}   
			}
