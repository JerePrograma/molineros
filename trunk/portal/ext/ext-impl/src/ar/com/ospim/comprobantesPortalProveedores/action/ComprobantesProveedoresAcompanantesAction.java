...
public class ComprobantesProveedoresAcompanantesAction extends PortletAction {
...
//		PreAutorizacion preautorizacion=null;
		...
		ComprobanteAcompanante comprobante =null;
		...
		if (!StringUtils.checkEmpty(cmd)) {
			...
			if(cmd.equals("buscar")){
		           ...
		           return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobantes_search_result_acompanantes"));		
			...
			if(cmd.equals(Constants.EDIT) ){
				 ...
				 c.setPtoVenta(Integer.parseInt(ptovta));
				 c.setNroComprobante(nro);
				 List<ComprobanteAcompanante> lista = ComprobanteServiceUtil.getListaAcompanantes(c,0);
				 session.setAttribute(WebKeysComprobantes.COMPROBANTE_ACOMPANANTE_EN_EDICION, lista.get(0));
				 return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobante_acompanante_editar"));
			}
           
			if(cmd.equals(Constants.UPDATE) ){
				
				comprobante=(ComprobanteAcompanante)  session.getAttribute(WebKeysComprobantes.COMPROBANTE_ACOMPANANTE_EN_EDICION);
			...
			if(cmd.equals("generarReclamos")){
				
		           generarReclamos(renderRequest,session,entidad,user);		   	
		...
		return mapping.findForward("portlet.comprobantes.comprobantes_proveedores_acompanantes");
...
private void filterComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
	...
	Integer reclamo=null;
	try {
	  reclamo=ParamUtil.getInteger(renderRequest, "reclamo");	
    } catch (Exception e) {}
	...
	filtro.setReclamoId(new BigDecimal(reclamo) );
	filtro.setPendientes(pendientes);
		
	List<ComprobanteAcompanante> lista = ComprobanteServiceUtil.getListaAcompanantes(filtro,0);
	
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_FILTRO_ACOMPANANTES ,filtro);
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_ACOMPANANTES,lista);
...
private void verificarComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
    ...
    List<ComprobanteAcompanante> lista = (List<ComprobanteAcompanante>) session.getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_ACOMPANANTES);
    List<ComprobanteAcompanante> listaAux=new ArrayList<ComprobanteAcompanante>();
    if(lista!=null && !lista.isEmpty()) {
      for(Integer i=0;i<lista.size();i++) {
    	if (ids.contains(lista.get(i).getId().toString().trim())) {
    	    ...
    	    listaAux=ComprobanteServiceUtil.getListaAcompanantes(filtro,0);
...
private String verificaComprobante(ComprobanteAcompanante c) throws Exception {
	String cError="";
...
/*		
	  if(c.getCud()==null ) {
	    ...
	    for(AfiDocumentacion d:documentacionList) {
		  if(d.getCodigoCUD()!=null && d.getFecha_baja()!=null && d.getFecha_ingre().before(c.getPeriodoPrestacion())
			&& d.getFecha_baja().after(c.getPeriodoPrestacion()) && c.getAfiliado().getInte()==d.getAfiliado().getInte()) {
			existeCUD=true;
			break;
		  }
	    }
...
*/	  
	...
	filtro.setAfiliado(c.getAfiliado());
	filtro.setCodigoPrestacion(c.getCodigoPrestacion());
	filtro.setPeriodoPrestacion(c.getPeriodoPrestacion());
	
    List<AutorizacionPrestacional> list = ComprobanteServiceUtil.getListaAutorizacionesPrestacionales(filtro);
	if(list==null || list.isEmpty()) {
		if(cError.length()>0) cError+=";";
		cError+="AUT";	
	}
	
	if(c.getCantidadAreaMedica()==null || c.getCantidadAreaMedica()== 0D ){
		if(cError.length()>0) cError+=";";
		cError+="ARM";
	}else if(c.getImporteAreaMedica()==null || c.getImporteAreaMedica()== 0D ){
		if(cError.length()>0) cError+=";";
		cError+="ARM";
	}
...
private void actualizaComprobante(ComprobanteAcompanante comprobante,HttpServletRequest renderRequest) throws SystemException{
	...
	//Area Médica
	String cantidadAM=renderRequest.getParameter("cantidadAM");
	if(cantidadAM.contains(",")) {
		   cantidadAM=cantidadAM.replace(".", "").replace(",", ".");
	}
	comprobante.setCantidadAreaMedica(Double.valueOf(cantidadAM));
	
	String importeAM=renderRequest.getParameter("importeAM");
	if(importeAM.contains(",")) {
		   importeAM=importeAM.replace(".", "").replace(",", ".");
	}
	comprobante.setImporteAreaMedica(Double.valueOf(importeAM));
	
	String cargoOspim=renderRequest.getParameter("cargoospim");
	if(cargoOspim.contains(",")) {
		   cargoOspim=cargoOspim.replace(".", "").replace(",", ".");
	}
	comprobante.setCargoOspim(Double.valueOf(cargoOspim));
	
	String cargoTercerizadora=renderRequest.getParameter("cargops");
	if(cargoTercerizadora.contains(",")) {
		cargoTercerizadora=cargoTercerizadora.replace(".", "").replace(",", ".");
	}
	comprobante.setCargoTercerizadora(Double.valueOf(cargoTercerizadora));
	
	String cargoTercerizadoraMono=renderRequest.getParameter("cargoimesa");
	if(cargoTercerizadoraMono.contains(",")) {
		cargoTercerizadoraMono=cargoTercerizadoraMono.replace(".", "").replace(",", ".");
	}
	comprobante.setCargoTercerizadoraMonotributistas(Double.valueOf(cargoTercerizadoraMono));
	
	String reconocido=renderRequest.getParameter("reconocidoSSS");
	if(reconocido.contains(",")) {
		reconocido=reconocido.replace(".", "").replace(",", ".");
	}
	comprobante.setReconocidoSSS(Double.valueOf(reconocido));
	
	String recuperable=renderRequest.getParameter("recuperable_sur");
	if(recuperable.contains(",")) {
		recuperable=recuperable.replace(".", "").replace(",", ".");
	}
	comprobante.setRecuperable(Integer.valueOf(recuperable));
	
	String observacionesPrestacion= renderRequest.getParameter("observacion_prestacion");
	comprobante.setObservacionesPrestacion(observacionesPrestacion);
	
}


private long updateComprobante(ComprobanteAcompanante comprobante, String user) throws Exception{
	long id = 0;
	
	id = ComprobanteServiceUtil.updateComprobanteAcompanante(comprobante, user);
	return id;
}
	


private void generarReclamos(RenderRequest renderRequest,HttpSession session,String entidad,User user) throws Exception{
	String ids = ParamUtil.getString(renderRequest, "ids");
    ...
    if(lista!=null && !lista.isEmpty()) {
      for(Integer i=0;i<lista.size();i++) {
    	...
    	if (ids.contains(lista.get(i).getId().toString().trim())) {
    	    ...
    	    ComprobanteAcompanante cbte =lista.get(i);
    	    
    		verificaComprobante(cbte);
/*    		
    		if("OK".equalsIgnoreCase(cbte.getError()) &&
    				(cbte.getReclamoId()==null || cbte.getReclamoId().intValue()==0
    				) ) {
    			
    			verificaComprobanteAreaMedica(cbte);
*/
//generar RP    
    			if("OK".equalsIgnoreCase(cbte.getError()) &&
        				(cbte.getReclamoId()==null || cbte.getReclamoId().intValue()==0
    			  ...
    			  String responsable = TraeListasServiceUtil.getSystemConfig("REVISION_RESPONSABLE_ACOMPANANTE");
    			  String justificacion = TraeListasServiceUtil.getSystemConfig("JUSTIFICACION_ACOMPANANTE");
    			  ReclamoPrestacional reclamo =new ReclamoPrestacional();
    			  reclamo.setTipoPedido("EXCEPCION");
    			  reclamo.setEstado(0);//Precarga
    			  reclamo.setSector("DISCAPACIDAD");
    			  reclamo.setAfiliado(cbte.getAfiliado());
    			  reclamo.setCodigoIntegracion(6); // No recuperable
    			  ...
    			  Nomenclador n= ComprobanteServiceUtil.buscaNomencladorSSSByCodigo(cbte.getCodigoPrestacion());
    			  if(n!=null && n.getIdPrestacionOSPIM()!=null && n.getIdPrestacionOSPIM()>0) {
    				prestacion.setId_prestacion(n.getIdPrestacionOSPIM());
    			  }
    			  prestacion.setFrecuencia("UNICA");
    			  prestacion.setFechaPrestacion(cbte.getPeriodoPrestacion());
    			  prestacion.setComprobanteTipo(cbte.getTipoComprobante());
    			  prestacion.setComprobanteLetra(cbte.getLetraComprobante());
    			  prestacion.setComprobanteSucursal(String.format("%05d",cbte.getPtoVenta()));
    			  prestacion.setComprobanteNro(cbte.getNroComprobante());
    			  prestacion.setComprobanteCUIT(cbte.getAcreedorEmpresa().getCuit());
    			  prestacion.setComprobanteCUITSucursal("000");
    			  prestacion.setComprobanteFecha(cbte.getFechaEmision());
    			
    			  prestacion.setComprobanteCantidad(Double.valueOf(cbte.getCantidad()));
    			  prestacion.setComprobanteImporte( cbte.getImporteComprobante().doubleValue()/Double.valueOf(cbte.getCantidad()));
    			  prestacion.setComprobanteTotal(cbte.getImporteComprobante().doubleValue());
    			
    			  //Area Medica
    			  prestacion.setRecuperable(cbte.getRecuperable());
    			  prestacion.setReconocidoSSS(cbte.getReconocidoSSS());
    			  prestacion.setCargo_ospim(cbte.getCargoOspim());
    			  prestacion.setCargo_ps(cbte.getCargoTercerizadora());
    			  prestacion.setCargo_imesa(cbte.getCargoTercerizadoraMonotributistas());
    			  prestacion.setObservaciones(cbte.getObservacionesPrestacion());
    			  prestacion.setCantidad(cbte.getCantidadAreaMedica());
    			  prestacion.setImporte(cbte.getImporteAreaMedica());
    			  reclamo.getPrestaciones().add(prestacion);
    			  //Fin Area Médica
    			  
    			  //Revision
    			  RevisionesReclamo revision = new RevisionesReclamo();
    			  revision.setFecha_revision(new Date());
    			  revision.setUsr_presente(responsable);
    			  revision.setUsr_resolucion("AUTORIZADO");
    			  revision.setUsr_responsable_resolucion(responsable);
    			  revision.setObservacion(justificacion);
                  reclamo.setRevisiones(new ArrayList<RevisionesReclamo>());
                  reclamo.getRevisiones().add(revision);
    			  //Fin Revision
                  
                  reclamo.setJustificaconMedica(justificacion); 
                  reclamo.setDictamenComision(justificacion); 
                  
    			  Integer idReclamo = ReclamosPrestacionesServiceUtil.insertar(reclamo , user );
    			  cbte.setReclamoId(new BigDecimal(idReclamo));
    			  cbte.setOrden(idReclamo);
// Fin generar RP    			
    			}	
//    		}
    	}
  ...
  private String verificaComprobanteAreaMedica(ComprobanteAcompanante c) throws Exception {
	String cError="";
	if(c.getCantidadAreaMedica()==null || c.getCantidadAreaMedica()== 0D ){
		if(cError.length()>0) cError+=";";
		cError+="ARM";
...
//	    		lista.get(i).setOrden(999999);
	      ...
	      String idFacturaImg="";
	      for(ComprobanteAcompanante comprobante:lista) {
