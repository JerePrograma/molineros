...
//			nroCredenPrev = ParamUtil.getInteger(renderRequest, "nroCredencialPrevencion", 0);
			...
			String prestacion_fecha = ParamUtil.getString(renderRequest,
			...
			if (prestacionFecha == null) {
				...
				if (pagReintegroConReclamo ==1){
					busqueda = BusquedaAfiliadoServiceUtil.getBusquedaAfiliadosComponenteReintegro(cuil, inte, tipoDoc,
									nroDoc, seccional_int, apellido, nombre,
									entidad, nroAfiliado, nroSocioPrev, nroCredenPrev);
					
				} else {
					busqueda = BusquedaAfiliadoServiceUtil
							.getBusquedaAfiliadosComponente(cuil, inte, tipoDoc,
									nroDoc, seccional_int, apellido, nombre,
									entidad, nroAfiliado, nroSocioPrev, nroCredenPrev);					
				}
				
				renderRequest.removeAttribute(WebKeysAfiliados.BUSQUEDA_AFILIADO);
				renderRequest.setAttribute(WebKeysAfiliados.BUSQUEDA_AFILIADO,
						busqueda);
			} else {
				if (pagReintegroConReclamo ==1){
					busqueda = BusquedaAfiliadoServiceUtil
							.getBusquedaAfiliadosComponenteReintegro(cuil, inte, tipoDoc,
									nroDoc, seccional_int, apellido, nombre,	
