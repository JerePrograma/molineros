       ...
       if(StringUtils.checkEmpty(cmd)){ 			
		...
		if(!StringUtils.checkEmpty(cmd)){
			            ...
			            if (idExterno != null && idExterno > 0) {
			                ...
			                if (token != null) {
			                    try {
			                        ...
			                        ClienteAppMobile.actualizarEstadoReintegro(idExterno, "RE", token);
			                    } catch (Exception e) {
			...
			if(cmd.equals(WebKeysAutorizaciones.CUENTA)){
				...
				ReclamoPrestacional reclamoPrestacionalEdit =getReclamoPrestacionalFromRequest(PortalUtil.getHttpServletRequest(renderRequest), reclamoPrestacional , null,cmd );					 
				
				//Si  es pago a Seccional 
				if(!"2".equalsIgnoreCase(reclamoPrestacionalEdit.getCuenta().getCmbTitular())){					
					if (reclamoPrestacionalEdit.getCuenta().getImagenCBU() == null || StringUtils.checkEmpty(reclamoPrestacionalEdit.getCuenta().getImagenCBU())) {
						CuentaDocumentoHelper.getImagenNombre(reclamoPrestacionalEdit);
					}
				}
				
							
				ReclamosPrestacionesServiceUtil.altaModiCuenta(reclamoPrestacionalEdit , user);
			...
			if(cmd.equals(WebKeysAutorizaciones.CUENTA_SELECT)){
		        ...
		        AfiCuentaBancaria afiCuenta = ReclamosPrestacionesServiceUtil.traerCuentaBancariaAsociadasPorId(idCuenta);
				
		        ReclamoPrestacionalCuenta cuentaSelect =  CuentaDocumentoHelper.getCuenta(afiCuenta);
				...
				session.removeAttribute("ID_CUENTA_BANCARIA_SELECCIONADA");
			    session.setAttribute("ID_CUENTA_BANCARIA_SELECCIONADA", idCuenta);
