package ar.com.ospim.customtags;
...
/**
 * <a href="AttributeTabsTag.java.html"><b><i>View Source</i></b></a>
 * 
 * @author Martin Moreyra
 * 
 */
public class AttributeTabsTag extends ParamAndPropertyAncestorTagImpl {
	...
	public int doStartTag() throws JspException {
		try {
			...
			String att = (String) request.getAttribute(_param);
			if (StringUtils.isBlank(att)
					|| (att != null && att.trim().equals("null"))) {
				att = ParamUtil.getString(request, _param);
				if (att == null || att.trim().equals("")) {
					att = (String) request.getSession().getAttribute(_param);
				}
			}
			if (att == null) {
				att = _tabsValues[0];
			...
			if (att != null) {
				request.getSession().setAttribute(_param, att);
			}
			...
			request.setAttribute("liferay-ui:tabs:refresh",
					String.valueOf(_refresh));
			request.setAttribute("liferay-ui:tabs:onClick",
					String.valueOf(_onClick));
	...
	public boolean getSectionSelected() {
		if ((_names.length == 0)
				|| ((_names.length > _namesPos) && (_names[_namesPos]
						.equals(_value)))) {
