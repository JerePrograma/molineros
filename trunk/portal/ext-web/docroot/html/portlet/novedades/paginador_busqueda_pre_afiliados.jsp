...
try{
 	total_reg= (Integer)session.getAttribute(WebKeysAfiliados.FILTRO_BUSQUEDA_PREAFILIADOS_TOTAL_REGISTROS); // "total_registros"
	offset_reg= (Integer)session.getAttribute(WebKeysAfiliados.FILTRO_BUSQUEDA_PREAFILIADOS_OFFSET_REG);     // "offset_reg"  

	/*Este ajuste es para mantener la busqueda de pre afiliado */
	if(total_reg == null && offset_reg == null){
		BusquedaPreAfiliadosFiltro f = (BusquedaPreAfiliadosFiltro) session.getAttribute(WebKeysAfiliados.FILTRO_BUSQUEDA_PREAFILIADOS);
...
<table class="lfr-table">
	...
	<tr>
		...
		<td><select name="<portlet:namespace/>pagina_sel" id="<portlet:namespace/>pagina_sel" 
				onchange="javascript:<portlet:namespace />buscarPreCargaAfiliados();" >
