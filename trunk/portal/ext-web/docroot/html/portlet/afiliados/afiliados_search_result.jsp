...
<p>
	...
	<liferay-ui:success key="coberturaMedicaOk"  message="<%=(String)request.getAttribute(\"msgCoberturaMedicaOk\")  %>"  />
					...
					SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
				 	...
				 	for (int i = 0; i < afiliadosList.size(); i++) {
		 				...
		 				if(!"CAI".equalsIgnoreCase(portlet_name)){
						   ...
						   row.addText(afiliado.getUltimo_plan().getDescripcion()!=null?afiliado.getUltimo_plan().getDescripcion():"", rowURL);
						
						  /*  row.addText(afiliado.getIngre_fechaAsString(),rowURL); */
						   ...
						   if(afiliado.getSuspencionCobertura()!=null 
								   && (afiliado.getSuspencionCobertura().get(0).getVigenHasta()==null || (Validator.isNotNull(afiliado.getSuspencionCobertura().get(0).getVigenHasta()) 
											&& afiliado.getSuspencionCobertura().get(0).getVigenHasta().getTime() >= DateUtils.getMismoDia_00_00hs(new Date()).getTime() )) ) {
 							   String cobertura = " <img height='16'  width='16' src='/html/themes/classic/images/common/close.png' title='Suspendida la cobertura médica desde el: "+sdf.format(afiliado.getSuspencionCobertura().get(0).getVigenDesde())+"'/>";
							   ...
							   row.addText("<img height='16'  width='16' src='/html/themes/classic/images/common/checked.png' title='Cobertura médica vigente' />",rowURL);
						   }else{
							   row.addText("",rowURL);
						   }
		 				}else{
							   ...
							   row.addText(afiliado.getUltimo_plan().getDescripcion()!=null?afiliado.getUltimo_plan().getDescripcion():"");
						...
						/* if(showABMCrm) {
							...
							sb.append("&nbsp;<img height='16'  width='16' alt=\"<liferay-ui:message key='alta-contacto'/>\" src=\"");
			 				sb.append(themeDisplay.getPathThemeImages());
			 				sb.append("/common/telephone.png\" onClick=\"javascript:nuevoCrmContacto('");
			 				sb.append(afiliado.getCuil_titular());
			 				sb.append("','");
			 				sb.append(afiliado.getInteAsString());
			 				sb.append("');\" />");
