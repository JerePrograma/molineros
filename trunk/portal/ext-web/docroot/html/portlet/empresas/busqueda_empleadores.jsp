...
<%
 		boolean showABMButtons = PermissionUtil.userContainsRole(user,WebKeysTesoreria.ROL_ABM_EMPRESA);
%>
		<fieldset class="block-labels">
				...
				<table class="lfr-table">
					<tr>
						...
						<td><input id="<portlet:namespace />descripcion" name="<portlet:namespace />descripcion" size="50" maxlength="60" type="text" value="" /></td>
		...
		<form action="" id="<portlet:namespace />empresas" name="<portlet:namespace />empresas">	
...
<script type="text/javascript">
	...
	jQuery('#<portlet:namespace />buscar').click(function(){
		...
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/empresas/buscar_padron_entidad&cuit_entidad='+cuit+
	...
	function <portlet:namespace />altaEmpleador() {
		...
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>"/>&struts_action=/empresas/editar_empleadores_entry';		
		document.<portlet:namespace />empresas.method = 'post';
		submitForm(document.<portlet:namespace />empresas, url);
	}
</script>
