...
public class BuscarComprobantesLiquidadosSeguimientoSurAction extends PortletAction {
	private static Log _log = LogFactoryUtil
			.getLog(BuscarComprobantesLiquidadosSeguimientoSurAction.class);
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Integer ptoVta = ParamUtil.getInteger(renderRequest,"pto_venta");
			String tipo = ParamUtil
					.getString(renderRequest, "tipo_comprobante", null);
			Integer nro = ParamUtil
					.getInteger(renderRequest, "nro_comprobante", 0);
			...
			String letra = ParamUtil
					.getString(renderRequest, "letra", null);

			PortletSession portletSession = renderRequest.getPortletSession();

			List<SeguimientoSurComprobante>busqueda = SeguimientoSurServiceUtil.recuperaComprobantesLiquidados(idPrestador, cuit, razonSocial, tipo, letra, 
					ptoVta, nroS, fechaEmision, fechaRecibido, fechaVencimiento);
			...
			for(SeguimientoSurComprobante c:seguimiento.getComprobantes()){
				...
				for(SeguimientoSurComprobante cb:busqueda){
					if(cb.getCuit().equalsIgnoreCase(c.getCuit()) &&
					   cb.getLetraComprobante().equalsIgnoreCase(c.getLetraComprobante()) &&
					   cb.getTipoComprobante().equalsIgnoreCase(c.getTipoComprobante()) &&
					   cb.getPtoVenta() == c.getPtoVenta() &&
					   cb.getSucuComprobante() == c.getSucuComprobante() &&
					   cb.getNroComprobante().equalsIgnoreCase(c.getNroComprobante())
					  ){
					   existe=true;
					   break;
					}
			...
			renderRequest
					.removeAttribute(WebKeysAutorizaciones.BUSQUEDA_COMPROBANTES_SEGUIMIENTOSUR);
			renderRequest.setAttribute(
					WebKeysAutorizaciones.BUSQUEDA_COMPROBANTES_SEGUIMIENTOSUR,
					busqueda);
		...
		return mapping
				.findForward("portlet.autorizaciones.comprobantesliquidados.popup.result.search");
