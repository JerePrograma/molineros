...
public class AutorizacionPrestacional implements Serializable {
    ...
    private Integer idPreautorizacion; 
    private boolean mailPrestadorEnviado;
    private boolean lecheMaternizada;
    private boolean conDependencia;
    private Integer totalRegistros;
    private String observacionesInternas;
    private Integer copago;
    
	public AutorizacionPrestacional() {
	}

	public AutorizacionPrestacional(String cuil_titular, int inte,
	...
	public void setId(int idTratamiento) {
		id = idTratamiento;
	}
	...
	public static AutorizacionPrestacional getMapping(ResultSet rs, String prefix)
			throws SQLException {
		AutorizacionPrestacional ap = new AutorizacionPrestacional();
		...
		String seccDesc="";
		try {
			seccDesc=rs.getString(prefix + "descripcion_seccional");
		} catch (Exception e) {}
		Empresa emp = new Empresa(cuitAcreedor, sucuAcreedor, null);
		emp.setRazon_soc(razonSocial);
		ap.setAcreedor(emp);
		ap.setSeccional(new Seccional(seccional, seccDesc));
		try {
			ap.setIdPreautorizacion(rs.getInt(prefix + "pre_autorizacion"));
		} catch (Exception e) {
		}
		
		try {
			ap.setMailPrestadorEnviado(rs.getBoolean(prefix + "mail_prestador_enviado"));
		} catch (Exception e) {
		}
		...
		try{
			ap.setNroAutorizacion(rs.getInt(prefix+"nro_autorizacion"));
		    ap.setDiscapacitado(rs.getBoolean(prefix+"discapacitado"));
		    ap.setMotivoExcepcion(rs.getString(prefix+"motivo_excepcion") );
		    ap.setMotivoEstado(rs.getString(prefix+"motivo_estado") );
		    ap.setLecheMaternizada(rs.getBoolean(prefix+"leches"));
		    ap.setConDependencia(rs.getBoolean(prefix+"dependencia"));
		}catch(Exception e){
			ap.setNroAutorizacion(0);
			ap.setDiscapacitado(false);
			ap.setMotivoExcepcion(null);
			ap.setMotivoEstado(null);
		}
		
		
		try {
			ap.setObservacionesInternas(rs.getString(prefix + "observaciones_internas"));
		}catch(Exception e){}
		
		
		try{
			ap.setCopago(rs.getInt(prefix+"copago"));
		}catch(Exception e){
			ap.setCopago(0);
		}	
	...
	public Integer getIdPreautorizacion() {
		return idPreautorizacion;
	}

	public void setIdPreautorizacion(Integer idPreautorizacion) {
		this.idPreautorizacion = idPreautorizacion;
	}

	public boolean isMailPrestadorEnviado() {
		return mailPrestadorEnviado;
	}

	public void setMailPrestadorEnviado(boolean mailPrestadorEnviado) {
		this.mailPrestadorEnviado = mailPrestadorEnviado;
	}

	public boolean isLecheMaternizada() {
		return lecheMaternizada;
	}

	public void setLecheMaternizada(boolean lecheMaternizada) {
		this.lecheMaternizada = lecheMaternizada;
	}
	...
	public boolean isConDependencia() {
		return conDependencia;
	}

	public void setConDependencia(boolean conDependencia) {
