...
and a.alta_fecha>'20110101'--(a.alta_usr ='agerbasi' or a.alta_usr = 'ezambello') 
...
and not exists (select 1 from liquidacion_actas la where la.cuit=a.cuit and la.cuil=ape.cuil and la.periodo_d=ape.periodo)
...
select rc.acta_id, rcp.recibo_concepto_id, rcp.recibo_ingreso_id, trunc(rcp.pendiente_derivar,2) as pendiente_derivar
...
and rcb.fecha>'20110101'
and rcb.baja_fecha is null
and exists (select 1 from periodos_a_pagar pap where pap.id=rc.acta_id);
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION derivar_actas_convenios() OWNER TO postgres;
