CREATE OR REPLACE FUNCTION split_cadena(cadena character varying, separador character varying)
...
cadena_aux=cadena;
create temp table aux (valor integer);
if posicion_final<>0  or length(cadena_aux)<>0 then
	LOOP
		...
		if length(cadena_aux)>0 and posicion_final>0 then			
			insert into aux values (cast(substring(cadena_aux from posicion_inicial for posicion_final-1) as integer));	
			cadena_aux=substring(cadena_aux,posicion_final+1,length(cadena_aux)-(posicion_final-1));	
		else if posicion_final<0 then
			insert into aux values (cast(substring(cadena_aux from posicion_inicial for length(cadena_aux)) as integer));	
			cadena_aux='0';	
		end if;
		end if;
