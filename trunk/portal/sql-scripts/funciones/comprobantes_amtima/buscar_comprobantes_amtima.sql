drop function buscar_comprobantes_amtima(IN p_pto_vta numeric, IN p_tipo character varying, IN p_nro character varying, 
...
CREATE OR REPLACE FUNCTION buscar_comprobantes_amtima(IN p_pto_vta numeric, IN p_tipo character varying, IN p_nro character varying, 
IN p_cuit character varying, IN p_letra character varying, IN p_sucu integer, IN p_fecha_emision date, IN p_fecha_recepcion date,
IN p_periodo date, IN p_cuit_acreedor character varying, IN p_sucu_acreedor character varying, IN id_seccional integer)
  RETURNS TABLE(
...
select     c.fecha_emision,
  ...
  case  when exists (
	select 1 from comprobante_orden_pago_amtima copo
  ...
  case when exists (select 1 from comprobante_orden_pago_amtima copo
	inner join orden_pago_amtima opo
	on copo.id_orden_pago_amtima = opo.id_orden_pago
	where c.id_punto_venta =  copo.id_punto_venta
	and c.compro_tipo =copo.compro_tipo
	...
	and c.compro_nro =copo.compro_nro
	and c.cuit = copo.cuit
	and opo.baja_fecha is not null) then true else false end as op_existente,
   c.anulado_fecha,
   c.anulado_usr
from  comprobante_amtima c
where ( $1 is null or $1 = c.id_punto_venta)
   ...
   and ( $9 is null or $9 = periodo_prestacion)
   and ( $10 is null or $10 = cuit_acreedor)
   and ( $11 is null or $11 = sucu_acreedor)
   and ( $12 is null or $12 = seccional)
