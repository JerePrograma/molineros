CREATE OR REPLACE FUNCTION buscar_reporte_reintegros_farmacia(IN id_reporte integer)
  ...
  ad_localidad integer, ad_numero character varying, rp_id_reintegro integer, rp_fecha timestamp without time zone, 
  rp_nro_receta integer, rp_fecha_receta date, rp_profesional character varying, rp_cantidad integer, 
  rp_troquel integer, rp_cober_sss numeric, rp_cober_amtima numeric, rp_cober_ospim numeric, rp_monto_ospim numeric, 
  rp_monto_amtima numeric, rp_precio_al_publico numeric, rp_precio_ospim numeric, rp_total_med numeric, 
  rp_total_cobertura numeric, rp_total numeric, rp_alta_fecha timestamp without time zone, rp_alta_usr character varying, 
  rp_modi_fecha timestamp without time zone, rp_modi_usr character varying, rp_baja_fecha timestamp without time zone, 
  rp_baja_usr character varying, rp_id_medicamento integer, rp_id integer, m_troquel numeric, m_nombre character varying, 
  m_presentacion character varying, m_monto_ioma numeric, m_norma_ioma character, m_cober_ioma character, 
  m_laboratorio character varying, m_precio numeric, m_fecha timestamp without time zone, m_controlado character, 
  m_importado character, m_tipo_venta character, m_iva character, m_cod_dto_pami character, m_cod_lab integer, 
  m_nro_registro numeric, m_baja character, m_cod_barra character varying, m_unidades integer, m_tamanio character, 
  m_heladera character, m_sifar character, m_baja_especial character, m_accion character varying, m_droga character varying, 
  m_id_medicamento integer, m_porc_ospim numeric, m_porc_amtima numeric, m_porc_sssalud numeric, m_pmoe_n numeric, 
  suma_por_afiliado numeric, suma_total numeric) AS
$BODY$
...
select oppr.id_lista_reintegro_farmacia_reporte, r.id_reintegro, cuil_titular, inte, rp.id_medicamento, sum(rp.cantidad*(rp.monto_ospim+rp.monto_amtima)) as total
from  lista_reintegro_farmacia_reporte_detalle oppr, reintegro_farmacia r, medicamento_reintegro_farmacia rp

where
oppr.id_lista_reintegro_farmacia_reporte = id_reporte
and oppr.id_reintegro = r.id_reintegro
and r.id_reintegro = rp.id_reintegro
group by oppr.id_lista_reintegro_farmacia_reporte, r.id_reintegro, r.cuil_titular, r.inte, rp.id_medicamento;
...
--medicamento
 ...
 v.pmoe_n as ocho,	
	...
	from lista_reintegro_farmacia_reporte_detalle opor
	inner join 
	...
	inner join 
	seccional s on a.id_seccional = s.id_seccional
	inner join lista_reintegro_farmacia_reporte lrp
	on opor.id_lista_reintegro_farmacia_reporte = lrp.id
	...
	where opor.id_lista_reintegro_farmacia_reporte = id_reporte;
END;  
	$BODY$
