CREATE OR REPLACE FUNCTION reporte_sss_331_cantidad_afiliados_pr_anexo(fecha_ini_p date, fecha_fin_p date)
...
group by per_codigo, lfa.id_ospim, lfa.inte, lpad(cast(cast(mm.pat_cobertura as double precision) as varchar),3,'0'), 
       lpad(cast(isNull(pr.id_sssalud,0) as varchar),2,'0'); --PROVINCIA,; 
