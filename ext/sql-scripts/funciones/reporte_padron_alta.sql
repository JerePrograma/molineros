...
create temp table aux_alta AS
select cuil_titular 
from afi_aportes 
where fecha_ingre>=p_fecha_alta_desde 
and fecha_ingre<=p_fecha_alta_hasta
and (fecha_egre is null or fecha_ingre<>fecha_egre )
...
if p_aporte_id is not null then
	...
	where not exists (select 1 from afi_aportes apo
			...
			and apo.inte=0
			and fecha_ingre>=p_fecha_alta_desde 
			and fecha_ingre<=p_fecha_alta_hasta
...
and aa.fecha_ingre=(select max (fecha_ingre)
		   ...
		   and (fecha_egre is null or (fecha_egre>=p_fecha_alta_desde ))
		   and (baja_fecha is null or (baja_fecha>=p_fecha_alta_desde ))
