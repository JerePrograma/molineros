-- Function: inserta_recibo_no_os_ingreso(integer, numeric, integer, character varying, numeric, date, integer, character varying, integer, integer, integer)

-- DROP FUNCTION inserta_recibo_no_os_ingreso(integer, numeric, integer, character varying, numeric, date, integer, character varying, integer, integer, integer);

CREATE OR REPLACE FUNCTION inserta_recibo_no_os_ingreso(p_recibo_id integer, p_nro_cheque numeric, p_id_banco integer, p_numero_deposito character varying, p_importe numeric, p_fecha date, p_id_estado_efectivo integer, p_user character varying, p_id_cuenta_bcria_destino_deposito integer, p_id_recibo_ingreso_tipo_deposito integer, p_id_anticipo_recibo_concepto integer)
  RETURNS integer AS
$BODY$
declare resultDom integer;
BEGIN
	
INSERT INTO recibo_no_os_ingresos(
...
return currval('recibo_no_os_ingresos_id_seq');
END;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION inserta_recibo_no_os_ingreso(integer, numeric, integer, character varying, numeric, date, integer, character varying, integer, integer, integer)
  OWNER TO postgres;
