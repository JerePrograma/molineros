...
public class ConvenioServiceImpl {
	private static Log _log = LogFactoryUtil.getLog(ConvenioServiceImpl.class);

	private static ConvenioServiceImpl instance = null;

	public static ConvenioServiceImpl getInstance() {
		if (null == instance) {
			instance = new ConvenioServiceImpl();
		}
		return instance;
	}
	...
	public List<Convenio> getConvenios(String convenioNro, String cuit,
		...
		try {
			String sql = "{call buscar_convenios(?, ?, ?)}";
	...
	public Convenio get(int id) {
		...
		CallableStatement stmt = null;
		try {
			String sql = "{call buscar_convenio(?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, id);
	...
	public List<ConvenioPago> getPagosConvenios(int id) {
		Connection con = null;
		CallableStatement stmt = null;
		List<ConvenioPago> pagos = null;
		try {
			String sql = "{call buscar_convenio_pagos(?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, id);
	...
	public void borrar(int id, String usr)
			throws ImposibleBorrarConvenioException, SQLException {
		Connection con = null;
		CallableStatement stmt = null;
		try {
			String sql = "{call borra_convenio(?, ?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, id);
	...
	public int save(Convenio convenio, String screenName, Connection connectionParameter)
		...
		try {
			...
			String sql = "{call inserta_convenio (?,?,?,?,?,?,?,?,?,?,?,?)}";
			stmt = con.prepareCall(sql.toString());
			...
			if (convenio.getEstadoSeguimiento() != null) {
				stmt.setInt(11, convenio.getEstadoSeguimiento().getId());
			} else {
	...
	public void update(Convenio convenio, String screenName)
			throws SystemException, DuplicateConvenioIdException {
		Connection con = null;
		CallableStatement stmt = null;
		try {
			con = ConnectionHelper.getConnection();
			String sql = "{call actualiza_convenio (?,?,?,?,?,?,?,?,?,?,?,?)}";
			stmt = con.prepareCall(sql.toString());
	...
	public void save(Convenio convenio, ConvenioPago cp, String screenName, Connection connectionParameter)
		...
		try {
			...
			String sql = "{call inserta_convenio_pagos(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}";
			...
			stmt.setBigDecimal(7, cp.getCheque() != null ? cp.getCheque()
					.getNumero() : null);
			...
			_log.error("Error al salvar pago convenio", e);
			throw e;
		} finally {
			if(connectionParameter==null){
				ConnectionHelper.cerrar(stmt, con);
	...
	public void update(Convenio convenio, ConvenioPago cp, String screenName)
			throws SQLException {
		Connection con = null;
		CallableStatement stmt = null;
		try {
			String sql = "{call update_convenio_pagos(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, convenio.getId());
			stmt.setString(2, cp.getTipo().getMapping());
	...
	public List<ActaRelacionada> getActasRelacionadas(int id) {
		...
		try {
			String sql = "{call buscar_convenio_actas_relacionadas(?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, id);
			ResultSet rs = stmt.executeQuery();
	...
	public void saveActaRelacionada(ActaRelacionada actaRel, String screenName, Connection connectionParameter)
		...
		try {
			...
			String sql = "{call inserta_convenio_acta_relacionada (?, ?, ?, ?, ?)}";
	...
	public List<Convenio> getConveniosSinRecibos(String cuit) {
		_log.debug("buscando convenios");
		...
		List<Convenio> convenios = null;
		try {
			String sql = "{call buscar_convenios_sin_recibos(?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
	...
	public List<ConvenioPagoIngresado> getPagosIngresados(int id, int reciboId) {
		...
		try {
			String sql = "{call buscar_convenio_pagos_ingresados(?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, id);
			...
			while (rs.next()) {
				ConvenioPagoIngresado pago=new ConvenioPagoIngresado(new Recibo(rs
						.getInt("recibo_id")), rs.getBigDecimal("importe"));
				if(reciboId>0 && pago.getRecibo().getId()==reciboId){
					pagos.add(pago);
				}else if(reciboId==0){
					pagos.add(pago);
				}
	...
	public List<ReporteConvenioBean> reporteConvenios(Date fechaIni,
		...
		try {
			...
			if(entidad==WebKeysGlobal.AMTIMA){
				sql = "{call reporte_convenios_amtima(?, ?)}";
			}else if(entidad==WebKeysGlobal.UOMA){
				sql = "{call uoma.reporte_convenios_uoma(?, ?)}";	
			}else{
				sql = "{call reporte_convenios(?, ?)}";	
			}
			...
			while (rs.next()) {
				repo.add(ReporteConvenioBean.getMapping(rs));
			}
		} catch (Exception e) {
	...
	public List<ReporteCobranzaConvenioBean> reporteCobranzaConvenios(
		...
		try {
			String sql = "{call reporte_aplicacion_cobranzas_convenios(?, ?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setDate(1, new java.sql.Date(fechaIni.getTime()));
			stmt.setDate(2, new java.sql.Date(fechaFin.getTime()));
	...
	public boolean verificarReciboConvenio(int convenio_id)
			throws SQLException {
		Connection con = null;
		CallableStatement stmt = null;
		boolean existe=true;
		try {
			String sql = "{? = call existe_recibo_convenio(?)}";
			con = ConnectionHelper.getConnection();			
			stmt = con.prepareCall(sql.toString());
	...
	public boolean actualizaEstadoSeguimientoConvenio(int convenioId, int estadoSegId, String usr)
			throws Exception {
		Connection con = null;
		CallableStatement stmt = null;
		boolean actualizoOk = false;
		try {
			String sql = "{call actualiza_estado_seguim_convenio(?,?,?)}";

			con = ConnectionHelper.getConnection();

			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, convenioId);
			...
			_log.error("Error cambio estado convenio", e);
			throw e;
		} finally {
	...
	public boolean actualizaEstadoSeguimientoConvenioNoOS(int convenioId, int estadoSegId, String usr)
			throws Exception {
		Connection con = null;
		CallableStatement stmt = null;
		boolean actualizoOk = false;
		try {
			String sql = "{call actualiza_estado_seguim_convenio_no_os(?,?,?)}";
	...
	public List<ConvenioPagosReporte> getConveniosAvisoVencimiento(Integer dias) {
		_log.debug("buscando getConveniosAvisoVencimiento");

		Connection con = null;
		CallableStatement stmt = null;
		List<ConvenioPagosReporte> convenios = null;
		try {
			String sql = "{call reporte_convenio_pagos(?)}";
			con = ConnectionHelper.getConnection();
			stmt = con.prepareCall(sql.toString());
			stmt.setInt(1, dias);
			...
			ResultSet rs = stmt.executeQuery();
			convenios = new ArrayList<ConvenioPagosReporte>();
			while (rs.next()) {
				ConvenioPagosReporte conv = ConvenioPagosReporte.getMapping(rs);
				conv.setTipoConvenio("OSPIM");
				convenios.add(conv);
			}
		} catch (Exception e) {
			_log.error("Error al traer getConveniosAvisoVencimiento", e);
		} finally {
			ConnectionHelper.cerrar(stmt, con);
		}
		_log.debug("saliendo de  getConveniosAvisoVencimiento");
		return convenios;
	}
