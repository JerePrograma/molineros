...
public class ActaPago {
	...
	public enum Forma {
		Cheque("C"), Deposito("D"), Efectivo("E"), Pagare("P");

		private String mapping;

		private Forma(String mapping) {
			this.mapping = mapping;
		}
	...
	public Acta getActaCancelatoria() {
		return actaCancelatoria;
	}

	public void setActaCancelatoria(Acta actaCancelatoria) {
		this.actaCancelatoria = actaCancelatoria;
	}
	...
	public static ActaPago getMapping(ResultSet rs) throws SQLException {
		return getMapping(rs, "");
	}

	public static ActaPago getMapping(ResultSet rs, String prefix)
			throws SQLException {
		...
		ap.setActaId(rs.getInt(prefix+"acta_id"));
		
		int actaCancelatoriaId = rs.getInt(prefix + "acta_cancelatoria_id");
		if (actaCancelatoriaId != 0) {
			ap.setActaCancelatoria(new Acta(actaCancelatoriaId));
		}
		
		int convCancelatorioId = rs.getInt(prefix + "convenio_cancalatorio_id");
		if (convCancelatorioId != 0) {
			ap.setConvenioCancelatorio(new Convenio(convCancelatorioId));
		}

		BigDecimal total = ap.getImporte().add(ap.getInteres());
		String forma = rs.getString(prefix + "forma");
		if (null!=forma && forma.equals("C")) {
			if (rs.getBigDecimal(prefix + "nro_cheque") != null) {
				Cheque cheque = new Cheque();
				cheque.setFecha(ap.getFechaPago());
				cheque.setNumero(rs.getBigDecimal(prefix + "nro_cheque"));
				cheque.setBanco(new Banco(rs.getInt(prefix + "banco_cheque"),
						""));
				cheque.setImporte(total);
				ap.setIngreso(cheque);
			}
		} else if (null!=forma && forma.equals("D")) {
			ap.setIngreso(new DepositoBancario(total, ap.getFechaPago()));
		} else if (null!=forma && forma.equals("E")) {
			Efectivo efect = new Efectivo(total);
			efect.setFecha(ap.getFechaPago());
			ap.setIngreso(efect);
		}else if(null!=forma && forma.equals("P")){
			Pagare pagare = new Pagare(total);
			pagare.setFecha(ap.getFechaPago());
			ap.setIngreso(pagare);
		}
