...
public class BuscarAfiliadosComponenteAction extends PortletAction {

	private static final Log _log = LogFactoryUtil.getLog(BuscarAfiliadosComponenteAction.class);

	public void processAction(ActionMapping mapping, ActionForm form,
							  PortletConfig portletConfig, ActionRequest actionRequest,
							  ActionResponse actionResponse) throws Exception {

		final String rid = "BACOMP-A#" + System.currentTimeMillis() + "-" + Math.abs((int) (Math.random() * 100000));
		final boolean dbg = _log.isDebugEnabled();

		if (dbg) {
			_log.debug("[" + rid + "][ACTION][START] processAction iniciado");
			_log.debug("[" + rid + "][ACTION][FORWARD] setForward -> portlet.afiliados.result.search");
		}

		setForward(actionRequest, "portlet.afiliados.result.search");

		if (dbg) {
			_log.debug("[" + rid + "][ACTION][END] processAction finalizado");
		}
	}
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		final String rid = "BACOMP-R#" + System.currentTimeMillis() + "-" + Math.abs((int) (Math.random() * 100000));
		final boolean dbg = _log.isDebugEnabled();
		...
		try {
			...
			if (null != seccional) {
				try {
					...
					if (dbg) {
						_log.debug("[" + rid + "][PARSE][SECCIONAL] Parse OK. seccional=" + seccional
								+ " -> seccional_int=" + seccional_int);
					}
				} catch (NumberFormatException e) {
					seccional_int = 0;
					_log.warn("[" + rid + "][PARSE][SECCIONAL] No se pudo parsear seccional='" + seccional
							+ "'. Se usará 0", e);
				}
			} else {
				if (dbg) {
			...
			if (null != renderRequest.getParameter("nroSocioPrevencion")) {
				...
				if (dbg) {
					_log.debug("[" + rid + "][PARSE][NRO_SOCIO_PREV] No vino nroSocioPrevencion. Se usa 0");
				}
			}
			...
			if (null != renderRequest.getParameter("nroCredencialPrevencion")) {
				...
				if (dbg) {
					_log.debug("[" + rid + "][PARSE][NRO_CREDEN_PREV] No vino nroCredencialPrevencion. Se usa 0");
				}
			}
