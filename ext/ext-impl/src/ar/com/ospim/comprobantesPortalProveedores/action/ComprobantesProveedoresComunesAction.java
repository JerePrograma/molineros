...
public class ComprobantesProveedoresComunesAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		if (!StringUtils.checkEmpty(cmd)) {
			...
			if(cmd.equals("buscar")){
		           ...
		           return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobantes_search_result_proveedores"));		
			...
			if(cmd.equals(Constants.EDIT) ){
				 ...
				 c.setPtoVenta(Integer.parseInt(ptovta));
				 c.setNroComprobante(nro);
				 List<ComprobanteHospital> lista = ComprobanteServiceUtil.getListaHospitales(c,0);
				 
				 session.setAttribute(WebKeysComprobantes.COMPROBANTE_PROVEEDORES_EN_EDICION, lista.get(0));
				 return mapping.findForward(getForward(renderRequest,
							"portlet.comprobantes.comprobante_proveedores_editar"));
			}
           
			if(cmd.equals(Constants.UPDATE) ){
				
				comprobante=(ComprobanteHospital)  session.getAttribute(WebKeysComprobantes.COMPROBANTE_PROVEEDORES_EN_EDICION);
			...
			if(cmd.equals("generarPrecarga")){
				
		           generarPrecarga(renderRequest,session,entidad,user);		   	
		...
		return mapping.findForward("portlet.comprobantes.comprobantes_proveedores_list");
...
private void filterComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
	...
	List<ComprobanteHospital> lista = ComprobanteServiceUtil.getListaProveedores(filtro,0);
	
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_FILTRO_PROVEEDORES ,filtro);
	session.setAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_PROVEEDORES,lista);
...
private void verificarComprobantes(RenderRequest renderRequest,HttpSession session,String entidad) throws Exception{
    ...
    List<ComprobanteHospital> lista = (List<ComprobanteHospital>) session.getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_PROVEEDORES);
    ...
    if(lista!=null && !lista.isEmpty()) {
      for(Integer i=0;i<lista.size();i++) {
    	if (ids.contains(lista.get(i).getId().toString().trim())) {
    	    ...
    	    listaAux=ComprobanteServiceUtil.getListaProveedores(filtro,0);
...
private void generarPrecarga(RenderRequest renderRequest,HttpSession session,String entidad,User user) throws Exception{
	String ids = ParamUtil.getString(renderRequest, "ids");
...
//generar Precarga Comprobante    
    			if("OK".equalsIgnoreCase(cbte.getError()) &&
        				(cbte.getGenerado()==null || !cbte.getGenerado()
        				) ) {
    				
    				ar.com.ospim.global.beans.Comprobante comprobante = new ar.com.ospim.global.beans.Comprobante(cbte.getPtoVenta(),
    						cbte.getTipoComprobante(),cbte.getNroComprobante(),cbte.getAcreedorEmpresa().getCuit(),
    						cbte.getFechaEmision(), cbte.getFechaRecepcion(),
    						StringUtils.checkNotEmpty(cbte.getImporte()) ? cbte.getImporte()
    								: null,cbte.getLetraComprobante(),cbte.getPtoVenta(),cbte.getFechaVencimiento(), null,
    								cbte.getPeriodoPrestacion());
    				
    				
    				Empresa empresa = null;
    				if (StringUtils.checkNotEmpty(cbte.getAcreedorEmpresa().getCuit())) {
    					empresa = new Empresa(cbte.getAcreedorEmpresa().getCuit(), cbte.getIdPrestador()!=null  && cbte.getIdPrestador()!=0
    							? cbte.getIdPrestador().toString():"000", null);
...
//******************    				
//******************    		
    				if(cbte.getObservaciones()!=null && !"null".equalsIgnoreCase(cbte.getObservaciones())) {
    				   comprobante.setObservaciones(cbte.getObservaciones());
    				}   
//*******************    				
    				...
    				ar.com.ospim.global.services.ComprobanteServiceUtil.save(comprobante, user, WebKeysGlobal.OSPIM);	
    				
    			    cbte.setGenerado(true);
    			    cbte.setOrden(999);
// Fin generar Precarga   			
    			}	
    	}
      }
