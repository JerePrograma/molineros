...
public class EditarOrdenesPagoAction extends PortletAction {
...
//DS				
				if(fromCajaChica!=null && fromCajaChica){
					op.getComprobantes().get(0).getAcreedorEmpresa().setSucursal("000");
				}
//DS				
				OrdenPagoServiceUtil.save(op, user, entidad);
			} else {
...
//DS - Inicio			
//			Boolean fromCajaChica = (Boolean)session.getAttribute("fromCajaChica");
			String aprobados =(String)session.getAttribute("cajachicauomacomprobantesaprobados");   
			if(fromCajaChica!=null && fromCajaChica){
				...
				CajaChicaServiceUtil.addOrdenDePagoUoma(cajaChica,op,aprobados,user.getScreenName(), WebKeysGlobal.UOMA);
				
			}
			session.removeAttribute("fromCajaChica");
			session.removeAttribute("cajachicauomacomprobantesaprobados");
