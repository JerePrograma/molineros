...
public class EditarOrdenPagoAmtimaFromReintegros extends PortletAction {
	private static Log _log = LogFactoryUtil
			.getLog(EditarOrdenPagoAmtimaFromReintegros.class);
	...
	public void processAction(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (reintegrosList != null && !reintegrosList.isEmpty()) {
				...
				String cuitAcreedor = WebKeysGlobal.CUIT_AMTIMA;
				if (seccional.getIdSeccional() == 9999) {
	...
	private List<Comprobante> setearComprobante(String cuitAcreedor,
			OrdenPagoAmtima op, BigDecimal total, Seccional seccional)
		...
		Comprobante comp = new Comprobante(0, "REI", nro_comp,
				WebKeysGlobal.CUIT_AMTIMA, current, current, total, " ", 0,
				null);
		...
		Concepto concepto = new Concepto(
				ConceptoServiceUtil.getIdReintegrosAmtima(current));
		...
		List<Comprobante> compsEnBase = ComprobanteServiceUtil
				.getComprobantesLikeNro(comp, false);
		int seq = 1;
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		return mapping.findForward(getForward(renderRequest,
				"portlet.farmacia.editar_orden_pago_entry"));
	}
