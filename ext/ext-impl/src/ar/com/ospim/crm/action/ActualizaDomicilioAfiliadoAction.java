...
public class ActualizaDomicilioAfiliadoAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
		...
		boolean emailModificado = !Objects.equals(
			    email != null ? email.trim().toLowerCase() : "",
			    emailOriginal != null ? emailOriginal.trim().toLowerCase() : ""
			);
		...
		if(!StringUtils.checkEmpty(cmd)){
			...
			try{
				if(cmd.equalsIgnoreCase(Constants.VIEW)){
					...
					renderRequest.setAttribute(WebKeysCrm.CRM_AFILIADO_EMAIL, email);
				...
				if(cmd.equalsIgnoreCase(Constants.SAVE)){
					...
					*/
				    ...
				    boolean domicilioModificado = 
				            !domicilioActual.getCalle().equalsIgnoreCase(domicilio.getCalle()) ||
				            !domicilioActual.getNumero().equalsIgnoreCase(domicilio.getNumero()) ||
				            !java.util.Objects.equals(domicilioActual.getPiso(), domicilio.getPiso()) ||
				            !java.util.Objects.equals(domicilioActual.getDepto(), domicilio.getDepto()) ||
				            !java.util.Objects.equals(domicilioActual.getPostal_codi(), domicilio.getPostal_codi()) ||
				            !java.util.Objects.equals(domicilioActual.getBarrio(), domicilio.getBarrio()); //||
				    ...
				    if (domicilioModificado && emailModificado) {				        
				        ...
				        try {
				            ...
				            // 2. Actualiza solo el email del afiliado, y la modi_fecha de la tabla afi_domicilio
				            String sql = "{call actualiza_email_y_domicilio_afiliado(?,?,?,?,?)}";
				            stmt = con.prepareCall(sql);
				            stmt.setString(1, cuilTitular);
				            stmt.setInt(2, inte);
				            stmt.setString(3, email.toLowerCase());
				            stmt.setString(4, user.getScreenName());
				            stmt.setInt(5, idDomicilio);
				            stmt.execute();
				    ...
				    if (!domicilioModificado && !emailModificado && !telefonoActualizado) {
