...
public class EmpleadoresBaseAction extends PortletAction {
	...
	protected Empresa getEmpleadorEntryCompleto(HttpServletRequest request)
			throws Exception {

		String cuit = ParamUtil.getString(request, "cuit", null);
		String sucu = ParamUtil.getString(request, "sucursal", null);
		if (sucu == null) {
			sucu = ParamUtil.getString(request, "sucursal", null);
		}
	...
	protected Seccional getSeccionalEntryCompleto(HttpServletRequest request)
			throws Exception {
		...
		if (cuit != null && cuit.length() > 0 && idSeccional>0) {
			seccional = EmpresaServiceUtil.getSeccionalCompleto(cuit,  idSeccional);
		}
