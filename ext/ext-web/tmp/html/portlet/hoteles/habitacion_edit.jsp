...
<%
	...
	Habitacion habitacion=(Habitacion)request.getSession().getAttribute(WebKeysHoteles.HABITACION_EN_EDICION);
	...
	Integer id_habitacion=habitacion!=null?habitacion.getNumero():0;
	if(habitacion==null){
...
<form action="" method="post" name="<portlet:namespace />fmS">
		...
		<table class="lfr-table">
		   <tr>
			    ...
			    <td><input id="<portlet:namespace />nro"
					...
					maxlength="4" type="text" tabindex="1" <%=!esEdicion ?"readonly=\"readonly\"":"" %>
					value='<%=habitacion.getNumero()%>' />
				</td>
				<td>Descripción:</td> 
			    <td><input id="<portlet:namespace />descripcion"
					name="<portlet:namespace />descripcion" size="20"
					maxlength="50" type="text" tabindex="2" 
					value="<%=habitacion.getDescripcion()==null?"":habitacion.getDescripcion() %>" />
				</td>
			    ...
			    <td><input id="<portlet:namespace />grupo"
					...
					value="<%=habitacion.getGrupo()==null?"":habitacion.getGrupo() %>" />
				</td>
	...
	<br>
	<input type="hidden" name="<portlet:namespace />id_habitacion"
		id="<portlet:namespace />id_habitacion" value="<%=id_habitacion%>" />
...
function <portlet:namespace />salvarEdicion(){
	if (<portlet:namespace />validarCampos()) {
		...
		'<liferay-portlet:param name="struts_action" value="/hoteles/hoteles_habitaciones_abm" />'+
