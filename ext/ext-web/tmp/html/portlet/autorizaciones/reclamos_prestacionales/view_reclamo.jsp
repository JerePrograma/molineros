...
<div id="<portlet:namespace />global"
	...
	<div id="helpComprobantes" class="containerPlus draggable {buttons:'c', skin:'default', width:'700',title:'Ayuda',closed:'true'}" style="top: 500px; left: 200px">
			Ejemplo de factura: CUIT 30999999999 FCP B 00001 - 00000028 F. Emisi�n 2/07/2020 <br>
			Comprobante: FCP,  <br>
			Letra: B,  <br>
			Suc.: 00001 ( o s�lo el n�mero 1 y el sistema autocompletar� los 0 a izquierda al guardar)  <br>
			Nro: 00000028 ( o s�lo el n�mero 28 y el sistema autocompletar� los 0 a izquierda al guardar)  <br>
			CUIT: 30999999999
	</div>
							...
							<td><table>
									<select <% if (!esEdicion) { %> disabled='disabled' <%}%>
										...
										<option value=EXCEPCION
											<%=Validator.isNotNull(reclamoprestacional) && Validator.isNotNull(reclamoprestacional.getTipoPedido())  && reclamoprestacional.getTipoPedido().equals("EXCEPCION") ? "selected" : ""  %>>EXCEPCI�N</option>
										...
										<option value="EXTRACAPITA"
											<%=Validator.isNotNull(reclamoprestacional) && Validator.isNotNull(reclamoprestacional.getTipoPedido())  && reclamoprestacional.getTipoPedido().equals("EXTRACAPITA") ? "selected" : ""  %>>EXTRAC�PITA</option>
									</select>
								</table></td>
							...
							<td><table> 
								...
								<td><select name="<portlet:namespace/>integracion" id="<portlet:namespace/>integracion"
									<% if (!esEdicion) { %> disabled='disabled' <%}%> style15"display:none"  >												
										<option value="0">Seleccione Integraci�n</option>
...
<!-- 							</tr> -->
							<!-- tr>
								...
								<td>&nbsp;</td>
								<td><liferay-ui:message key="Beneficiario en tr�mite" /></td>
							</tr-->
	...
	<fieldset class="lfr-table"> 
		...
		<legend>
			<liferay-ui:message key="Datos de la Prestaci�n" />
		</legend>
		...
		<div id="<portlet:namespace />busqueda_farmacia" align="left" width="80%">
			<table class="lfr-table"
				style="border-collapse: separate; border-spacing: 3px;">
				<tr>
					<td colspan="15"><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;F. Prestaci�n: </label></td>
		...
		<div id="<portlet:namespace />busqueda_prestaciones" align="left" width="80%">
			<table width="75%">
				<tr>
					<td ><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;F. Prestaci�n: </label></td>
			...
			<table class="lfr-table"
				...
				<tr>
					<td colspan="15">
						<fieldset class="block-labels">
							<legend>
								<liferay-ui:message key="Autorizado por �rea M�dica:" />
							</legend>
							<table>
								<tr>
									<td><label><liferay-ui:message key="Cantidad" />:</label>
									...
									<select name="<portlet:namespace />recuperable_sur" id="<portlet:namespace />recuperable_sur"
									 <% if (!esEdicion) { %> disabled="disabled" <%} %> onchange="cambiorecuperable();">
														...
														<option value="3">Integraci�n</option>
				...
				<tr>
					...
					<td><textarea rows="3" cols="100" <% if (!esEdicion) { %>
						...
						<b><liferay-ui:message
								key="La observaci�n de 200 caracteres como m�ximo." /></b></td>
					<td colspan="12">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						...
						<table>
							<tr>
								<td><b><label id="CantidadDeContactosAsociados">
											Ning�n Contacto Asociado. </label></b></td>
								<td><input type="button"
		...
		<table align="center" class="lfr-table"
			...
			<tr>
				<td colspan="5">
					<fieldset class="block-labels">
						<legend>
							<liferay-ui:message key="Justificaci�n M�dica" />
						</legend>
				...
				<td colspan="5">
					<fieldset class="block-labels">
						<legend>
							<liferay-ui:message key="Dictamen Comisi�n" />
						</legend>
						...
						<fieldset class="block-labels">
								...
								<tr>
									...
									<tr id="<portlet:namespace/>observacion_medica_tr" style="display:none;">
											...
											<td><select name="<portlet:namespace/>observacion_medica" id="<portlet:namespace/>observacion_medica"
												<% if (!esEdicion) { %> disabled='disabled' <%}%>>												
													<option value="0">Seleccione observaci�n</option>
							...
							<table align="center" width="600px">
								...
								<tr>
									...
									<td width="210px"><label>D�bito Prestadora:</label> <input
...
jQuery("#<portlet:namespace />sector").change(function(){
	...
	try {	
		...
		if (valor >= 1 && load == true){
			...
			var confirmar = false;
			confirmar=confirm ('Se eliminaran los �tems por no pertenecer al tipo correspondiente '+'\nDesea hacerlo?');
			if(confirmar){
...
function ValidarDatosObligatorios(Edicion){
	...
	if (idgestion == 0  && jQuery('#<portlet:namespace/>estado option:selected').text().trim() == 'CERRADO' ){
		alert('Debe ingresar el tipo de gesti�n del Reclamo ( Secci�n Cierre de Reclamo) ');
	...
	/* 	var isDisabled = jQuery('#<portlet:namespace />dosporciento').is(':disabled');			
	    if (!isDisabled) { */
			if(! confirm("Al seleccionar la opci�n RECHAZADO el sistema rechazar� todas las prestaciones del caso, no podr� asociarlas a reintegros. Est� seguro ?")){
				return false;	
			/* } */
	    }	
	}
		...
		if ( jQuery('#<portlet:namespace />auditoriaadministrativa').val()!="Ok" ){ // auditoria administrativa 
			...
			if (justificacion.length ==0  && resp ){ // no hay revisiones activas 
				alert('Tiene que ingresar la justificaci�n m�dica del Caso para efectuar el Cierre del Caso.');
		...
		if(tipoSelecttipopedido == 3){ //si estado = cerrado
			if (jQuery('#<portlet:namespace/>cantrevisionesactivas').val()<1  && resp ){ // no hay revisiones activas 
				alert('Recuerde, debe tener registrada por lo menos una revisi�n activa para el cierre del caso!!!!.');			
				resp=false;
			}
		}
...
// SI ES CIERRE DEL CASO NO SE CONTROLA SI SE DIERON DE BAJA TODAS LAS PRESTACIONES
    ...
    if (Edicion && addprestacion) {
    	if (valor <1   && resp){
    		alert('Debe tener ingresada por lo menos una prestaci�n');
    		resp=false;
    	}
    }else{
    		if (valor <1  && resp ){
    ...
    var integracion = jQuery("#<portlet:namespace />integracion").val();
	 if ('EXCEPCION' ==  jQuery("#<portlet:namespace />tipopedido").val()){
		if (integracion == '0'){
			alert('Debe seleccionar un tipo de integraci�n ');
			resp=false;
		}
	 ...
	 if (Edicion && resp ) {
		 if (idgestion!=0 &&idgestion!=5 ) {
	    	if (valor <1   ){
	    		alert('Debe tener ingresada por lo menos una prestaci�n para poder cerrar el reclamo.');
	    		resp=false;
	    	}
		 } 	
	 }
	 ...
	 if(codError == '6'){
	       alert('La fecha de la prestaci�n no puede ser posterior a la fecha de baja del afiliado');
		   resp=false;	   
	 }
...
function <portlet:namespace />volverEstadoObservado() {

	var confirmar = false;
	/* Recupera el Id del Reclamo */
	var idgestion=jQuery('#<portlet:namespace />id_reclamosel').val();
	
	confirmar=confirm ('Estas observando la precarga, la misma ser� devuelta ' + 
			'a la seccional. ' + '\nEstas seguro?');
...
function <portlet:namespace />editaReclamo(fromAutoriza) {
	...
	if ( ValidarDatosObligatorios(true))  {
	  ...
	  /* var data=jQuery('#<portlet:namespace/>estado').val();
	  if ( document.getElementById("<portlet:namespace />estadosel").value == data){	 	
		 document.getElementById("<portlet:namespace />estado").value="0";		
	  } */

	 /*  if ( document.getElementById("<portlet:namespace />tipopedido").disabled = "disabled"){
		document.getElementById("<portlet:namespace />tipopedido").disabled = "";	
	  } */
	
	  /*esta chanchada es porque el action toma el id de cierre de tipogestion que es un hidden y no de tipo_gestion_cierre_reclamo*/
		var idgestion=jQuery('#<portlet:namespace/>tipo_gestion_cierre_reclamo').val()
		jQuery('#<portlet:namespace />tipogestion').val(idgestion);
	    //jQuery('#<portlet:namespace />id_reclamosel').val(0);
...
function <portlet:namespace />reabrirReclamo(fromAutoriza) {
...
/* 	  var data=jQuery('#<portlet:namespace/>estado').val();
	...
	/*   if ( document.getElementById("<portlet:namespace />tipopedido").disabled = "disabled"){
	  ...
	  var accionEnCurso = document.<portlet:namespace />reclamo_fm.<portlet:namespace /><%= Constants.CMD %>.value;
	  document.<portlet:namespace />reclamo_fm.<portlet:namespace /><%= Constants.CMD %>.value='<%=Constants.RESTORE %>';
...
function manejartipogestion(){
	...
	var nroLote=jQuery('#<portlet:namespace/>nroLote').val();
	jQuery('#<portlet:namespace />tipogestion').val(idgestion);	
	if("1"==idgestion && sector=="PRESTACIONES MEDICAS" && (nroLote==null || nroLote=="" || nroLote=="0")){
		 ...
		 var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/autorizaciones/propone_lote_reclamo_prestacional';		 
			jQuery.ajax({   
				...
				success: function(data){
					var obj = jQuery.parseJSON(data);
					jQuery('#<portlet:namespace/>nroLote').val(obj.lote);
				}
			}); 
	}
...
function manejarTipoPedido(){
	...
	if ( tipoPedido.selectedIndex==0 ){
		...
		document.getElementById("<portlet:namespace />tipopedido").focus();
	}
	//if(tipoPedido.value!="EXTRACAPITA"){
	//	jQuery("#<portlet:namespace/>comprobante_letra").append(new Option("A", "A"));
	//}
...
/* 	jQuery("#<portlet:namespace/>tipo_gestion_cierre_reclamo").append(new Option("SELECCIONE LA GESTION", "0"));
	document.getElementById("<portlet:namespace />tipopedido").selectedIndex==0 */
	jQuery("#<portlet:namespace/>tipo_gestion_cierre_reclamo").append(new Option("SELECCIONE UNA OPCION", "0"));
	jQuery("#<portlet:namespace/>tipo_gestion_cierre_reclamo option[value='0']").attr("selected", true);
	if(tipoPedido.value=="EXCEPCION"){
		...
		jQuery("#<portlet:namespace/>tipo_gestion_cierre_reclamo").append(new Option("PAGADO POR MECANISMO INTEGRACION", "6"));
...
function <portlet:namespace />agregarRevision() {		
	...
	if ( DatosRevisionOk())  {
		...
		jQuery("#<portlet:namespace />botonrevision").hide();
		jQuery("#<portlet:namespace/>mensajerevisionefectuada").html("Revisi�n Efectuada, el Sistema soporta solo una revisi�n activa (No de baja).");
...
function <portlet:namespace />editarPrestacionSeleccionada(tipoAccion) {
	...
	// Solo validar montos si complet� algo del �rea m�dica
   ...
   if(flagAmparo == false && (importeFC==null || importeFC==0 || importeFC=='')){
	...
	if (nom_seleccionado_edit ==1){		 
		if (codigoSeguimiento_filtro_edit<1  ) {
		  alert('Debe seleccionar la prestaci�n');
		  return false;
		} 	
    ...
    if(fechaPrestacionDia==null || fechaPrestacionDia==0 || fechaPrestacionDia=='' ||
    	       ...
    	       alert('Debe ingresar la fecha de la Prestaci�n');
    	return false;	
    }
	...
	jQuery('#<portlet:namespace />totalEdicion').val('');
 	...
 	jQuery('#<portlet:namespace />cargoimesaEdicion').val('');
	...
	document.getElementById("<portlet:namespace />recuperable_sur").selectedIndex = 0; 	
...
function <portlet:namespace />agregarPrestacion() {	
	...
	var cargoimesa= jQuery('#<portlet:namespace />cargoimesa').val();
    ...
    var recuperableSur  =  jQuery('#<portlet:namespace />recuperable_sur').val();
    ...
    var frecuenciacontrol =document.getElementById("<portlet:namespace/>frecuencia");
    ...
    var tipoPedidoControl =document.getElementById("<portlet:namespace />tipopedido");
	...
	/* document.getElementById("<portlet:namespace/>sector").disabled = "disabled"; */	  
 	...
 	jQuery('#<portlet:namespace />cargoospim').val('');
 	jQuery('#<portlet:namespace />cargops').val('');
 	jQuery('#<portlet:namespace />cargoimesa').val('');
 	jQuery('#<portlet:namespace />reconocidoSSS').val('');
 	jQuery('#<portlet:namespace />observacion_prestacion').val('');
	document.getElementById("<portlet:namespace />frecuencia").selectedIndex = 0;
	jQuery('#<portlet:namespace />troquel').val(""); // farmacia 
	jQuery('#<portlet:namespace />codigoSeguimiento_filtro').val("");// prestaciones medicas
	//jQuery('#<portlet:namespace />recuperable_sur').attr('checked', false);
	document.getElementById("<portlet:namespace />recuperable_sur").selectedIndex = 0;
	...
	if (jQuery('#<portlet:namespace/>estado').val()==3){
		if (varCantRevisiones > 0 ){
			...
			if(varDebitoTercerizadora == true){
				jQuery("#<portlet:namespace />debitoprestadora")[0].checked = true;

			}																												
		}else{
			alert("Debe agregar una Revisi�n");
...
/* function onOffControlesRequest(valor) {
	document.getElementById("<portlet:namespace />fechaseccionalDia").disabled = valor;
	document.getElementById("<portlet:namespace />fechaseccionalMes").disabled = valor;
	document.getElementById("<portlet:namespace />fechaseccionalAnio").disabled = valor;
} */


function <portlet:namespace />imprimirReclamo(){
...
function ValidaDatosReclamo(){
    ...
    var tipoNomencladorPrestacion =jQuery('#<portlet:namespace />tiponomenclador').val();
	 ...
	 jQuery.ajax({   
	   ...
	   if(codError == '1'){
	       alert('La fecha de la prestaci�n no puede ser posterior');
		   respuesta=false;	   
		}
		...
		if(codError == '4'){
		   	alert('No existe Prestaci�n en el nomenclador');
			respuesta=false;
		 }
		if(codError == '5'){
...
function validarExisteComprobante( params ) {
    ...
    var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/autorizaciones/validar_existe_comprobante';
	   ...
	   if(rtaExisteCompro){
		  alert('Ya existe una prestaci�n en esa fecha para el mismo comprobante');
		  respuesta=false;
...
*/
    ...
    if(isNaN(cargoimesadouble)) {		jQuery('#<portlet:namespace />cargoimesaEdicion').val()  ; cargoimesadouble=0; 	}
    if(isNaN(reconocidoSSS)) {		jQuery('#<portlet:namespace />reconocidoSSSEdicion').val()  ; reconocidoSSS=0; 	}
...
//  valida la suma de los importes no debe superar el importe ingresado 
    ...
    if(total==0 && (importeFC>0 ||importeFCEdicion>0) && estado==3){
	   alert('Debe ingresar los importes en el �rea M�dica');
	   return false; 
    }
...
*/
    ...
    if ( document.getElementById("<portlet:namespace />tipopedido").selectedIndex==1) { // tipo de pedido excepcion 
	  if (total!=importedouble && estado==3){
		alert('El importe total de la prestaci�n debe coincidir con la suma de cargo Ospim y cargo tercerizadora');
		return false;
	  }
    }
    ...
    if(recuperable==2){
    	if(reconocidoSSS>0){
    	   	   alert('El importe reconocido debe estar vac�o');
    	   	   jQuery('#<portlet:namespace />reconocidoSSSEdicion').val('');
        	   return false;
    	}
    }else{
...
/*	
jQuery('#<portlet:namespace />importeEdicion').val(importedouble);
jQuery('#<portlet:namespace />cargoospimEdicion').val(cargoospimdouble);
jQuery('#<portlet:namespace />cargopsEdicion').val(cargopsdouble);
*/	
...
function ValidaMontos()
{
	...
	var cargoimesadouble = parseFloat(jQuery('#<portlet:namespace />cargoimesa').val());
	var reconocidoSSS = parseFloat(jQuery('#<portlet:namespace />reconocidoSSS').val());
	var estado =jQuery("#<portlet:namespace/>estado").val();
...
//	totalCargos= cargoospimdouble + cargopsdouble;
	...
	if ( document.getElementById("<portlet:namespace />tipopedido").selectedIndex==1) { // tipo de pedido excepcion 
		if (totalCargos!=importedouble && estado=='3'){
			alert('El importe total de la prestaci�n debe coincidir con la suma de Cargo Ospim m�s Cargo Tercerizadora.');
			return false;
		}
	}
   ...
   if ( document.getElementById("<portlet:namespace />tipopedido").selectedIndex==2) { // tipo de pedido reintegro
	    if (importedouble <totalCargos && estado=='3'){
			alert('El importe total de prestaci�n debe coincidir con la suma de a Cargo Ospim m�s Cargo Tercerizadora');
			return false;
		}   
		if (totalCargos==0 && estado=='3'){
			alert(' la suma de a Cargo Ospim m�s a Cargo Tercerizadora debe ser mayor que cero.');
			return false;
		}
   }
...
function crit_busqueda() {
	  var input=document.getElementById('<portlet:namespace />buscadorcie10buscador').value.toUpperCase();
	  var output=document.getElementById('<portlet:namespace/>cie_diez').options;
	  var dato;       
      pos=jQuery('#<portlet:namespace />posforcie10').val();
      for(var i=pos;i<document.getElementById("<portlet:namespace/>cie_diez").options.length ;i++) {
		  dato = output[i].text;		  
		  if(dato.indexOf(input)>-1){
		        output[i].selected=true;		        
		        jQuery('#<portlet:namespace />codigoCie10').val(output[i].value);
		        jQuery('#<portlet:namespace />posforcie10').val(++i);
		        return false;
		      }		 
      } 
...
/* function validaFacturacionDirectayReintegro(){
	...
	if (jQuery('#<portlet:namespace />montoPsPrestaciones').val()>0 && jQuery('#<portlet:namespace />montoPsPrestaciones').val()!="" ){// forzar facturacion directa o reintegro
		...
		if (document.getElementById("<portlet:namespace />tipopedido").selectedIndex==1){ // excepcion 
 		...
 		if (document.getElementById("<portlet:namespace />tipopedido").selectedIndex==2){ // reintegro
			validaReintegro();			
	}
}
} */
...
/* function validaReintegro(){
		document.getElementById("<portlet:namespace/>tipo_gestion_cierre_reclamo").selectedIndex=3;		
	    document.getElementById("<portlet:namespace/>tipo_gestion_cierre_reclamo").disabled = true;
		jQuery('#<portlet:namespace />tipogestion').val(4); // reintegro 
} */
...
function seteaControlesFacturacionDirecta(estadoTrueFalse){
	...
	/* document.getElementById("<portlet:namespace />incluido_convenio_gerenciadora").disabled = estadoTrueFalse; */
	document.getElementById("<portlet:namespace />debitoprestadora").checked =estadoTrueFalse;
	/*  document.getElementById("<portlet:namespace />debitoprestadora").disabled = estadoTrueFalse; */	
	/*  document.getElementById("<portlet:namespace/>tipo_gestion_cierre_reclamo").disabled = estadoTrueFalse;*/
}
...
/* function habilitarControlesCierre() {
	document.getElementById("<portlet:namespace/>sector").disabled =false;  
	document.getElementById("<portlet:namespace />tipopedido").disabled =false; 
	document.getElementById("<portlet:namespace />debitoprestadora").disabled =false; 
	document.getElementById("<portlet:namespace />incluido_convenio_gerenciadora").disabled =false; 
	document.getElementById("<portlet:namespace/>tipo_gestion_cierre_reclamo").disabled =false;
} */
...
//	var cuiltitular= jQuery('#<portlet:namespace />cuil_titular').val();
	...
	*/
	...
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/autorizaciones/actualiza_domicilio';
	jQuery.post(url,{
