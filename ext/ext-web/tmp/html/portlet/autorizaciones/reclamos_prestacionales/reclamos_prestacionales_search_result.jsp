...
<portlet:defineObjects/>
			<%
				...
				List<ItemReclamoPrestacionalesTotal> reclamosPrestacionalesList = new ArrayList<ItemReclamoPrestacionalesTotal>();
				reclamosPrestacionalesList= (ArrayList<ItemReclamoPrestacionalesTotal>)renderRequest.getAttribute(WebKeysAutorizaciones.BUSQUEDA_RECLAMOS_PRESTACIONALES );
				if (reclamosPrestacionalesList == null || reclamosPrestacionalesList.size() == 0) {
					reclamosPrestacionalesList = (ArrayList<ItemReclamoPrestacionalesTotal>) portletSession.getAttribute(WebKeysAutorizaciones.BUSQUEDA_RECLAMOS_PRESTACIONALES, PortletSession.PORTLET_SCOPE);
				}
				...
				if(null!=reclamosPrestacionalesList){
					 	...
					 	for (int i = 0; i < reclamosPrestacionalesList.size(); i++) {
					 		ItemReclamoPrestacionalesTotal reclamo  = (ItemReclamoPrestacionalesTotal) reclamosPrestacionalesList.get(i);
		 					ResultRow row = new ResultRow(reclamo,reclamo.getId_reclamo(), i);
			 				PortletURL rowURL = renderResponse.createRenderURL();
			 				rowURL.setWindowState(LiferayWindowState.MAXIMIZED);
			 							 				
			 				rowURL.setParameter("struts_action","/autorizaciones/editar_reclamosprestaciones_entry");
...
/* 			 				row.addText(reclamo.getAfiliado() != null && reclamo.getAfiliado().getUltimo_plan().getDescripcion()!=null   ?    String.valueOf(reclamo.getAfiliado().getUltimo_plan().getDescripcion()   ) : "" , rowURL); */							// Action
							row.addText(reclamo.getPrestacionesConcat() != null ? reclamo.getPrestacionesConcat() : "");
							if(showABMButtons && reclamo.getEstadoreclamo() ==3 ) {
								row.addJSP( "left", SearchEntry.DEFAULT_VALIGN,"/html/portlet/autorizaciones/reclamos_prestacionales/relaciona_reclamoprestacional.jsp");
							}
							//|| showReadOnlyReclamPrestac
							if((showABMButtons) && reclamo.getEstadoreclamo() !=3 ) {
								row.addJSP( "left", SearchEntry.DEFAULT_VALIGN,"/html/portlet/autorizaciones/reclamos_prestacionales/editar_borrar_reclamosprestacionales.jsp");
							} else {
