...
function pasarParametrosAParentSecc(id, param) {	
    jQuery("#<portlet:namespace />id_seccional_afiliado").val(id);
    jQuery("#<portlet:namespace />seccional_afiliado").val(param);
    jQuery("#<portlet:namespace />secc_seleccionada_afiliado").val("1");    
    try {
        if (jQuery("#<portlet:namespace />id_seccional_afiliado").val() == "") {
    		jQuery("#<portlet:namespace />id_seccional_afiliado").val(id);    	
    		jQuery("#<portlet:namespace />seccional_afiliado").val(param);    	
    		jQuery("#<portlet:namespace />secc_seleccionada_afiliado").val("1");
        }
    ...
    jQuery("#<portlet:namespace />btnBuscarSeccionalAfiliado").hide();
    <portlet:namespace />cerrarSeccAfiliado();
 }
...
<%
	...
	if(null!=seccionales){
		...
		if(total==1){
			...
			%>
				<script type="text/javascript">
					pasarParametrosAParentSecc("<%=seccUnica.getId()%>", "<%=seccUnica.getDescripcion()%>");
				</script>
			<%
		//More de una coincidencia
			...
			List resultRows = searchContainer.getResultRows();
		 	for (int i = 0; i < seccionales.size(); i++) {
				...
				sb.append("<a href='javascript:pasarParametrosAParentSecc(\"");
