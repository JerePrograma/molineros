...
boolean buscarDestino= ParamUtil.getBoolean(request, "buscar_destino");
...
%>
		<table width="100%">
			<tr>
					...
					<td valign="top">
						<input id="<portlet:namespace />id_prestador<%=suf%>" name="<portlet:namespace />id_prestador<%=suf%>"  type="text" maxlength="6" size="6" value="" 
						    <% if (!esEditable) { %>
								<%="readonly='readonly'" %> 
							<%} else {%> 
								onBlur="javascript:<portlet:namespace />pierdeFoco<%=suf%>();" onKeyUp="javascript:<portlet:namespace />buscarEntidadOnDiv<%=suf%>(event)" onkeydown="cuitCambio<%=suf%>();" 
							<%} %>/>  <!-- onchange="cambiaCuit<%=suf%>();" -->
					</td>
				<%}%>
				<td valign="top"><liferay-ui:message key="cuit" />&nbsp;
					<input  id="<portlet:namespace />cuit_entidad<%=suf%>" name="<portlet:namespace />cuit_entidad<%=suf%>" maxlength="11" size="13" type="text" value="<%=cuit%>"   
						<% if (!esEditable) { %> 
							<%="readonly='readonly'" %>
						<%} else {%> 
							onBlur="javascript:<portlet:namespace />pierdeFoco<%=suf%>();" onKeyUp="javascript:<portlet:namespace />buscarEntidadOnDiv<%=suf%>(event)" onkeydown="cuitCambio<%=suf%>();"
						<%} %>/> <br/>  <!-- onchange="cambiaCuit<%=suf%>();" -->
				...
				<td valign="top">
					<input  id="<portlet:namespace />sucursal_entidad<%=suf%>" name="<portlet:namespace />sucursal_entidad<%=suf%>" maxlength="6" size="5" type="text" value=""
						<% if (!esEditable) { %> 
							readonly='readonly' 
						<%} else {%> 
			...
			<tr>
				<td width="100%" colspan="8" align="center" style="font-family: monospace; font-size: 8; font: smaller; ">
					<label id="lblempresaIVA">&nbsp; </label><label id="lblempresaGAN">&nbsp; </label>
					<label id="lblempresaMONO">&nbsp; </label><label id="lblempresaREG">&nbsp; </label>
				</td>
			</tr>	
		</table>
		
		<input type="hidden" name="<portlet:namespace />id_seccional<%=suf_entidad%>" id="<portlet:namespace />id_seccional<%=suf_entidad%>" value=""/>	
		<input type="hidden" name="<portlet:namespace />ent_seleccionada<%=suf%>" id="<portlet:namespace />ent_seleccionada<%=suf%>" value=""/>
		<input type="hidden" name="<portlet:namespace />imp_ganancias" id="<portlet:namespace />imp_ganancias" value=""/>
		<input type="hidden" name="<portlet:namespace />imp_iva" id="<portlet:namespace />imp_iva" value=""/>
		<input type="hidden" name="<portlet:namespace />monotributo" id="<portlet:namespace />monotributo" value=""/>
		<input type="hidden" name="<portlet:namespace />codigo_regimen" id="<portlet:namespace />codigo_regimen" value=""/>
		<input type="hidden" name="<portlet:namespace />cbu" id="<portlet:namespace />cbu" value=""/>
...
function buscarEnPopUp<%=suf%>(cur){
	    ...
	    var id_prestador=jQuery("#<portlet:namespace />id_prestador<%=suf%>").val();   
	   	...
	   	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/buscar_padron_entidad_afip&cuit_entidad='+cuit_entidad+
		  '&entidad='+encodeURI(entidad)+'&sucursal='+sucursal+'&suf=<%=suf%>'<%if(id_seccional>0){%>+'&id_seccional=<%=id_seccional%>'<%}%>;
...
function <portlet:namespace />buscarEntidadOnDiv<%=suf%>(e){	
	...
	if(jQuery("#<portlet:namespace />ent_seleccionada<%=suf%>").val() == "1" &&  (keyPressed==8 || keyPressed==46)){		
		...
		jQuery("#<portlet:namespace />id_seccional<%=suf_entidad%>").val("");
		jQuery("#<portlet:namespace />id_prestador<%=suf%>").val("");
		jQuery("#<portlet:namespace />imp_ganancias").val("");
		jQuery("#<portlet:namespace />imp_iva").val("");
		jQuery("#<portlet:namespace />monotributo").val("");
		jQuery("#<portlet:namespace />codigo_regimen").val("");
		jQuery("#<portlet:namespace />cbu").val("");
		
		document.getElementById('lblempresaIVA').innerHTML = '' ;
		document.getElementById('lblempresaGAN').innerHTML = '' ;
		document.getElementById('lblempresaMONO').innerHTML = '' ; 
		document.getElementById('lblempresaREG').innerHTML = '' ;
...
function <portlet:namespace />cerrarDivBusquedaPadrones<%=suf%>(){		
	...
	try{
	  if (document.getElementById("<portlet:namespace />nro_comprobante_cc") != null){
		sugerirNumero<%=suf%>();
	  }
	}catch(err){}  
}
...
var id_seccionalJs = "<%=id_seccional%>";
	...
	jQuery("#<portlet:namespace />id_seccional<%=suf_entidad%>").val(id_seccionalJs);

	function pasarParametrosAParentBusquedaPadrones<%=suf%>(cuit, razon, sucursal, id_seccional, imp_ganancias, imp_iva, monotributo, codigo_regimen, cbu) {
		...
		jQuery("#<portlet:namespace />id_seccional<%=suf_entidad%>").val(id_seccional);
		...
		<%}%>
	 	jQuery("#<portlet:namespace />imp_ganancias").val(imp_ganancias);
		jQuery("#<portlet:namespace />imp_iva").val(imp_iva);
		jQuery("#<portlet:namespace />monotributo").val(monotributo);
		jQuery("#<portlet:namespace />codigo_regimen").val(codigo_regimen);
		jQuery("#<portlet:namespace />cbu").val(cbu); 
		
	 }
	 <%if(buscarDestino){%>	 
	 function pasarParametrosAParentBusquedaPadronesDestino<%=suf%>(cuit, razon, sucursal, id_seccional, destino , imp_ganancias, imp_iva, monotributo, codigo_regimen, cbu) {		
		jQuery("#<portlet:namespace />cuit_entidad<%=suf%>").val(cuit);
	    jQuery("#<portlet:namespace />entidad<%=suf_entidad%>").val(razon);
	    jQuery("#<portlet:namespace />sucursal_entidad<%=suf%>").val(sucursal);	    
	    //jQuery("#<portlet:namespace />empl_seleccionada").val("1");	    	   
