		...
		<fieldset class="block-labels">
				...
				<table class="lfr-table">			
					<tr>
						...
						<td><select name="<portlet:namespace/>tipo_debitos_tercerizadoras" id="<portlet:namespace/>tipo_debitos_tercerizadoras">
						...
						<td>
							...
							<input type="checkbox" name="<portlet:namespace />grabarDebitos" id="<portlet:namespace />grabarDebitos" value="false"/>
						</td>
						<td>
					...
					<tr>
						<td colspan="3">
							<div id="<portlet:namespace />archivos_debitos">
								<jsp:include
									page='/html/portlet/liquidaciones/reportes/archivos_debitos_terciarizadoras.jsp' />
							</div>
						</td>
					</tr>
				</table>
...
function <portlet:namespace />buscarMovimientos(){
	...
	//var grabar_debitos=jQuery("#<portlet:namespace/>grabarDebitos").val();
	...
	var tipo_proceso = jQuery("#<portlet:namespace/>tipo_proceso").val();	
...
jQuery('#<portlet:namespace />buscando').show();
	...
	if (valiadSiHayReporte()== false){
		alert('No hay datos para el período solicitado.');
		return false;
	} 
	
	
	var busquedaNom = {"periodo":periodo,"inte":inte,"fechaDesdeDia":"01","fechaDesdeMes":desde_mes,
				       "fechaDesdeAnio":desde_anio,"tipo_debito":tipo_debitos_tercerizadoras,"tipo_proceso":tipo_proceso,
	 			       "cmd":"search"};

	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"><portlet:param name="struts_action" value="/liquidaciones/debitos_tercerizadoras" /></portlet:renderURL>';
	
	jQuery('#<portlet:namespace />listado_debitos').load(url,busquedaNom, function(){
				jQuery('#<portlet:namespace />buscando').hide();											      
		});	
	...
	/* 
	var url = '/xlsservlet/?reporte=REPORTE_DEBITO_TERCERIZADORAS' 
		+ '&fechaDesdeDia=01'
    ...
    */
	...
	jQuery('#<portlet:namespace />tipo_debitos_tercerizadoras').change(function(){
		valiadSiHayReporteGrabado(); 
	});
