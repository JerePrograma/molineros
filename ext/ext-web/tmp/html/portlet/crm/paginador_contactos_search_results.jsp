...
try{
 	total_reg= (Integer)session.getAttribute(WebKeysCrm.FILTRO_BUSQUEDA_CONTACTOS_TOTAL_REGISTROS); // "total_registros"
	offset_reg= (Integer)session.getAttribute(WebKeysCrm.FILTRO_BUSQUEDA_CONTACTOS_OFFSET_REG);     // "offset_reg"  

	/*Este ajuste es para mantener la busqueda de inbox */
	if(total_reg == null && offset_reg == null){
		BusquedaContactoFiltro f = (BusquedaContactoFiltro) session.getAttribute(WebKeysCrm.FILTRO_BUSQUEDA_CONTACTOS);
...
<%-- 	<input type="hidden" name="<portlet:namespace/>viene_de"   id="<portlet:namespace/>viene_de" value="<%=viene_de%>" > --%>	
	<tr>
		...
		<td><select name="<portlet:namespace/>pagina_sel" id="<portlet:namespace/>pagina_sel" 		
				onchange="javascript:<portlet:namespace />buscarContactos();" >
