...
function buscarEnPopUp<%=suf%>(cur){
	    ...
	    var id_prestador=jQuery("#<portlet:namespace />id_prestador<%=suf%>").val();   
	   	...
	   	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/buscar_padron_entidad_crm&cuit_entidad='+cuit_entidad+
...
var id_seccionalJs = "<%=id_seccional%>";
	...
	function pasarParametrosAParentBusquedaPadrones<%=suf%>(cuit, razon, sucursal, id_seccional) {
	 ...
	 <%if(buscarDestino){%>	 
	 function pasarParametrosAParentBusquedaPadronesDestino<%=suf%>(cuit, razon, sucursal, id_seccional, destino) {		
		jQuery("#<portlet:namespace />cuit_entidad<%=suf%>").val(cuit);
	    jQuery("#<portlet:namespace />entidad<%=suf_entidad%>").val(razon);
	    jQuery("#<portlet:namespace />sucursal_entidad<%=suf%>").val(sucursal);	    
	    //jQuery("#<portlet:namespace />empl_seleccionada").val("1");	    	   
