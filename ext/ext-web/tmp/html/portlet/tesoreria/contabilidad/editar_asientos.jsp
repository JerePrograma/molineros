	...
	<tbody id="sortable">
		<c:if test="${asiento.id == 0}">
			<tr id="tr_1" ><td  id="td_1" style="width: 22px"><span><img src="/html/images/move.png" style="width: 20px; height: 20px"/></span><!-- no dejar espacio entre el tr y el td porque sino se rompe el js --></td>
				...
				<td><input type="text" name="observaciones_1" value="" size="50"/></td>
		...
		<c:if test="${asiento.id != 0}">
				 <c:forEach var="det" items="${asiento.detalle}">
				 	<tr id="tr_${det.pase}" ><td  id="td_${det.pase}" style="width: 22px"><span><img src="/html/images/move.png" style="width: 20px; height: 20px"/></span><!-- no dejar espacio entre el tr y el td porque sino se rompe el js --></td>
						...
						<td><input type="text" name="observaciones_${det.pase}" value="${det.observaciones}" size="50"/></td>
...
<script type="text/javascript">	
	...
	function agregar() {   
 	  ...
 	  jQuery('<tr id="tr_'+index+'">' + 
		...
		'<td><input type="text" name="observaciones_'+index+'" value="" size="50"/></td>'+
	...
	function validar(){
		...
		for (var index = 0; index < jQuery( "#sortable" ).children().size(); index++){
			...
			var comprobante = row.childNodes[3].childNodes[0].value;
			var debe = row.childNodes[4].childNodes[0].value;
			var haber= row.childNodes[5].childNodes[0].value;
			var errorDebe = false;
			...
			if (!errorDebe && !errorHaber){
				if (jQuery.trim(haber) != "" && parseFloat(haber) != 0){
					row.childNodes[4].childNodes[0].value = "0";
					debe = "0";
				} else {
			...
			if (IsNumeric(debe) || debe == "0"){
				totalDebe = Math.round((totalDebe + (Math.round(parseFloat(row.childNodes[4].childNodes[0].value) * 100) / 100)) *100) /100;
			}
			
			if (IsNumeric(haber) || haber == "0"){
				totalHaber = Math.round((totalHaber + (Math.round(parseFloat(row.childNodes[5].childNodes[0].value) * 100) / 100)) *100) /100;
			}
