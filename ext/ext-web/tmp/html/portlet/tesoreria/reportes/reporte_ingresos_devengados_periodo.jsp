...
<div id="<portlet:namespace/>div_ranking_deuda_empresa" >
		<fieldset class="block-labels">
				<legend><liferay-ui:message key="reporte-ingresos-devengados-periodo" /></legend>
				<table class="lfr-table">			
					<tr>
						...
						<td>
							<input id="<portlet:namespace />generar" value="<liferay-ui:message key="generar"/>" title="<liferay-ui:message key="generar" />" type="button" onClick="javascript:<portlet:namespace />ingresosDevengados();"/>
						</td>
						
						<td>
							<input id="<portlet:namespace />excel" value="<liferay-ui:message key="excel"/>" title="<liferay-ui:message key="excel" />" type="button" onClick="javascript:<portlet:namespace />ingresosDevengadosExcel();"/>
						</td>
...
<portlet:namespace />verificaEstadoReporteIngresoDevengado();

function <portlet:namespace />ingresosDevengados(){	
	...
	if(confirmar){
	  ...
	  var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/tesoreria/ingresos_devengados_periodo_generacion';
...
function <portlet:namespace />ingresosDevengadosExcel(){
	var url = '/xlsservlet/?reporte=REPORTE_INGRESOS_DEVENGADOS' 
	url += '&rnd=' + Math.floor(Math.random()*100);
	window.location.href =url;
}


function <portlet:namespace />verificaEstadoReporteIngresoDevengado() {
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/tesoreria/reporte_verifica';
	url+= '&reporte=reporte.ingreso_devengado_periodo';
	jQuery.ajax({   
		url: url,
		async: false,
