...
<portlet:defineObjects/>
			...
			List<Banco> bancos = (List<Banco>)request.getSession().getAttribute(WebKeysTesoreria.BANCOS_EN_SESSION);
				...
				//Si debe mostrarse el btn de agregar afiliado								
					...
					if (recibo != null){
						cheques = recibo.getChequesNoDepositados();
					}
					if(null!=cheques){
					 	...
					 	for (int i = 0; i < cheques.size(); i++) {
		 					...
		 					if(chq.getCuentaBancaria()!=null &&
		 							chq.getCuentaBancaria().getDescripcion()!= null){
		 						row.addText(chq.getCuentaBancaria().getDescripcion());
		 					}else{
		 						row.addText("");
	 			 			}	
							...
							// Action
		 					if (showABMButtons && esEdicion){
			 					...
			 					sb.append("/common/delete.png\" onClick=\"javascript:borraChequeADepositar('");
...
<script type="text/javascript">
	document.getElementById("total_cheques_no_depositados").value = "<%= totalImporte.toString()%>";
</script>		
