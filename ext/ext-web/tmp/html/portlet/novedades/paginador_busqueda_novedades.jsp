...
try{
 	total_reg= (Integer)session.getAttribute(WebKeysAfiliados.FILTRO_BUSQUEDA_NOVEDADES_TOTAL_REGISTROS); // "total_registros"
	offset_reg= (Integer)session.getAttribute(WebKeysAfiliados.FILTRO_BUSQUEDA_NOVEDADES_OFFSET_REG);     // "offset_reg"  

	/*Este ajuste es para mantener la busqueda de inbox */
	if(total_reg == null && offset_reg == null){
		BusquedaNovedadesSssFiltro f = (BusquedaNovedadesSssFiltro) session.getAttribute(WebKeysAfiliados.FILTRO_BUSQUEDA_NOVEDADES);
...
<table class="lfr-table">
	...
	<tr>
		...
		<td><select name="<portlet:namespace/>pagina_sel" id="<portlet:namespace/>pagina_sel" 
				onchange="javascript:<portlet:namespace />buscarNovedades();" >
