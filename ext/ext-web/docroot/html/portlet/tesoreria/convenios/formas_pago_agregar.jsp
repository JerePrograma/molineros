...
if (convenio != null && convenio.getId() != 0 && !esEdicion){
	esEdicion = false;
}
...
<table class="lfr-table">
  <tr>
	<td width="100%" valign="top" style="width: 1200px;">
  <fieldset class="block-labels">
	<legend>Pagos</legend>
	<table class="lfr-table">
		<%if (esEdicion){ %>
		<tr>
			<td width="100%" style="width: 1100px;">
			<table style="border-spacing:3px; border-collapse: separate;">
				<tr>
					<td>Cantidad de Cuotas:</td>
					<td><input type="text" id="cant_cuotas" name="cant_cuotas" size="4" onchange="calcularCapitalInteres(); actualizarDetalleCuotas();"/></td>
					<td>Tasa aplicada:</td>
					<td><input type="text" id="tasa" name="tasa" size="4" value="3" onchange="calcularCapitalInteres(); actualizarDetalleCuotas();"/></td>
					<td colspan="3"><input type="button" value="Ver detalle propuesto " onclick="helpCuotas(event, 'helpCuotas')"/></td>
				</tr>
				<tr>
					<td>
					...
					<td colspan="5">Cheque&nbsp;<input type="radio" name="tipo_pago" id="radio_cheque" value="Cheque"/>
						&nbsp;&nbsp;Depósito&nbsp;<input type="radio" name="tipo_pago" id="radio_deposito" value="Deposito"/>
						&nbsp;&nbsp;Pagaré&nbsp;<input onClick="javascript:cambiarEtiquetas();" type="radio" name="tipo_pago" id="radio_pagare" value="Pagare"/></td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<input type="text" value="" name="<portlet:namespace />nro_cheque" id="<portlet:namespace />nro_cheque" style="width: 65px;"/>
					</td>
					<td>
				...
				<tr>
					...
					<td>
						<input type="text" value="" name="<portlet:namespace />capital_cheque" id="<portlet:namespace />capital_cheque" onkeydown="allowOnlyDigitsAndDecimals(event)" onchange="agregarCeros(this);" style="width: 65px;"/>
					</td>
					<td>
						<liferay-ui:message key="interes"/>:
					</td>
					<td>
						<input type="text" value="" name="<portlet:namespace />interes_cheque" id="<portlet:namespace />interes_cheque" onkeydown="allowOnlyDigitsAndDecimals(event)" onchange="agregarCeros(this);"/>
					</td>
					<td colspan="3">&nbsp;</td>
				</tr>
				<tr>
		...
		<tr>
			<td colspan="7">
				...
				<liferay-util:include page="/html/portlet/tesoreria/convenios/convenio_pagos_search_result.jsp">
...
<script type="text/javascript">
	function <portlet:namespace />agregarPago(){
			...
			if (cuotaNro <= 0){
				alert("El número de cuota debe ser mayor a 0");
			...
			var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/tesoreria/agregar_convenio_pago&cheque_nro=' +chequeNro 
			+ '&id_banco=' + idBanco
			...
			jQuery('#<portlet:namespace />cheques').load(url, function() {
														jQuery('#<portlet:namespace />agregandoCheque').hide();	
														 ...
														 jQuery('#<portlet:namespace />interes_cheque').val("");		
	...
	function borraDepositoBancario(cuotaNro){
		if(!confirm("<liferay-ui:message key='are-you-sure-you-want-to-delete-this-entry'/>")){
			...
			var cuitEmisor = jQuery('#<portlet:namespace />cuit_entidad').val();
			var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/tesoreria/sacar_convenio_pago&tipo_pago=deposito&cuota_nro_cta_bcria=' + cuotaNro +'&cuitEmisor=' + cuitEmisor + '&esEdicion=' + "<%=esEdicion%>" 	+'&accionOriginal='+ "<%=act%>";
			jQuery('#<portlet:namespace />cheques').load(url, function() {
	...
	function borraCheque(chequeNro, idBanco, idCtaBcria){
		if(!confirm("<liferay-ui:message key='are-you-sure-you-want-to-delete-this-entry'/>")){
			...
			var cuitEmisor = jQuery('#<portlet:namespace />cuit_entidad').val();
			var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/tesoreria/sacar_convenio_pago&tipo_pago=cheque&cheque_nro=' +chequeNro; 
			url = url+  '&id_banco=' + idBanco+ '&idCtaBcria=' + idCtaBcria+'&cuitEmisor=' + cuitEmisor + '&esEdicion=' + "<%=esEdicion%>" 	+'&accionOriginal='+ "<%=act%>";
			jQuery('#<portlet:namespace />cheques').load(url, function() {
																						jQuery('#<portlet:namespace />agregandoCheque').hide();     
																						sumarInteresYCapital();       															
																			   }
	...
	function filtrarCtaBancaria() {
		...
		var idBanco=jQuery('#<portlet:namespace />id_banco').val();
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/tesoreria/buscarCtasBancariasPorBanco&idBanco='+idBanco+'&cuit='+cuitEmisor+'&sucur='+sucuEmisor;
