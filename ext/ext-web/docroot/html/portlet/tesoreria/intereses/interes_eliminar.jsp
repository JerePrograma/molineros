...
<%
	...
	if (editStr != null && editStr.trim().toUpperCase().equals(WebKeysInteres.ACCION_DELETE)){
		esEdicion = true;
	}
...
<form action="" method="post" name="<portlet:namespace />fmNTRS">
		...
		<table class="lfr-table">
			<tr>
				...
				<td><label><liferay-ui:message key="fecha-inicio" />:</label>
					...
					maxlength="10" type="text" disable value='<%=(origFechaIni == null) ? "" : origFechaIni %>' /></td>
				<td><label><liferay-ui:message key="fecha-fin" />:</label> <input
					...
					maxlength="10" type="text" value='<%=(origFechaFin == null) ? "" : origFechaFin %>' /></td>
	...
	<input type="hidden" name="<portlet:namespace />fecha_inicio"
		id="<portlet:namespace />fechaInicioInteres" value="<%=origFechaIni%>" />
	<input type="hidden" name="<portlet:namespace />fecha_fin"
		id="<portlet:namespace />fechaFinInteres" value="<%=origFechaFin%>" />	
...
function <portlet:namespace />initDateFields(){
	if(<%=_interes!=null && _interes.getFechaInicio()!=null && _interes.getFechaInicio() != ""%> ){
		jQuery("#<portlet:namespace />fechaInicioInteres").val('<%=origFechaIni%>');
		jQuery("#<portlet:namespace />fechaFinInteres").val('<%=origFechaFin%>');
		jQuery("#<portlet:namespace />importeInteresDia").val('<%=auxInteres%>');    
	}  
	document.getElementById("<portlet:namespace/>fechaInicioInteres").disabled=true;
	document.getElementById("<portlet:namespace/>fechaFinInteres").disabled=true;
	document.getElementById("<portlet:namespace/>importeInteresDia").disabled=true;
