...
<portlet:defineObjects/>
			...
			List<SeguimientoSurPrestador>  detalles= new ArrayList<SeguimientoSurPrestador>();
	        if(seguimiento.getPrestadores()!=null && seguimiento.getPrestadores().size()>0 ){
	        	detalles=seguimiento.getPrestadores();
	        }
			...
			PortletURL portletURLTercerizadora = renderResponse.createRenderURL();
	 		...
	 		headerNamesTercerizadora.add("CUIL ");
			...
			if(null!=detalles){
 				List resultRowsInspector = searchContainer.getResultRows();
 			 	for (int i = 0; i < detalles.size(); i++) {
 			 		SeguimientoSurPrestador detalle = detalles.get(i);
 			 		if (detalle.getBaja_fecha()!=null ){
 			 			continue;
 			 		}
	 					ResultRow row = new ResultRow(detalle, detalle.getId(), i);
	 					row.addText(detalle.getIdPrestador().toString());
	 					row.addText(detalle.getCuitPrestador());
	 					row.addText(detalle.getDescripcionPrestador() );
	 					...
	 					if (showABMButtons && esEdicion && seguimiento.getCierre_fecha()==null && seguimiento.getBaja_fecha()==null){
		 					...
		 					sb.append("/common/edit.png\" onClick=\"javascript:editaSeguimientoSurPrestador('");
		 					sb.append(detalle.getId());
		 					sb.append("','");
		 					sb.append(detalle.getIdPrestador());
		 					sb.append("','");
		 					sb.append(detalle.getCuitPrestador());
		 					sb.append("','");
		 					sb.append(detalle.getDescripcionPrestador());
		 					sb.append("','");
		 					...
		 					sb.append(themeDisplay.getPathThemeImages());
		 					sb.append("/common/delete.png\" onClick=\"javascript:borraSeguimientoSurPrestador('");
		 					sb.append(detalle.getId());
		 					sb.append("');\" />");
