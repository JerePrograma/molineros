...
<%@ page import="ar.com.ospim.liquidaciones.services.CartillaConvenioServiceUtil" %>
			...
			List<Comprobante> comprobantesErroneos = (ArrayList<Comprobante>)renderRequest.getPortletSession().getAttribute(WebKeysComprobantes.COMPROBANTES_RESULT_ERRONEOS);
					...
					for (int i = 0; i < comprobantesProcesados.size(); i++) {
					 ...
					 for (int i = 0; i < comprobantesErroneos.size(); i++) {
					     ...
					     if (yaProcesado) {
					         sb.append("<img id='").append(iconoId).append("' alt='Datos procesados' ");
					         sb.append("title='Datos procesados' ");
					         sb.append("src='").append(themeDisplay.getPathThemeImages()).append("/common/checked.png' ");
					         sb.append("style='cursor:default;' />");
					     } else {
					         sb.append("<img id='").append(iconoId).append("' alt='Procesar datos' ");
					         sb.append("title='Procesar datos' ");
					         ...
					         sb.append("onclick=\"javascript:").append(renderResponse.getNamespace())
					           .append("descargarPrestadores('").append(titulo).append("', '").append(tipo).append("');\"/>");
					     }
...
function <portlet:namespace />descargarPrestadores(titulo, tipo) {
	var iconoId = "";
	...
	var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>' +
			  '&struts_action=/<%= portlet_name %>/administracion_prestadores' +
			  '&cmd=descargarDatosPrestadores' +
			  '&tipo=' + tipo +
			  '&titulo=' + titulo +
			  '&rnd=' + Math.random();
	...
	jQuery('#<portlet:namespace />busquedaComprobDiv').load(url, function() {
		jQuery(iconoId)
			...
			.attr('alt', 'Datos procesados')
			.attr('title', 'Datos procesados') 
			.css('cursor', 'default')
			.off('click'); 
