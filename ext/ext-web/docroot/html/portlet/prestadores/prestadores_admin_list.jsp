...
<%
	...
	if (portlet_name == null || portlet_name.trim().equals("")){
		portlet_name = "prestadores";
	}else if(renderResponse.getNamespace().equals("_PRE_1_")){
		portlet_name = "prestadores";
	}
...
%>
		...
		<legend>Descarga de Prestadores</legend>
	...
	<table>		
				<tr>
					<td>
						<input id="<portlet:namespace />descargar" value="Descargar Datos" title="Descargar datos de prestadores" type="button"
						onclick="javascript:<portlet:namespace />downloadPrestadores();"/>							
					</td>
					<td>
					&nbsp;
					</td>	
		...
		<div align="center" id="<portlet:namespace />busquedaComprobDiv">
				<liferay-util:include page="/html/portlet/prestadores/comprobantes_download_prestadores.jsp"/>
		</div>	
...
function <portlet:namespace />downloadPrestadores(){		
				...
				var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/<%=portlet_name%>/administracion_prestadores';
				url += '&cmd=importarDatosPrestadores'+
