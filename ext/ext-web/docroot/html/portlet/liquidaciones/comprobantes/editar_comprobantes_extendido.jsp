...
<%@ page import="ar.com.ospim.liquidaciones.ComprobanteConceptoIVAInvalidoException"%>
...
<%
	...
	if(nuevo){
	  Comprobante cpte = new Comprobante();
	  cpte.setGravadoIVA27(BigDecimal.ZERO);
	  cpte.setGravadoIVA21(BigDecimal.ZERO);
	  cpte.setGravadoIVA105(BigDecimal.ZERO);
	  cpte.setIva27(BigDecimal.ZERO);
	  cpte.setIva21(BigDecimal.ZERO);
	  cpte.setIva105(BigDecimal.ZERO);
	  cpte.setExento(BigDecimal.ZERO);
	  cpte.setPercepcionIVA(BigDecimal.ZERO);
	  cpte.setPercepcionIIBB(BigDecimal.ZERO);
	  cpte.setOtrosTributos(BigDecimal.ZERO);
	  cpte.setImporteComprobante(BigDecimal.ZERO);
	  session.setAttribute(WebKeysLiquidaciones.COMPROBANTE_EXTENDIDO_ACUMULADO, cpte);
	}  
...
<liferay-ui:error exception="<%= ComprobanteConceptoIVAInvalidoException.class %>" message="exception-comp-concepto-iva-invalido" />
...
<table width="100%">
		...
		<td> 
			...
			<select id="<portlet:namespace />tipo_comprobante" onchange="actualizarCuit();actualizarLetra(true);buscarConceptoCuitTipo();<%if(portlet_name.equals("uoma")){%>habilitarCuotas();<%}%>"
				...
				<% if(portlet_name.equals("uoma")){%>					
					...
					<option value="FOS">FCP OSR</option>
				<%}%>
			</select>
		</td>
	...
	<tr>
		...
		<td>
			<input type="text" id="<portlet:namespace />importe_comprobante" name="<portlet:namespace />importe_comprobante" onkeydown="allowOnlyDigitsAndDecimals(event)"
				onchange="agregarCeros(this); sugerirEnConcepto(this); calculoConcepto();" value ="<%=comp != null &&comp.getImporteComprobante() != null ? comp.getImporteComprobante().toString() :  new String("") %>" <% if (!esEdicion) {%>readonly="readonly"<%} %>/>
		</td>
		<%if(portlet_name.equals("uoma")){%>
				<td colspan="5">
	...
	<tr>
		<td colspan="9">
			<liferay-util:include page="/html/portlet/liquidaciones/comprobantes/conceptos_agregar_extendido.jsp">
	...
	<tr>
		...
		<%} %>
		<% if (comp != null && comp.getAlta_fecha() != null && !soloVer){  %>
...
//BE sure to follow the pattern revealed below
	...
	comboNDB[cont++]=new Option("A","A");
	comboNDB[cont++]=new Option("B","B");
	comboNDB[cont++]=new Option("C","C");
	<% if(!portlet_name.equals("uoma")){%>	
	    comboNDB[cont++]=new Option("D","D");
	    comboNDB[cont++]=new Option("E","E");
	    comboNDB[cont++]=new Option("M","M");
	<%}%>
...
//HASTA ACA!
	...
	function actualizarLetra(crearCombo){
		...
		if(crearCombo==true){			
			if(sel.value=="RCB"){
				...
				populate(comboNCR);
			}else if (sel.value=="FCP" || sel.value=="FOS"){
				populate(comboFCP);
	...
	function sugerirNumero(){		
		...
		<%if(portlet_name.equals("uoma")){%>		
			...
			if ((((sel.value == "RCB" || sel.value == "TCK") && letra=='X') || sel.value == "ANT" || sel.value == "VAR") && trim(cuit)!= ""){
		<%}else{%>		
			if ((sel.value == "ANT" || sel.value == "VAR" ) && trim(cuit)!= ""){
		<%}%>
	...
	<% if (comp != null && comp.getLetraComprobante() != null) {%>
	 ...
	 function sugerirEnConcepto(){
		 ...
		 var isugerido=roundNumber(parseFloat(trim(comp))-parseFloat(jQuery("#total_conceptos").val()));
		 if(isugerido==null || isNaN(isugerido)){
			 isugerido="";
		 }
		 
		 //jQuery("#<portlet:namespace />importe_concepto").val(isugerido);
		 jQuery("#<portlet:namespace />importe_gravado").val(isugerido);
