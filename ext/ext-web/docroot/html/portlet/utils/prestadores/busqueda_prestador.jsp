...
<%
	...
	String controlaPlanPrestadorStr = ParamUtil.getString(request, "controlaPlan");
	if (controlaPlanPrestadorStr == null || controlaPlanPrestadorStr.equals("false")){
		controlaPlanPrestadorStr ="false";
	}
	boolean controlaPlanPrestador = Boolean.parseBoolean(controlaPlanPrestadorStr);
	...
	String soloHospitalesStr = ParamUtil.getString(request, "esLiquidadorHospital");
	if (soloHospitalesStr == null || soloHospitalesStr.equals("false")){
		soloHospitalesStr ="false";
	}
	boolean soloHospitales = Boolean.parseBoolean(soloHospitalesStr);

%>

	<table class="lfr-table">
		<tr>
			<td><liferay-ui:message key="cod-prestador" /></td>
			<td>
				<input id="<portlet:namespace />id_prestador<%=ext%>" name="<portlet:namespace />id_prestador<%=ext%>"  type="text" maxlength="6" size="6" value="" 
				    <% if (!esEditable) { %>
						<%="readonly='readonly'" %> 
					<%} else {%> 
						onBlur="javascript:<portlet:namespace />pierdeFocoPd<%=ext%>();" 
						onKeyUp="javascript:<portlet:namespace />buscarPrestadorOnDiv<%=ext%>(event);" 
						onkeydown="allowOnlyDigits(event);"
						<%if(controlaPlanPrestador){%>onchange="<portlet:namespace />buscarPlanesDelPrestador();"<% } %>
					<%} %>/>
			</td>
...
<input id="<portlet:namespace />prest_seleccionada<%=ext%>" name="<portlet:namespace />prest_seleccionada<%=ext%>" type="hidden" value=""/>
...
function <portlet:namespace />buscarPrestador<%=ext%>() {	
    ...
    if (id_prestador == null || id_prestador == '0'){
    	id_prestador = "";
    }
    ...
    if(id_prestador.length == 0 && cuit_prestador.length==0 && prestador.length==0){
	    ...
	    var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=<%=searchURL%>&id_prestador='+id_prestador+
		  '&cuit_prestador='+cuit_prestador+
	    		  '&nombre_prestador='+encodeURIComponent(prestador)+'&ext=<%=ext%>'+'&solo_vigentes=<%=solo_vigentes%>' +
	    		  '&solo_hospitales=<%=soloHospitales%>';	    
...
function pasarParametrosAParentPd<%=ext%>(cuit,nombre,id) {
    ...
    <%if(controlaPlanPrestador){%><portlet:namespace />buscarPlanesDelPrestador();<% } %>
     try{
    	if(cuit!=cuitJs<%=ext%>){
   	       <portlet:namespace />buscarContratoPrestadorAut('prestador');
        }  
    }catch(err){}
...
function seleccionaCamposPrestad<%=ext%>(id, cod, param) {
    ...
    //Agregado solo para que funcione con el Alta de Tratamientos
    try{
