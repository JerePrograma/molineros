...
boolean showABMButtons = PermissionUtil.userContainsRole(user,WebKeysLiquidaciones.ROL_ENTIDAD_UOMA);
...
%>
				...
				<tr>
					...
					<td colspan="3">
						<select name="<portlet:namespace/>estado" id="<portlet:namespace/>estado">								
								...
								<option value="1" selected>No Pagados</option>
		...
		<input type="hidden" name="esAmtima" value="esAmtima"/>
...
<script type="text/javascript">
	...
	jQuery('#<portlet:namespace />buscar').click(function(){		
		...
		var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>"/>&struts_action=/uoma/buscar_comprobantes';
	...
	function <portlet:namespace />altaComprobante() {
		var url = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/uoma/editar_comprobante" /></portlet:renderURL>';
