...
boolean showABMButtons = true;//PermissionUtil.userContainsRole(user,WebKeysAutorizaciones.ROL_ABM_RECLAM_PREST );
...
if (prestacionesDelReclamo != null && prestacionesDelReclamo.size()>0){
	...
	cantPrestaciones= prestacionesDelReclamo.size() ; 
 	for (int i = 0; i < prestacionesDelReclamo.size(); i++) {	    
		...
		if ( true ){ enabledestado = "disabled='disabled'"; }
		...
		row.addText(sb1.toString());
	 	...
	 	//row.addText(format2D.format(presreclamo.getImporte()));	 	ge
		...
		row.addText(format2D.format(presreclamo.getComprobanteImporte()));	 	
	 	row.addText(format2D.format(presreclamo.getComprobanteCantidad() * presreclamo.getComprobanteImporte()));
...
//tipoEdicion=3 es Rechazo prestacion
	...
	if (jQuery("#<portlet:namespace />datos_edicion_prestacion").is(":hidden")){	
		...
		jQuery("#<portlet:namespace />datos_prestacion_ingreso_obs").hide();
...
function borrarPrestacionconvalida(idRegistro){
	...
	if (!jQuery("#<portlet:namespace />datos_edicion_prestacion").is(":hidden")){
	    ...
	    var params = "&<%= Constants.CMD %>=" + "<%= WebKeysAutorizaciones.RECLAMO_PRESTACIONAL_SECCIONAL %>";
