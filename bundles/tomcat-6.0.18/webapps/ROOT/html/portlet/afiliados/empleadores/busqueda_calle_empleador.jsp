	...
	function <portlet:namespace />buscarCalleOnDiv(e) {
		...
		if (jQuery("#<portlet:namespace/>localidad").val() == "265") {
		    ...
		    if (calle.length > 0) {
	            ...
	            <%} else if(renderResponse!=null && renderResponse.getNamespace()!=null && renderResponse.getNamespace().equals("_CGT_1_")) {%>					
	            	var url = '<portlet:renderURL windowState="<%=LiferayWindowState.EXCLUSIVE.toString()%>"/>&struts_action=/cgt_ddhh/buscar_calle&calle='+calle;
				<%}else{%>
